...
## Ownership
- `hen-compiler-core/` — Rust contract crate (`epi-s1-hen-compiler-core`); its own DOX node. Public surface in `src/lib.rs`: compile_plan, coordinate, frontmatter, graph_promotion, graph_sync, ledger, residency, wikilinks, smart_env, l_alignments, property_intelligence, relation_inference, artifact_evidence; contract tests in `tests/`.
