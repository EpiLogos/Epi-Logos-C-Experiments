...
- `src/base_view.rs` — `s1'.base.ensure` contract surface: derives base-view schemas from validated [[CT]] frontmatter contracts supplied through `BaseEnsureParams`, emits idempotent `{coordinate}.base-view.md` notes, and enforces reflection-only residency for base-view artifacts.
...
- `src/coordinate.rs` — coordinate validity (`is_valid_coordinate`). Per Track 45.T45.1 the validator accepts the full multi-level grammar the ontology holds so the Neo4j→repo `/map` reflection round-trips deep coordinates: multi-level sub-paths (`M2-5-0`, `M2-3-0-360`, range-unconstrained sub-positions), legacy deep/lens psychoid tags (`#0-2-9`, `#-0`), context frames raw and canonical (`M0-4.0/1`, `M0-4.(0/1)`, `M2-5-(0/1)-6`), and a trailing prime at any exposed level (`M2-5-0'`); base QL position stays bounded 0-5 (over-range/malformed still rejected). Grammar mirrors graph-services `CoordinateArrayParser::parse_one` — keep them in sync ([[S1-SPEC]]).
- `src/wikilinks.rs` — wikilink parsing (`WikilinkTarget`) and the canonical rename-reconciliation mutation law: bare/path-qualified targets preserve `.md`, heading/block anchors, and aliases while parser-excluded escaped/fenced examples remain literal; also owns coordinate-residency-on-move refusal helpers (`coordinate_for_residency`, `coordinate_residency_refusal`).
...
- `src/l_alignments.rs`, `src/property_intelligence.rs`, `src/relation_inference.rs`, `src/artifact_evidence.rs` — alignment, property, relation-inference, evidence law; artifact evidence collects C2 entity-candidate, C3 diagram/canvas, C4 type/MOC, and C5 World-graduation evidence from frontmatter plus `World/Types` ancestry; per CCT-15 it also owns `classify_c_layer` (the `s1'.type.classify_c_layer` law: frontmatter beats World/Types ancestry beats kind routing — templates → C1, entities/properties/tags → C2, canvases/diagrams → C3, same-name MOC index canvases → C4, flat World residency → C5; C-prime/CT/CPF branches are audited as reflective scaffold, never plain-C authoritative; unroutable artifacts are refused) and `c_layer_evidence_kind` (single evidence-kind source shared with graph promotion).
- `src/smart_env.rs` (private mod) — link-candidate suggestion (`suggest_link_candidates`).
- `src/frontmatter_mutation.rs` — typed, idempotent `c_n_*` sequence append and validated `c_n_*` scalar replacement plus the pure accepted-Q amendment plan: it replaces only a public canonical `q_*` scalar, writes its paired `qm_*_review_epoch_*` stamp, requires review/provenance/open-question evidence, and preserves the markdown body. Filesystem selection, domain-value validation, and persistence remain adapter concerns. Canon update flag: [[S1-SPEC]].
- `tests/` — contract tests (module_surface, base_view, frontmatter, frontmatter_mutation, q_vocabulary_unknown_key_rejection, compile_plan, graph_promotion_intent, c_layer_typology_classification, entity_candidate_lifecycle, wikilink_parser, etc.); `base_view` covers CT4b schema derivation, idempotent emission, and reflection-residency refusal; `frontmatter_mutation` covers sequence append/de-duplication, scalar replacement/body preservation, and refusal law; `wikilink_parser` covers block-anchor parsing, rename fidelity/exclusions, and coordinate-residency mismatch refusal; `entity_candidate_lifecycle` covers `:World` label/root-link/span-pointer promotion and graduation preservation. `tests/fixtures/` holds test data.
