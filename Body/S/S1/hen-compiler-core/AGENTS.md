...
- `src/frontmatter_mutation.rs` — typed, idempotent append law for coordinate-owned `c_n_*` string sequences; it preserves the markdown body and refuses missing frontmatter, non-coordinate keys, and incompatible existing values. Filesystem selection and persistence remain adapter concerns. Canon update flag: [[S1-SPEC]].
