//! `s1'.*` gateway handlers — the vault-governance methods, at their coordinate.
//!
//! # Coordinate
//!
//! | Field | Value |
//! |-------|-------|
//! | Coordinate | S1 |
//! | Residency  | Body/S/S1/hen-compiler-core/src/s1_handlers.rs |
//! | Position   | #1 — Obsidian / Material Container, governed vault access |
//! | Actualises | [[S1-SPEC]], Track 53 T53.04 |
//!
//! These bodies were `epi-cli/src/gate/s1_hen.rs`. Its own AGENTS.md line already
//! called it "a thin gateway adapter" whose law is Hen's — so the adapter now
//! lives where its law does. Nothing here reaches upward: the module needs only
//! `std`, `serde_json`, and Hen's own modules.
//!
//! `s1'.q_articulation.accept` is deliberately NOT here. It is a genuine
//! cross-coordinate composite — human approval (S5), graph verification and sync
//! (S2), then the vault write (S1) — and S1 may import neither S2 nor S5. It
//! composes at the composition root instead, which is the one place licensed to
//! depend on all three. Pushing it into S2 merely because S2→S1 is legal would
//! relocate S1's vault-write law into the graph crate to dodge a wiring problem.
//!
//! # Does NOT own
//! * Method routing or the registry — S3 (`epi-s3-gateway::router`).
//! * Graph verification, sync, or human-approval gating.
...
// T53.04: these are S1's own request/receipt shapes and are now reached at S1,
// not borrowed back down from the S3 contract crate. `gateway-contract` still
// re-exports them for its other consumers, but this adapter — whose whole law
// is Hen's — has no reason to route through S3 to describe S1.
use crate::s1_vault::{
...
pub fn resolve_vault_root(params: &Value) -> Result<PathBuf, String> {
...
fn discover_frontmatter_contracts(
    vault_root: &Path,
    ct_type: &str,
) -> Result<Vec<crate::ValidatedFrontmatterContract>, String> {
...
pub fn canonical_bimba_relative_path(path: &str) -> Result<PathBuf, String> {
...
pub fn atomic_replace(path: &Path, content: &[u8]) -> Result<(), String> {
    let parent = path
        .parent()
...
//
// The lifecycle LAW lives in `crate::entity_lifecycle`
...
// ---------------------------------------------------------------------------
// The port: S1 declares which `s1'.*` methods it owns and how to reach them.
// ---------------------------------------------------------------------------
...
/// Every `s1'.*` handler in this module has the same shape — take params,
/// return JSON or a message — so one adapter covers all of them rather than
/// thirteen near-identical structs.
struct S1Handler(fn(&Value) -> Result<Value, String>);

impl<C: Send + Sync> MethodHandler<C> for S1Handler {
    fn handle<'a>(&'a self, _ctx: &'a C, request: &'a MethodRequest) -> BoxFuture<'a, MethodResult> {
        let handler = self.0;
        Box::pin(async move {
            handler(&request.params)
                .map(MethodOutcome::immediate)
                // The S0 dispatcher mapped every one of these with
                // `internal_error`. The wire is frozen (Track 53 gotcha 7), so
                // the code stays `internal` even though several are really
                // invalid-params — changing it belongs to a wire track.
                .map_err(MethodError::internal)
        })
    }
}

/// Method name → owning function. `s1'.vault.rename_file` and
/// `s1'.vault.move_file` are two names for one handler, exactly as the
/// dispatcher's shared match arm had them.
pub const S1_METHODS: &[(&str, fn(&Value) -> Result<Value, String>)] = &[
    ("s1'.vault.read_file", read_file),
    ("s1'.vault.write_file", write_file),
    ("s1'.base.ensure", base_ensure),
    ("s1'.vault.rename_file", rename_or_move_file),
    ("s1'.vault.move_file", rename_or_move_file),
    ("s1'.semantic.suggest_links", suggest_links),
    ("s1'.type.classify_c_layer", type_classify_c_layer),
    ("s1'.entity.capture", entity_capture),
    ("s1'.entity.classify", entity_classify),
    ("s1'.entity.promote_to_type", entity_promote_to_type),
    ("s1'.world.graduate", world_graduate),
    ("s1'.entity.list", entity_list),
    ("s1'.world.list_entities", world_list_entities),
];

/// Register every method S1 owns into a registry the composition root builds.
///
/// Generic over the context because S1 needs none of it — which is the point:
/// the coordinate that owns these methods names no gateway type to serve them.
pub fn register_s1_handlers<C: Send + Sync + 'static>(
    registry: &mut MethodRegistry<C>,
) -> Result<(), DuplicateMethod> {
    for (method, handler) in S1_METHODS {
        registry.register(*method, Arc::new(S1Handler(*handler)))?;
    }
    Ok(())
}
...
mod port_tests {
    ...
    #[test]
    fn every_registered_method_is_an_s1_prime_name_and_unique() {
        let mut registry: MethodRegistry<NoCtx> = MethodRegistry::new();
        register_s1_handlers(&mut registry).expect("no duplicate method names");
        assert_eq!(registry.len(), S1_METHODS.len());
        for (method, _) in S1_METHODS {
            assert!(method.starts_with("s1'."), "{method} is not an s1' method");
            assert!(registry.contains(method), "{method} did not register");
        }
    }

    #[test]
    fn rename_and_move_share_one_handler_as_the_dispatcher_did() {
        let rename = S1_METHODS
            .iter()
            .find(|(m, _)| *m == "s1'.vault.rename_file")
            .unwrap()
            .1;
        let mv = S1_METHODS
            .iter()
            .find(|(m, _)| *m == "s1'.vault.move_file")
            .unwrap()
            .1;
        assert!(std::ptr::fn_addr_eq(rename, mv), "one arm served both names");
    }

    #[tokio::test]
    async fn a_handler_failure_keeps_the_frozen_internal_code() {
        let mut registry: MethodRegistry<NoCtx> = MethodRegistry::new();
        register_s1_handlers(&mut registry).unwrap();
        // No vaultRoot and no ambient env -> the handler errors; the dispatcher
        // used `internal_error` for every s1' arm and that must not drift.
        let error = registry
            .dispatch(
                &NoCtx,
                &MethodRequest::with_params("s1'.vault.read_file", serde_json::json!({})),
            )
            .await
