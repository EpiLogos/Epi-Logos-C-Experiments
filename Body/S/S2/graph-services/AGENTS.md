...
- `src/` modules — service surface: `sync_coordinator.rs`, `bidirectional_sync.rs`, `retrieval_query.rs` + `retrieval/` (coordinate/graphrag/hybrid), `doctor.rs`, `ontology.rs`, `dataset_import/` (node/edge/validation), `graph_api.rs`, `coordinate.rs`, `semantic.rs`, `embeddings.rs`, `seed.rs`, `pointers.rs`, `relationship_manager.rs`, `gds.rs`, `constraint.rs`, `schema.rs`, plus others.
- `tests/` — contract test suite (graph_api, coordinate_query, promotion_policy, semantic_cache, semantic_doc_includes_locality_signature, q_5_fallback_bucket_law, neo4j, dataset_import_live, etc.).
