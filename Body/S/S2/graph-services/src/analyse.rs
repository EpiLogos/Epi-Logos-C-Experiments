//! Document resonance analysis — graph-law authority.
//!
//! S2 owns the deterministic 72-vector resonance pipeline used by the
//! `epi graph analyse-resonance` CLI verb (S0 adapter). The trait
//! [`ResonanceAnalyser`] is the contract for future Pi-agent / LLM-backed
//! implementations; the [`DeterministicAnalyser`] is the baseline
//! keyword-affinity implementation that gives the corpus a typed
//! signature even before any LLM is wired.
