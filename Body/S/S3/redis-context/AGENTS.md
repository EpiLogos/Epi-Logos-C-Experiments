...
## Ownership
- `Cargo.toml` — crate manifest (`epi-s3-redis-context`, lib name `epi_s3_redis_context`); deps on `epi-kernel-contract` (downward to S-root, Track 53 T53.06), `redis`, `serde`, `serde_json`.
- `src/lib.rs` — crate root / public surface: re-exports of `redis_cache` and `aeon_eval`, plus the **re-export** of the S2↔S3 residency declaration (`RedisRuntimeRole`, `CacheTier`, `RedisConfig`, `S2_GRAPH_SEMANTIC_NAMESPACE`, `S3_TEMPORAL_NAMESPACE`, `REDISVL_*`, `redisvl_service_script`/`redisvl_setup_script`). **Track 53 T53.06** moved those declarations down to `epi-kernel-contract` `src/redis_residency.rs`: they are statements about the boundary between S2's semantic-cache namespace and S3′'s temporal namespace, and holding them here forced `epi-s2-graph-services` to declare an upward S2→S3 dependency to name its own namespace. Every `epi_s3_redis_context::…` import path still resolves; the S3 surface is unchanged.
- `src/redis_cache.rs` — `RedisCache` (the live client), `RedisKey`, the CCT-21 BeingPattern temporal handle key families, and the re-export of `CacheTier`/`RedisConfig` from the root contract. The client stays HERE — connecting, `PING`, `SETEX` are runtime, and `epi-kernel-contract` is deliberately runtime-free. `RedisKey`'s constructors (`session_now`, `day_context`, `agent_orientation`, …) are S3′ temporal semantics and must not follow the tiers downward.
