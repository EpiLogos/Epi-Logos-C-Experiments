...
- `src/redis_cache.rs` — `CacheTier` (Live/Active/Hot/Warm/Cold TTLs), `RedisCache`, `RedisConfig`, `RedisKey`.
...
## Verification
- `cargo test -p epi-s3-redis-context` (or `make rust-test` from repo root).
