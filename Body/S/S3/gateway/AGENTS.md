...
- `src/spacetime.rs` — SpaceTimeDB subscription / reducer client + fallback (~78k).
...
- `tests/` — contract + smoke tests (`dispatch_contract`, `session_store_contract`, `oracle_spread_contract`, `live_gateway_smoke`, etc.).
...
## Verification
- `cargo test -p epi-s3-gateway` (or `make rust-test` from repo root).
