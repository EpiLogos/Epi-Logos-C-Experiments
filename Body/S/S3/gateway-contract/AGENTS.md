...
- Canon update flag: [[S3-SPEC]], [[S0-SPEC]], [[M2'-SPEC]], and [[M3'-SPEC]] should record the admitted public `kernelBridge.m2.epogdoonProjection(address72)`, `kernelBridge.m2.cymaticMonoPolyState(address72)`, `kernelBridge.m2.planetaryElementalWeights()`, and `s5.oracle.iching.cast` routes; the I-Ching route is an [[S5]] governance adapter over the [[Nara]] oracle and returns only a persisted, provenance-bearing cast receipt.
