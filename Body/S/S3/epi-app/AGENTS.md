...
## Purpose
**RETIRING — do not build on this.** `epi-electron` — "M' Domain Electron Shell for Epi-Logos" (per `package.json` `description`); an Electron + React/Vite renderer shell superseded by the [[M'-SYSTEM-SPEC]] / epi-theia direction. New M' UI work belongs in `Body/M/epi-theia`, not here.
