...
- `src/native_gateway.rs` — canonical native `s5.episodic.*` gateway operations with session-scoped memory retrieval; never contacts the deprecated HTTP sidecar.
...
- `tests/native_gateway.rs` — native gateway deposit/search and session-isolation behavior without a sidecar.
