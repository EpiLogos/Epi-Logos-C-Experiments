...
- `src/q_review.rs`, `src/adapters.rs` — Q-review curation: detector-policy load from `~/.epi-logos/config.toml [autoresearch]`, durable per-day JSONL plus metadata persistence, VAK-filtered queue reads (an unrun day returns explicit absence), and the S5 night-pass conversion of S2's embedding-complete Bimba snapshot; and non-Aletheia adapters. Queue entries identify editable canonical `q_*` articulations; `qm_*_review_epoch` remains staleness evidence, never a pair-composition target. Queue production is read-only and does not own proposal, review-decision, or promotion mutation.
- `src/capacity_workflows/` — capacity-workflow façade split per S5-ARCHITECTURE §5 F1: `registry.rs` (seven profiles, including human-gated `TuningReview`), `runner.rs` (deterministic capacity-slice runner + routing/snapshot; restored from pre-split `capacity_workflows.rs` after the 17.T17.4 split orphaned it), `nara_voice.rs`, `recursive_review.rs`, `spine_inspector.rs`, `aletheia_lineage.rs`; `mod.rs` re-exports so `capacity_workflows::*` is unchanged.
- `src/anamnesis_proposer.rs`, `src/tuning_review.rs` — Tier-2 tuning proposal vocabulary and the governed lifecycle: Class A creates human-gated [[S5]] review records; Class B is eligible for auto-application only with a complete constitutional triplet; structural invariants, Tier-3 requests, locks, and runaway ceilings fail closed. Does NOT own registry/config/audit persistence or [[Aletheia]] Tier-3 dispatch.
- `src/resonance_corpus/` — file-backed resonance corpus/checkpoint surface for accumulated `(document, 72-vector)` training pairs, Bimba-node target vectors, dry-run and deliberate training plans, validation metrics, and paired EBM checkpoint/corpus-snapshot exports using the runtime's shared Candle architecture identifier.
- `src/resonance_ebm/` — feature-gated (`resonance_ebm`) position 5' runtime head: N-channel `MathemeHarmonicProfile` encoders, swappable attention, Candle-executed tritone-symmetric 72-vector inference, versioned architecture/channel-validated checkpoints, per-element kernel invocation, learned bioquaternion projection, Candle autograd over `q_p`, mirror invariant, and training-surface citations.
...
## Verification
- `cargo test --manifest-path Body/S/S5/epii-autoresearch-core/Cargo.toml` (or `make rust-test`).
...
- Resonance EBM runtime: `cargo check --offline --manifest-path Body/S/S5/epii-autoresearch-core/Cargo.toml --features resonance_ebm`; `cargo test --offline --manifest-path Body/S/S5/epii-autoresearch-core/Cargo.toml --features resonance_ebm resonance_ebm::mirror_consistency_loss`; `cargo test --offline --manifest-path Body/S/S5/epii-autoresearch-core/Cargo.toml --features resonance_ebm --test resonance_ebm_runtime`.
