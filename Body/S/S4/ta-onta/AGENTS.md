...
- `shared/` — `vak_address.ts` (VAK address mirror), `coordinate_phase.ts` (phase-preserving coordinate helper), `entitlement*.ts` (entitlement loader/model), and their `*.test.ts`.
