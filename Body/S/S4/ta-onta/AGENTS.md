...
## Purpose
Canonical S4' carrier source home: the six ta-onta carrier classes (khora/hen/pleroma/chronos/anima/aletheia) plus the spine that composes their contributions into a single PI extension entry. No crate/package manifest here — it is a TypeScript runtime tree consumed by the parent S4 surface.
...
- `spine/compositor.ts` — `SpineCompositor` (injection assembly under char budget, ledger extract, compiler passes, unified query).
- `spine/types.ts` — `SpineContribution` / `InjectionSlot` / `LedgerChannel` / `CompilerPass` / `SessionContext` interfaces all carriers implement.
- `shared/` — `vak_address.ts` (VAK address mirror) + `entitlement*.ts` (entitlement loader/model) and their `*.test.ts`.
