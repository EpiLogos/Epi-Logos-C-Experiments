...
export function refreshTopology() {
  return spawnSync("epi", ["--json", "agent", "extensions", "status", "--agent", "main"], {
    encoding: "utf8",
    timeout: 30_000,
  });
}
...
export function validateHenSync() {
  const result = spawnSync("epi", ["--json", "agent", "extensions", "status", "--agent", "main"], {
    encoding: "utf8",
    timeout: 30_000,
  });
  ...
  const stale = state ? state !== "synced" : /stale|lag|out[- ]of[- ]date/i.test(stdout);
