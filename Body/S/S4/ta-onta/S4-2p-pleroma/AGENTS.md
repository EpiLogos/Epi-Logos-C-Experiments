...
## Purpose
The Pleroma carrier (S4-2' in ta-onta): the bounded-execution substrate registry — "the execution substrate registry" owning eight bounded primitives, PI tool registration surface, execution-mode enforcement, Techne terminal/session tooling, and `techne_vama_summon` registration (per `CONTRACT.md`). TypeScript extension tree (no crate/package manifest); folds onto the S2 entity layer.
Canon: [[ARCHITECTURE-DIAGRAM-PACK]] -> [[S4-2-SPEC]] (carrier law); layer ground [[S4-SPEC]] / [[S2-SPEC]].

## Ownership
- `extension.ts` — PI extension entry; registers bounded primitives, damage-control, tilldone, Techne terminal tools, the tmux-topology handoff, and `techne_vama_summon` refusal-law metadata.
- `CONTRACT.md` — binding carrier contract: responsibility, 8 bounded primitives, tmux-authoritative/cmux-visible topology handoff, PI hook seams, invariants.
...
- `S2'/skills/` — atomic skill substrate (tmux authority, cmux visible projection, ralph-tui, worktrunk, context7, techne-* relay/spawn/webmcp bridges); `pleroma-skill-proxy/` owns the central skill-store symlink projector for `claude-native`, `codex-native`, and `hermes-acp` launch surfaces.
- `S2'/evals/` — 6 eval suites (atomic-tools, discharge, klein, manifest, ouroboros, topology-routing).
- `tests/` — `terminal_tools.test.ts` (node:test command-boundary contract); real topology allocation is proven by `epi-cli/tests/agent_tmux_topology.rs` against an isolated tmux socket.
...
## Verification
- `node --test "Body/S/S4/ta-onta/S4-2p-pleroma/tests/terminal_tools.test.ts"` (bounded-primitive registry + Techne terminal-tools contract test).
