...
- `modules/parent-slice-bifurcation.ts` — 12.T12.31's subscribed `agent:team:dispatch:complete` c=1/c=0 router: consumes Anima's successful native team-dispatch completion envelope; c=1 persists the folded parent DAY/NOW horizon through the existing session continuation boundary; c=0 returns to Anima with only the appended micro-history and the same [[ConversationSliceHandle]].
