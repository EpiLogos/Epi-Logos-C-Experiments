...
export default function (pi: ExtensionAPI) {
	...
	// Also scan .pi/agents/ (pi-vs-cc pattern) plus ta-onta constitutional roots
	...
	for (const [dir, source] of [
		[join(cwd, ".pi", "extensions", "ta-onta", "anima", "S4'", "agents"), "anima/S4'"],
		[join(cwd, ".pi", "extensions", "ta-onta", "aletheia", "S5'", "agents"), "aletheia/S5'"],
	] as const) {
		const agents = scanAgents(dir);
		if (agents.length) {
			groups.push({ source, commands: [], skills: [], agents });
		}
	}
