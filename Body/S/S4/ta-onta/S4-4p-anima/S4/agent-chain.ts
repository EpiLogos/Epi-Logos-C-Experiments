...
export default function (pi: ExtensionAPI) {
	...
	function runAgent(
		...
		// Resolve model: prefer agent's own model field, fall back to parent ctx model.
		let model: string | undefined;
		if (agentDef.model) {
			if (agentDef.model.includes("/")) {
				model = agentDef.model;
			} else {
				const parentProvider = (ctx as any).model?.provider || "anthropic";
				model = `${parentProvider}/${agentDef.model}`;
			}
	...
	// ── Run Chain (sequential in-process pipeline) ──
	...
	async function runChain(
		...
		for (let i = 0; i < activeChain.steps.length; i++) {
			...
			const stepTask = step.prompt
				.replace(/\$INPUT/g, input)
				.replace(/\$ORIGINAL/g, originalPrompt);

			stepStates[i].status = "running";
			stepStates[i].lastWork = stepTask.split("\n")[0] || step.agent;
			updateWidget();

			const result = await runAgent(agentDef, stepTask, i, ctx);
			...
			stepStates[i].elapsed = result.elapsed;
			updateWidget();
