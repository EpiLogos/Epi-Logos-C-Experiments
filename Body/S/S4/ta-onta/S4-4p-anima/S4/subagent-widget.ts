...
export default function (pi: ExtensionAPI) {
	...
	function makeSessionFile(id: number): string {
		...
		const dir = agentDir
			? path.join(agentDir, "sessions", "subagents")
			: path.join(process.env.HOME || process.cwd(), ".pi", "agent", "sessions", "subagents");
	...
	function spawnAgent(
		...
		return new Promise<void>((resolve) => {
			const args = state.sessionFile
				? ["--json", "agent", "subagent", "continue", "--session-key", state.sessionFile, "--task", prompt]
				: ["--json", "agent", "subagent", "run", "--agent", "vak", "--parent-session", "agent:main:main", "--task", prompt];
			...
			proc.on("close", (code) => {
				...
				state.elapsed = payload?.elapsedMs ?? (Date.now() - startTime);
				state.status = payload?.status === "completed" || code === 0 ? "done" : "error";
				if (payload?.sessionKey) {
					state.sessionFile = payload.sessionKey;
				}
				state.proc = undefined;
				state.textChunks = [payload?.output || stdout || stderr];
				updateWidgets();
	...
	pi.registerTool({
		...
		execute: async (callId, args, _signal, _onUpdate, ctx) => {
			...
			if (state.sessionFile) {
				spawnSync("epi", ["--json", "agent", "subagent", "stop", "--session-key", state.sessionFile], {
					encoding: "utf8",
					cwd: process.env.EPI_REPO_ROOT || ctx.cwd || process.cwd(),
				});
			}
	...
	pi.registerTool({
		...
		execute: async (_callId, _args, _signal, _onUpdate, ctx) => {
			const result = spawnSync("epi", ["--json", "agent", "subagent", "list", "--parent-session", "agent:main:main"], {
				encoding: "utf8",
				cwd: process.env.EPI_REPO_ROOT || ctx.cwd || process.cwd(),
			});
			if (result.status !== 0) {
