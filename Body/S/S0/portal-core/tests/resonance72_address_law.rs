...
fn lens_anchor_index_is_tick12_times_6_plus_position_never_lens_times_7_plus_mode() {
    ...
    for cycle in 0u64..2 {
        for sub_tick in 0u8..12 {
            ...
            assert!(profile.resonance72.lens_anchor_index < 72, "72-space escape");
...
fn legacy_and_lens_anchor_encodings_agree_on_the_same_cell() {
    for cycle in 0u64..2 {
        for sub_tick in 0u8..12 {
            ...
            assert!(r.legacy_resonance_index < 72, "legacy index escapes 72-space");
            let helix_bit = usize::from(profile.tick12) / 6;
            let base_lens = usize::from(profile.tick12) % 6;
            let recomposed_anchor = (base_lens + 6 * helix_bit) * 6 + usize::from(profile.position6);
            assert_eq!(
