...
fn profile_carries_m1_topology_invariants() {
    ...
    assert!((mag2 - 1.0).abs() < 1e-4, "ring quaternion off S3: |q|^2={mag2}");
    // element_count is the per-ring-position topological count, never a magic const.
    assert!(topo.element_count >= 1, "element_count derives from the ring LUT");
}
...
fn profile_m1_topology_klein_flip_descriptors_track_the_event() {
    ...
    assert!(crossing.klein_flip.is_some(), "tick 6 fires a klein_flip event");
    assert!(
    ...
    assert!(
        crossing.m1_topology.m1_origin_klein_flip.contains("present"),
...
fn profile_serializes_m1_topology_under_the_carrier_key() {
    ...
    assert!(topo["hopfIdentity"].is_string(), "carrier reads hopfIdentity");
    assert!(
