...
- `src/kernel/profile.rs` — generic `from_tick` profiles now carry the compiled [[M0]] verifier's neutral bootstrap report through `anuttaraWitness`: zero witnessed virtues and symbolic open questions until a session supplies actual evidence. `portal-core` enables the existing `epi-lib` `m0_verifier` feature rather than reimplementing verifier law; `tests/kernel_clock_projection.rs` proves the gateway profile edge.
