...
pub(super) fn inspect(epi: &EpiLib, coord: &str, json: bool) -> color_eyre::Result<()> {
...
pub(super) fn verify(epi: &EpiLib, json: bool) -> color_eyre::Result<()> {
...
pub(super) fn dump(epi: &EpiLib, json: bool) -> color_eyre::Result<()> {
...
pub(super) fn cf_roots(epi: &EpiLib, json: bool) -> color_eyre::Result<()> {
    let roots = [
        (
            "CF(0000)",
            "Receptive Dynamism",
...
pub(super) fn operators(json: bool) -> color_eyre::Result<()> {
    #[derive(serde::Serialize)]
    struct Op {
        symbol: &'static str,
        name: &'static str,
...
pub(super) fn hash(epi: &EpiLib, coord: &str, json: bool) -> color_eyre::Result<()> {
    let (ptr, name) = resolve_coord(epi, coord)
        .ok_or_else(|| color_eyre::eyre::eyre!("Unknown coordinate: {}", coord))?;
