        ...
        D: Deserializer<'de>,
    {
        ...
        if hex.len() != 64 {
            return Err(serde::de::Error::custom(
                "quintessence_hash must be 64 hex chars",
            ));
        }
        let mut out = [0u8; 32];
...
pub fn set_layer(key: &str, value: &str) -> Result<String, String> {
    ...
    match key {
        ...
        "jungian" => {
            if value.len() != 4 {
                return Err("jungian value must be 4-letter MBTI type (e.g. INFJ)".to_string());
            }
            profile.layers.insert(
                "2".to_string(),
                LayerMeta {
    ...
    for (idx_str, meta) in profile.layers.iter_mut() {
        if meta.elemental_profile.is_none() && meta.present {
            meta.elemental_profile =
                Some(elemental_profile_for_layer(idx_str.as_str(), &meta.source));
        }
    }
