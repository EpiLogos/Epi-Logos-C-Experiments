...
pub enum PratibimbaCmd {
    /// Initialise PersonalNexus anchor at #4.4.4.4 in Neo4j (requires epi nara wind first)
    Init {
        #[arg(long)]
        json: bool,
    },
...
pub fn dispatch(cmd: &NaraCmd, json: bool) -> Result<String, String> {
    match cmd {
        ...
        NaraCmd::Pratibimba { cmd: sub } => match sub {
            PratibimbaCmd::Init { json: j } => pratibimba::init(*j || json),
