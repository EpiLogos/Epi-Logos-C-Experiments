...
pub fn evaluate_vak(task: &str) -> VakCoordinates {
    ...
    // Priority order: more specific patterns first, Anima is the active-execution default
    let cf = if task_lower.contains("overview")
        || task_lower.contains("architecture")
        || task_lower.contains("design")
    {
        "(4.5/0)" // Psyche — session subject / contemplative synthesis
        ...
        || task_lower.contains("summarize")
    {
        ...
        "(4.0/1-4.4/5)" // Default to Anima — active execution / Ralph mode
    };
