...
fn surface_color(
    ...
    let dim = if resolution_level >= 1 && tick >= 6 {
        0.45
    } else {
        1.0
    };
...
fn burn_char(
    ...
    let glyph = glyph_id
        .with_scale_and_position(scaled_font.scale(), ab_glyph::point(cx as f32, cy as f32));
...
fn burn_labels(
    ...
    let label_scale = ab_glyph::PxScale {
        x: glyph_size,
        y: glyph_size,
    };
    ...
    let planet_scale = ab_glyph::PxScale {
        x: planet_glyph_size,
        y: planet_glyph_size,
    };
...
mod tests {
    ...
    fn render_clock_produces_non_empty_pixmap() {
        ...
        let non_bg = data
            .chunks(4)
            .any(|px| !(px[0] == 10 && px[1] == 10 && px[2] == 15));
        assert!(
            non_bg,
            "All pixels are background color — torus not rendered"
        );
    }
