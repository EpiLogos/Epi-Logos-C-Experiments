...
static SIGN_ELEMENT: [u8; 12] = [2, 4, 1, 3, 2, 4, 1, 3, 2, 4, 1, 3];
...
impl M2VibrationalPlugin {
    ...
    fn render_matrix(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        ...
        let inner = Layout::horizontal([Constraint::Percentage(70), Constraint::Percentage(30)])
            .split(outer[1]);
    ...
    fn render_grid(&self, area: Rect, buf: &mut Buffer) {
        ...
        for tick12 in 0..12usize {
            ...
            let row_prefix = format!("{}{:>2} {} {}", current_marker, tick12, glyph, strand_label);
    ...
    fn render(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        match self.view_mode {
            ViewMode::Detail => {
                ...
                let elem_idx = cell.element as usize;
                let content = vec![
                    Line::from(format!("half-decan {}", cell.half_decan_idx))
    ...
    fn build_cells_produces_72_valid_cells() {
        ...
        for row in &cells {
            for cell in row {
                assert!(
                    cell.decan_idx < 36,
                    "decan_idx out of range: {}",
                    cell.decan_idx
                );
                assert!(
                    cell.ruling_planet < 10,
                    "ruling_planet out of range: {}",
                    cell.ruling_planet
                );
                assert!(
                    cell.zodiac_sign < 12,
                    "zodiac_sign out of range: {}",
                    cell.zodiac_sign
                );
        ...
        for tick in 0..12usize {
            for ql in 0..6usize {
                assert_eq!(
                    ...
                    (tick * 6 + ql) as u8,
                    "half_decan_idx mismatch at tick={} ql={}",
                    tick,
                    ql
                );
            }
        ...
        for tick in 0..12 {
            for ql in 0..6 {
                ...
                assert!(
                    ...
                    "body_zone empty at tick={} ql={} decan={}",
                    tick,
                    ql,
                    cell.decan_idx
                );
