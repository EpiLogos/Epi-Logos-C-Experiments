...
use crate::portal::clock_state::{update_kairos, PortalClockState, SharedClockState};
...
impl BrailleCanvas {
    ...
    pub fn dot(&mut self, px: i32, py: i32, _color: Color) {
        if px < 0 || py < 0 || px >= (self.width * 2) as i32 || py >= (self.height * 4) as i32 {
            return;
        }
        let px = px as usize;
        let py = py as usize;
...
pub fn render_planet_symbols(buf: &mut Buffer, planet_degrees: &[u16; 10], area: Rect) {
...
fn render_side_panel(buf: &mut Buffer, area: Rect, state: &PortalClockState) {
    ...
    for (i, name) in STAGE_NAMES.iter().enumerate() {
        ...
        let style = Style::default().fg(color).add_modifier(if is_active {
            Modifier::BOLD
        } else {
            Modifier::empty()
        });
...
mod tests {
    ...
    fn cosmic_clock_renders_without_panic() {
        ...
        assert!(
            has_content,
            "CosmicClockPlugin should write something to the buffer"
        );
    }
}
