...
fn render_loop(
    ...
    loop {
        ...
        if let Some(pixmap) =
            crate::portal::clock_renderer::render_clock(w, h, &state, final_view, current_zoom)
        {
...
impl HypertilePlugin for UnifiedClockPlugin {
    fn render(&self, area: Rect, buf: &mut Buffer, _is_focused: bool) {
        ...
        self.render_size
            .cols
            .store(torus_area.width as u32, Ordering::Relaxed);
        self.render_size
            .rows
            .store(torus_area.height as u32, Ordering::Relaxed);
