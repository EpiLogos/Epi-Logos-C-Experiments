...
impl HypertilePlugin for M3KnowingPlugin {
    fn render(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        ...
        let left = Layout::vertical([Constraint::Percentage(35), Constraint::Percentage(65)])
            .split(cols[0]);
                            ...
                            match self.focus {
                                ...
                                PanelFocus::Vimarsa => {
                                    if let Some(d) = &self.dossier {
                                        if self.vimarsa_selected + 1 < d.vimarsa_field.items.len() {
                                            self.vimarsa_selected += 1;
                                        }
                                    }
                                }
...
impl M3KnowingPlugin {
    fn render_essence(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        ...
        let content = if let Some(ref e) = self.error {
            ...
            let phase_text = d.essence.phase.as_deref().unwrap_or("").to_string();
            let mut lines = vec![
                Line::from(vec![
                    Span::styled(
                        d.title.clone(),
            ...
            for word_line in textwrap_simple(&d.essence.text, area.width.saturating_sub(4) as usize)
            {
            ...
            if !pulse_text.is_empty() {
                ...
                lines.push(
                    Line::from(pulse_text.to_string()).style(Style::default().fg(Color::DarkGray)),
                );
            }
            lines
        } else {
            vec![Line::from("Press [i] to enter a coordinate")
    ...
    fn render_structural(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        ...
        for (i, (&fam, (&domain, &color))) in FAMILY_NAMES
            ...
            .enumerate()
        {
            let cursor = if i == self.struct_selected && self.focus == PanelFocus::Structural {
                "►"
            } else {
                " "
            };
    ...
    fn render_qv_facets(&self, area: Rect, buf: &mut Buffer, _is_focused: bool) {
        ...
        if let Some(ref d) = self.dossier {
            ...
            let essence_val: Option<String> =
                overlay_entry.as_ref().and_then(|oe| oe.essence.clone());
    ...
    fn render_vimarsa(&self, area: Rect, buf: &mut Buffer, is_focused: bool) {
        ...
        let content = if let Some(ref d) = self.dossier {
            d.vimarsa_field
                ...
                .enumerate()
                .map(|(i, item)| {
                    let cursor = if i == self.vimarsa_selected && self.focus == PanelFocus::Vimarsa
                    {
                        "►"
                    } else {
                        " "
