//! `epi graph constraint` — YAML-backed constraint registry, plus
//! runtime invocation against a trajectory deposit.
...
use crate::graph::cypher::{apply_params, check_statement, CypherMode};
...
pub fn register(
    ...
    if !matches!(
        outcome,
        crate::graph::cypher::CypherGuardOutcome::AllowedRead
    ) {
        return Err(format!(
