//! Constrained Cypher passthrough for `epi graph cypher`.
//!
//! Implements the spec §3 envelope: read queries are unrestricted,
//! write queries (CREATE/MERGE/SET/DELETE/REMOVE/DROP/CREATE INDEX
//! etc.) require an explicit `--write` flag, schema-altering DDL
//! requires `--admin`, and result-row count is budgeted to prevent
//! accidental graph-wide scans.
...
mod tests {
    ...
    fn comments_do_not_smuggle_writes() {
        // Even if a CREATE keyword sits inside a comment, the parser
        // must not see it. After stripping, only MATCH remains.
    ...
    fn keyword_match_respects_identifier_boundaries() {
        // "CREATED" should not match "CREATE" because of boundary check.
    ...
    fn empty_query_passes_guard_but_run_rejects_it() {
        // Guard treats empty strings as allowed (nothing to deny);
        // `run` performs the empty-string rejection at call time.
