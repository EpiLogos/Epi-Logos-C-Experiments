...
- `src/` command families — `gate/` (gateway RPC/control), `nara/` (M4 personal), `portal/` + `tui/` (ratatui-hypertile), `vault/`, `agent/` + `techne/` (agent/gateway lifecycle), `graph/`, `know.rs` (`epi know` unified coordinate-knowing packet), `slot.rs` (Pi-Agent model-slot + harness-slot config), `sync/`, `ffi/`, `core/`, `notebook/`, `profile/`, `book/`, `code/`, `sesh/`, `up.rs`, `vimarsa/`; `src/main.rs` also hosts the thin `pi train-ebm` / `pi export-ebm-state` S0 membrane over S5 resonance-corpus law.
