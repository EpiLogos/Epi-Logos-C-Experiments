...
impl TestEnv {
    ...
    pub fn repo_with_pi() -> Self {
        let env = Self::empty();
        fs::create_dir_all(env.repo_root.join("Body/S/S4/pi-agent/extensions")).unwrap();
        fs::create_dir_all(env.repo_root.join("Body/S/S4/pi-agent/prompts")).unwrap();
        fs::write(
            env.repo_root.join("Body/S/S4/pi-agent/README.md"),
            "# PI Agent Foundation\n",
        )
        .unwrap();
        fs::write(
        ...
        for (name, content) in curated_extension_files() {
            fs::write(
                env.repo_root
                    .join(format!("Body/S/S4/pi-agent/extensions/{name}")),
                content,
            )
            .unwrap();
        }
        fs::write(
            env.repo_root
                .join("Body/S/S4/pi-agent/prompts/epi-system.md"),
            "# Epi System\n",
        )
        .unwrap();
        fs::write(
            env.repo_root
                .join("Body/S/S4/pi-agent/prompts/epi-agent-help.md"),
            "# Epi Help\n",
        )
        .unwrap();
        write_file(
            env.repo_root
                .join("Body/S/S4/ta-onta/S4-4p-anima/S4'/skills/brainstorming/SKILL.md"),
            "---\nname: brainstorming\ndescription: Brainstorm implementation directions\n---\n\n# Brainstorming\n",
        );
        write_file(
            env.repo_root
                .join("Body/S/S4/ta-onta/S4-4p-anima/S4'/agents/nous.md"),
            "---\nname: nous\ndescription: Fresh perspective agent\ntools:\n  - Read\nskills:\n  - brainstorming\n---\n\n# Nous\n",
        );
        write_file(
            env.repo_root
                .join("Body/S/S4/ta-onta/S4-5p-aletheia/S5'/skills/repl/SKILL.md"),
            "---\nname: repl\ndescription: Structural exploration skill\n---\n\n# REPL\n",
        );
        write_file(
            env.repo_root
                .join("Body/S/S4/ta-onta/S4-5p-aletheia/S5'/agents/anansi.md"),
            "---\nname: anansi\ndescription: Structural topology agent\ntools:\n  - Read\nskills:\n  - repl\n---\n\n# Anansi\n",
        );
        write_file(
            env.repo_root
                .join("Body/S/S4/ta-onta/S4-2p-pleroma/S2'/skills/pleroma-skill-proxy/SKILL.md"),
    ...
    pub fn with_fake_pi() -> Self {
        ...
        env.extra_env
            .push(("EPI_AGENT_GATEWAY_PREFLIGHT".to_owned(), "skip".to_owned()));
        env
    }
    ...
    pub fn apply_to_process(&self) -> ProcessEnvGuard {
        ...
        unsafe {
            ...
            env::remove_var("EPI_AGENT_GATEWAY_PREFLIGHT");
...
pub fn run_epi(args: &[&str], env: &TestEnv) -> TestOutput {
    ...
    command.env_remove("EPI_AGENT_GATEWAY_PREFLIGHT");
