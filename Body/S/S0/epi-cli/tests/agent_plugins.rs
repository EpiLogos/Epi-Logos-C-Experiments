...
#[test]
fn plugin_discovery_lists_s5_epi_logos_resource_spine() {
    let env = TestEnv::repo_with_assets();
    create_modern_plugin_bundle(env.repo_root.join("Body/S/S5/plugins"), "epi-logos");

    let out = run_epi(["agent", "plugins", "list", "--json"].as_slice(), &env);

    assert!(
        out.stdout.contains("\"name\": \"epi-logos\""),
        "stdout: {}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Body/S/S5/plugins/epi-logos"),
        "stdout: {}",
        out.stdout
    );
}
