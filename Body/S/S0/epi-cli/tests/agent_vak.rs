...
#[test]
fn vak_active_tasks_route_to_anima() {
    let result = evaluate_vak("Implement the team dispatch handler");
    assert_eq!(
        result.cf.as_deref(),
        Some("(4.0/1-4.4/5)"),
        "active implementation tasks should route to anima"
    );
}

#[test]
fn vak_default_routes_to_anima_not_psyche() {
    // Tasks that don't match any specific keyword should fall through to Anima (active execution)
    // NOT Psyche — Psyche is only for contemplative/session-synthesis tasks
    let result = evaluate_vak("Process the incoming payload");
    assert_eq!(
        result.cf.as_deref(),
        Some("(4.0/1-4.4/5)"),
        "unclassified tasks should default to anima not psyche"
    );
}
