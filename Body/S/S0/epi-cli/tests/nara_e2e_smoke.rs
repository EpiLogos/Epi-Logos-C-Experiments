...
fn clock_degree_from_oracle_not_quaternion_atan2() {
    ...
    assert!(
        exact_720 < 360.0,
        "phase=0 means Strand A, degree must be < 360"
    );
}
...
fn tick12_quantizes_correctly() {
    ...
    let test_cases: &[(u16, u8)] = &[(0, 0), (29, 0), (30, 1), (59, 1), (330, 11), (359, 11)];
    for &(degree, expected_tick) in test_cases {
        let tick12 = (degree / 30) as u8;
        assert_eq!(
            tick12, expected_tick,
            "degree={} should give tick12={}",
            degree, expected_tick
        );
    }
}
