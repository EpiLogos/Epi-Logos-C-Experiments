...
- `include/m0.h` + `src/m0.c` — `ARCHETYPE_COORDINATE_LUT[12]` binds every compiled [[M0]] archetype slot to its canonical live-graph identity; [[S2]] consumes this authority for the [[M0']] residual browser instead of maintaining a Rust or renderer copy.
- `src/lib.rs` — Rust FFI surface and tests (`m0_verifier` feature exposes `bootstrap_witness_for_tick`, `contemplation_prompt_lut`, and additive `evaluate_state` / `is_language_member` adapters for the live [[Anuttara]] gateway; each executes the compiled C verifier/LUT/128-entry registry rather than recreating its law in Rust. `m0_calc_*` corpus tests, `m0_archetype_routing`, `m0_rfactor_band_turn`, `m2_asma_mirror_idx_round_trip`, and the T1.11 symbolic-coordinate-string round-trip run in the default build — `m0_calculus.c` + `anuttara_language.c` are UNCONDITIONAL kernel substrate per `build.rs` since Tranche 01.T1.13; the `m0_calculus` cargo feature is a no-op check surface)
- `src/lib.rs::m0_verifier::archetype_coordinate_lut` — zero-policy FFI projection of that compiled twelve-slot coordinate authority, with a real C-backed ordering test.
- `tests/` — Rust integration tests (`clock_backbone_node_test.rs`); `test/` — C test trees (m0–m5, vak, engine, pillar1, fixtures)
- `Makefile` (delegates to repo root), `build.rs`, `docs/` (m0–m5 references), `scripts/` (LUT/fixture generators); `src/m0_anuttara_lint.c` plus its real C test scan hardcoded-shaped numeric relations, `tunable-audit-report.md` is generated scanner output, and `tunable-audit-triage.md` records bounded manual adjudication without rewriting generated evidence
...
- Contract-surface flag: [[S0-ARCHITECTURE]] should absorb the `M0_TuneProposal` tuning-dispatch metadata, `m0_check_slot_privacy_boundary_compliance` public seam, additive compiled-verifier adapters `evaluate_state` / `is_language_member` / `contemplation_prompt_lut` / `archetype_coordinate_lut`, and the `is_evolutionary_gap` → `epogdoon_has_round_trip_loss` ABI rename during the next canon harmonisation pass. [[M2'-SPEC]] / [[M3'-SPEC]] already carry the semantic amendment.
