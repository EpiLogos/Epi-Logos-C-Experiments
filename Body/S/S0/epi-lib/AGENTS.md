...
- Contract-surface flag: [[S0-ARCHITECTURE]] should absorb the `M0_TuneProposal` tuning-dispatch metadata, `m0_check_slot_privacy_boundary_compliance` public seam, and additive compiled-verifier adapters `evaluate_state` / `is_language_member` during the next canon harmonisation pass.
