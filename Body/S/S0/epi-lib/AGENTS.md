...
## Verification
- C: `make test`. Focused M4 session lifecycle check: `make -C Body/S/S0/epi-lib test_m4` plus `cargo test --manifest-path Body/S/S0/epi-lib/Cargo.toml m4_session`. Focused tuning verifier check: `make -C Body/S/S0/epi-lib test_m0_tune_invariant_constraint`. Rust FFI bridge: `make rust-test`. Per-crate: `cargo test -p epi-lib`.
