...
- **Reserved seams.** `DEEP_PANE_RESERVATIONS` names the deep slot each designated `pending` surface lands in, so its owning tranche mounts into an already-decided position: `agenticControlRoom` → `personal-deep-main` (28.5), `coordinateTree` and `backendStudio` → the deep left rail (28.6 / 28.13), each marked `· **deep:** reserved` on its own row. Reserving is not mounting: all three must stay out of the factory and out of the deep models until their tranche flips the row. `readiness-gate` (28.11) carries no marker on purpose — it is a per-binding inline wrapper, not a slot occupant, so it has no pane-set position to reserve.
...
| `agenticControlRoom` | main tabset (designated, deep render) · **deep:** reserved — 28.T28.5 → `personal-deep-main` | M5' chrome | pending | 28.5 owns; GOVERNANCE PRIMARY deep render (§5) | `agentic-control-room` |
