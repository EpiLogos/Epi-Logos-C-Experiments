 ...
 *   profile generation carrying `kleinFlipAtThisTick = true` deposits a glyph on
 *   the event log, anchored to its `tick12`; the canonical tritone crossing
 *   (5→6, observed at tick 6) is styled apart from other transitions; a flip at a
 *   non-tritone tick renders an `unexpected-flip` badge (M1'-SPEC §6). A small
...
export function M1KleinFlipEventStrip() {
    ...
    const flip = face.vortex?.kleinFlipAtThisTick ?? null;
    ...
    useEffect(() => {
        ...
        setLog(prev =>
            [...prev, { generation, tick12, canonical: isCanonicalFlipTick(tick12) }].slice(-LOG_CAP)
        );
    }, [generation, flip, tick12]);
    ...
    return (
        <section className="mext-widget-detail" data-testid="m1-klein-flip-strip">
            ...
            <div className="pane-toolbar" data-testid="m1-klein-flip-toolbar">
                ...
                <span data-testid="m1-klein-flip-this-tick" data-flip={flip ? 'true' : 'false'}>
                    this-tick flip: {flip ? 'yes' : 'no'}
                </span>
                <span
