/**
 * Coordinate: M' (block contract — Tranche 44.T44.1 / DR-PSS-1 / DR-PSS-3)
 * Residency: Body/M/pratibimba-app/src/blocks
 * Actualises: the binding Track-44 surface standard — Blocks are the
 *   CTX-framed wire units M' surfaces render. Mandatory ctx {cf, ct, cp}
 *   (DR-PSS-1), the closed 19-type core vocabulary, privacy classes,
 *   affordances, BlockSpec, and the catalog law: consumers reject any type
 *   absent from `blocks.catalog` or whose IOD-17 faces disagree (DR-PSS-3).
 *   `block-contract.schema.json` is the mirrored JSON schema so S-layer Rust
 *   cores emit conforming blocks.
 * Provenance: ported 2026-07-14 from the frozen
 *   Body/M/epi-theia/extensions/m-extension-runtime/src/common/block-contract.ts
 *   (already pure TS; re-tested here — cribbed code counts as new code).
 * Does NOT own: the BlockRegistry/BlockHost renderer (44.2), projections
 *   (44.3), the verdict loop (44.4), the live `blocks.catalog` gateway method
 *   (Techne-operated, not yet on the wire — CORE_BLOCKS_CATALOG is the static
 *   registration union until it lands; the method constant is the seam).
 */
