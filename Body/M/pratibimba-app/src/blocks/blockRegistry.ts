/**
 * Coordinate: M' (block registry — Tranche 44.T44.2 / Track 44 no-orphan law)
 * Residency: Body/M/pratibimba-app/src/blocks
 * Actualises: BlockRegistry — `type → BlockSpec`, whose union of
 *   registrations IS the live catalog (44.1 law); core specs for the 19-type
 *   vocabulary; the no-orphan audit (every core type owned + spec'd, Track 14
 *   release-gate feed); the designed ownership map and gateway-method
 *   contract table (BK-GW rows: blocks.catalog/annotate/verdict/doc.*).
 * Provenance: ported 2026-07-14 from the frozen
 *   Body/M/epi-theia/extensions/block-kit/src/common/registry.ts. Owner
 *   registrations' sourceAnchors are the DESIGN-LAW ownership map from the
 *   frozen warehouse — they re-anchor to carrier surfaces as 44.3+ land
 *   (the no-orphan law binds on type-ownership, not on the Theia paths).
 * Does NOT own: the block wire contract (./blockContract.ts), the verdict
 *   loop (44.4), the live blocks.catalog gateway method (Techne-operated,
 *   pending wire seam).
 */
