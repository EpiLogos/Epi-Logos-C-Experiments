...
- `src/browser/` — frontend: `frontend-module.ts`, `empty-state.tsx`, `privacy-chrome.ts`, `m4-nara-widget.tsx`, `canvas-editor.tsx`, `widgets/` (kairos-display, logos-cycle, medicine-view, lens-application, pratibimba-coordinate, personal-coordinate-sidebar, transform-containers, mercurius-relay-indicator, dialogical-arena, day-calendar, ambient-state-strip, tuning-bar, time-axis-switcher, quintessence-display, personal-cymatic-field, psyche-anchor-coherence, being-pattern-perspective, rfactor-fretboard, oracle-cast, oracle-history plus `widgets/oracle/` subcomponents), `onboarding/identity-wizard.tsx`, `editor/`, `services/`.
- `style/` — `index.css`, `highlights.css`, `privacy-chrome.css`, `dialogical-arena.css`.
...
## Verification
`pnpm --dir Body/M/epi-theia/extensions/m4-nara test` (runs `pnpm build` then `node --test` over `tests/privacy-chrome.test.mjs`, `tests/symbolic-protein-reading.test.mjs`, `tests/oracle-cast.test.mjs`, `../test/m4-nara-canvas-editor.test.mjs`, `m4-nara-kairos-display.test.mjs`, `m4-nara-mercurius-relay-indicator.test.mjs`, `m4-nara-e4-personal-energy.test.mjs`, `m4-nara-dialogical-arena.test.mjs`, `m4-nara-quintessence-display.test.mjs`, `m4-nara-personal-cymatic-field.test.mjs`). R-factor fretboard focused check: `node --test Body/M/epi-theia/extensions/test/m4-nara-rfactor-fretboard.test.mjs`. Full suite: `pnpm --dir Body/M/epi-theia test:contracts`.
