...
- `src/browser/` — frontend: `frontend-module.ts`, `empty-state.tsx`, `privacy-chrome.ts`, `m4-nara-widget.tsx`, `canvas-editor.tsx`, `widgets/` (kairos-display, logos-cycle, medicine-view, lens-application, pratibimba-coordinate, transform-containers, mercurius-relay-indicator, dialogical-arena, day-calendar, ambient-state-strip, tuning-bar, time-axis-switcher, quintessence-display, psyche-anchor-coherence, being-pattern-perspective, rfactor-fretboard), `onboarding/identity-wizard.tsx`, `editor/`, `services/`.
