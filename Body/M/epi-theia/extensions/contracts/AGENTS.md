...
- The UI token/colour-token/theme-mapping/motion-token/accessibility/typography/composition/foundation rule docs above (binding `epilogos.*` surface).
...
- Edit `ui-design-tokens.ts` / `ui-typography.ts` / `ui-motion-tokens.ts` / `ui-accessibility.ts` as source surfaces where applicable; keep `.json`/`.md` siblings consistent — extensions consume-not-fork these (no local hex/spacing/duration/motion/a11y redefinition).
