...
## Purpose
Implementation-control contract artifacts for the M' Theia extensions workspace — extension/composition preflight contracts plus the canonical UI design-token, typography, and composition-rule sources. No runtime package; pure contract docs consumed by sibling extensions.
...
- The UI token/typography/composition/foundation rule docs above (binding `epilogos.*` surface).
...
- Edit `ui-design-tokens.ts` / `ui-typography.ts` as the source surface; keep `.json`/`.md` siblings consistent — extensions consume-not-fork these (no local hex/spacing/duration redefinition).
