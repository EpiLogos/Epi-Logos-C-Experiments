...
- Edit `ui-design-tokens.ts` / `ui-typography.ts` / `ui-motion-tokens.ts` as source surfaces where applicable; keep `.json`/`.md` siblings consistent — extensions consume-not-fork these (no local hex/spacing/duration/motion redefinition).
