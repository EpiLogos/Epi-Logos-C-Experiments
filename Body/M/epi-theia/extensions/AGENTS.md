...
- `contracts/ui-design-tokens.ts` + `ui-colour-tokens.ts` + `ui-theme-mapping.ts` + `ui-motion-tokens.ts` + `ui-accessibility.ts` + `ui-typography.ts` + `ui-iconography.ts` — binding UI token/colour-token/theme-mapping/motion-token/accessibility/typography/iconography surface.
