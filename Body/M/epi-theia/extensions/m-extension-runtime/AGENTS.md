...
- `src/common/` — `shared-bridge.ts`, `bridge-api.ts` (KernelBridgeAPI boundary), `block-contract.ts` + `block-contract.schema.json` (CTX-framed Block/BlockSpec, `blocks.catalog`, and IOD-17 parity gating), `coordinate-context.ts`, `readiness.ts` (nine-state taxonomy), `empty-state-registry.ts` (per-extension empty-state registry), `observability.ts`, `profile.ts` (MathemeHarmonicProfile boundary), `preferences-schema.ts`, `recursive-self-review-gate.ts`, `route.ts`, `bridge-readiness.ts`, `contribution-contracts.ts`.
...
## Local Contracts
- Code Coordinate Headers: `src/common/bridge-api.ts`, `block-contract.ts`, `coordinate-context.ts`, `readiness.ts`, `profile.ts`, `observability.ts`, `route.ts` (`//**` doc-comments cite the owning tranche / upstream contract files).
