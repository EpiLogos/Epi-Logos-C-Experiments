...
- `src/common/` — `shared-bridge.ts`, `bridge-api.ts` (KernelBridgeAPI boundary), `coordinate-context.ts`, `readiness.ts` (nine-state taxonomy), `observability.ts`, `profile.ts` (MathemeHarmonicProfile boundary), `recursive-self-review-gate.ts`, `route.ts`, `bridge-readiness.ts`, `contribution-contracts.ts`.
...
- `tests/` — `node --test` `.mjs` suites (bridge-readiness, kairos-enablement-step, reset-section, cold-start-pasu-gate).
