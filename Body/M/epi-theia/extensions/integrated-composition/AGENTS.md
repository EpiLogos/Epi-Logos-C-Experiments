...
- `src/common/profile-tick-subscription.ts` + `src/browser/composition-profile-context.tsx` — composition-scoped profile tick subscription primitive, React provider/hook, and the daily 0/1 toggle chrome/gesture mount shared by both integrated plugins.
