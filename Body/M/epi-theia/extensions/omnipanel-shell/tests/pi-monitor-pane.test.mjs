// Track 15 T15.2 — Pi Runtime Monitor rehomed into OmniPanel.
...
const {
    PiMonitorPane
} = require('../lib/browser/components/omni/panels/PiMonitorPane.js');

const portalTemporalSurface = Object.freeze({
    canonicalSessionKey: 'agent:main:main',
    activeAgentId: 'pi.main',
    dayId: '11-06-2026',
    nowPath: 'Idea/Empty/Present/11-06-2026/20260611-154015-15T1502/now.md',
    nowWikilink: '[[20260611-154015-15T1502/now]]',
    kernelGeneration: 44,
    kernelSubTick: 2,
    terminalBacked: true,
...
const terminalBackedSession = Object.freeze({
    ...
    dayId: '11-06-2026',
    vaultNowPath: 'Idea/Empty/Present/11-06-2026/20260611-154015-15T1502/now.md',
...
test('omnipanel-runtime owns Pi runtime monitor projection without terminal scrollback', () => {
...
test('PiMonitorPane renders inside OmniPanel diagnostics and redacts run handles', () => {
    const html = ReactDOMServer.renderToStaticMarkup(
        React.createElement(PiMonitorPane, {
            portalTemporalSurface,
            resolvedSession: terminalBackedSession
        })
    );

    assert.match(html, /data-test="pi-monitor-pane"/);
    assert.match(html, /data-terminal-backed="true"/);
    assert.match(html, /Pi Runtime Monitor/);
    assert.match(html, /agent:main:main/);
    assert.match(html, /epi agent tmux inspect --session-key agent:main:main/);
    assert.doesNotMatch(html, /super-secret-terminal-handle/);
    assert.doesNotMatch(html, /RAW_SCROLLBACK_SECRET/);
});
