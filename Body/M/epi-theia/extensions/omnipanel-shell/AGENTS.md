...
## Ownership
- `src/common/` — shared runtime contracts: `omnipanel-types.ts` (`OMNIPANEL_WIDGET_ID`, tab ids, `CrossLayoutIntent`), `omnipanel-runtime.ts` (including the [[Pi]] runtime-monitor projection rehomed from [[agentic-control-room]]), `omnipanel-session-state.ts`, `dispatch-genealogy.ts`, `index.ts` barrel; plus `ACR-MIGRATION-MAP.md`.
