...
- `src/browser/` — frontend widgets: `bimba-graph-viewer`, `canon-studio`, `coordinate-tree` (M0' chrome); `logos-atelier`, `evidence-pane`, `review-pane`, `autoresearch-pane`, `agentic-control-room` (M5' chrome); `bridge-gate` (shared readiness); plus `frontend-module.ts` (DI wiring) and first-build stubs `backend-studio/`, `smart-connections/`, `services/privacy-drop-feed.ts`.
