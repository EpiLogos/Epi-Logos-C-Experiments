...
All scripts run from the repo root. They read from `docs/datasets/` and write to stdout.
...
python3 docs/scripts/lookups/surface_qv_fields.py --all

# Anuttara #0-3 branch only
python3 docs/scripts/lookups/surface_qv_fields.py --all --prefix "#0-3"

# Only nodes with completeFormulation
python3 docs/scripts/lookups/surface_qv_fields.py --all --require completeFormulation
```
...
python3 docs/scripts/lookups/surface_formulations.py --all --count

# Full output for anuttara
python3 docs/scripts/lookups/surface_formulations.py docs/datasets/anuttara-deep/nodes-full-data.json
```
...
```bash
# Full table (all 184)
python3 docs/scripts/lookups/surface_generation_events.py

# M1 subsystem only (64 nodes = I-Ching frame)
python3 docs/scripts/lookups/surface_generation_events.py --context M1

# Single hexagram
python3 docs/scripts/lookups/surface_generation_events.py --hex H1

# Summary statistics + binary distribution
python3 docs/scripts/lookups/surface_generation_events.py --summary

# Export as JSON (for oracle.rs cross-reference)
python3 docs/scripts/lookups/surface_generation_events.py --json > /tmp/gen_events.json
```
