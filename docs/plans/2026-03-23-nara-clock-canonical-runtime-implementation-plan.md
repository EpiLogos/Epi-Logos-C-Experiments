...
**Architecture:** Dataset coordinates (`#0`-`#4`) are the canonical address space. C in `epi-lib` owns bare-metal structs, LUTs, and deterministic transforms; Rust in `epi-cli` mirrors C through FFI and owns runtime orchestration, gateway transport, session integration, and portal UX. All state must obey the raw -> derived -> rendered rule and the canonical primitives in `docs/plans/CLOCK-AND-NARA-SPECS/00-canonical-invariants.md`: `quintessence_hash[32]`, `quintessence_quaternion`, `live_quaternion`, `tick12`, `PlanetState[10] + EarthBodyState`, and `nara.oracle.payload`.
...
2. `docs/plans/CLOCK-AND-NARA-SPECS/00-canonical-invariants.md`
3. `docs/plans/CLOCK-AND-NARA-SPECS/00-spec-harmonization-plan.md`
4. The harmonized clock/Nara specs:
   - `docs/plans/CLOCK-AND-NARA-SPECS/01-quintessence-hash-architecture.md`
   - `docs/plans/CLOCK-AND-NARA-SPECS/02-16-lenses-backbone-temporal.md`
   - `docs/plans/CLOCK-AND-NARA-SPECS/03-spanda-double-helix-12fold.md`
   - `docs/plans/CLOCK-AND-NARA-SPECS/04-shadow-dynamics-three-computations.md`
   - `docs/plans/CLOCK-AND-NARA-SPECS/09-cosmic-clock-plugin-tui-spec.md`
   ...
   - `docs/specs/M/M4-nara-personal-interface.md`
   - `docs/specs/M/M4-nara-subtle-body-map.md`
   - `docs/specs/M/HMS-quaternionic-overlay.md`
...
- `docs/plans/CLOCK-AND-NARA-SPECS/dataset-bridge/00-master-coordination.md`
- `docs/plans/CLOCK-AND-NARA-SPECS/dataset-bridge/2026-03-19-codex-binary-findings-first-pass.md`
...
- Update `docs/plans/2026-03-10-nara-runtime-full-plan.md` implementation status to reflect reality after landing.
