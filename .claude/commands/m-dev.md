...
**The gate lane is machine-wide — never run the real-port suites unlaned.** `verify-tranche`, `verify-all`, `pnpm test:e2e` and `pnpm smoke` all acquire `.codex/verify.lock` (`.codex/scripts/gate-lane.mjs`) before binding the gateway/sidecar/vite ports, and it is RE-ENTRANT: a suite spawned beneath a run that already holds the lane passes straight through, so the gate never deadlocks against its own child. Two agents doing UF work concurrently used to destroy each other's evidence — one run's global-teardown killed the other's gateway and deleted its `test-results/e2e-run.json` mid-suite. Measured 2026-07-28: the same carrier suite reported 94/9, then 62 failed, then 43 passed, then **108 passed / 0 failed** once serialised; the first three were reading a demolished environment, not the code. So a UF red that looks like mass failure (connection-refused, ~100ms failures, a missing run-state file) is a COLLISION until proven otherwise — re-run laned before attributing it to anyone's code. `pnpm test:e2e:unlaned` exists for debugging a single spec and must never be the basis of a receipt.
