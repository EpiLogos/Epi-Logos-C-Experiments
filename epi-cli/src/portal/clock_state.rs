...
impl CodonClass {
    ...
    pub fn is_non_dual(self) -> bool { !self.is_dual() }
    pub fn is_palindromic(self) -> bool {
        matches!(self, CodonClass::PerfectPalindromic | CodonClass::ImperfectPalindromic)
    }
...
impl WalkType {
    ...
    pub fn all() -> &'static [WalkType; WALK_TYPE_COUNT] {
        &[
            WalkType::Degree, WalkType::Amino, WalkType::Zodiac,
            WalkType::Spanda, WalkType::Decan, WalkType::Hexagram,
            WalkType::Enneadic, WalkType::Seasonal, WalkType::LineChange,
        ]
    }
}
...
pub fn derive_walk_mode(q: [f32; 4]) -> WalkMode {
    ...
    for i in 1..4 {
        if abs[i] > abs[max_idx] { max_idx = i; }
    }
    match max_idx {
        0 => WalkMode::Ground,
        1 => WalkMode::Torus,
...
pub fn compute_aspects(state: &SharedClockState) {
    ...
    for a in 0..10u8 {
        ...
        if deg_a == 0xFFFF { continue; }
        for b in (a + 1)..10u8 {
            let deg_b = s.kairos.planets[b as usize].degree;
            if deg_b == 0xFFFF { continue; }
...
pub fn save_micro_orbit(orbit: &[u16]) {
    ...
    let json = format!(
        "[{}]",
        orbit.iter().map(|d| d.to_string()).collect::<Vec<_>>().join(",")
    );
...
mod tests {
    ...
    fn quat_mul_identity() {
        ...
        for i in 0..4 {
            assert!((result[i] - q[i]).abs() < 1e-6, "identity mul failed at index {i}");
        }
        let result2 = quat_mul(q, id);
        for i in 0..4 {
            assert!((result2[i] - q[i]).abs() < 1e-6, "mul identity (right) failed at index {i}");
        }
    }
    ...
    fn compute_aspects_finds_conjunction() {
        ...
        compute_aspects(&state);
        let s = state.lock().unwrap();
        assert!(!s.aspects.is_empty(), "should find conjunction between Sun and Moon");
    ...
    fn composed_quaternion_stored_after_cast() {
        ...
        let mag = (s.composed_quaternion[0].powi(2)
            ...
            + s.composed_quaternion[3].powi(2)).sqrt();
        assert!((mag - 1.0).abs() < 1e-4, "composed quaternion should be unit length");
    }
    ...
    fn micro_orbit_roundtrip() {
        ...
        assert_eq!(loaded, test_data, "micro-orbit should survive save/load roundtrip");
        // Clean up
    ...
    fn walk_type_labels_nonempty() {
        for wt in WalkType::all() {
            assert!(!wt.label().is_empty(), "walk type {:?} should have a label", wt);
        }
    }
