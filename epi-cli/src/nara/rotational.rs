...
pub fn generate_rotational_states(codon6bit: u8) -> Vec<RotationalState> {
    ...
    order.sort_by(|&a, &b| {
        ...
        va.cmp(&vb).then_with(|| {
            (states[a].polarity as u8).cmp(&(states[b].polarity as u8))
        })
    });
...
mod tests {
    ...
    fn aaa_has_non_dual_states() {
        ...
        assert!(non_dual_count > 0, "AAA (perfect palindromic) should have at least one non-dual state");
    }
    ...
    fn negative_states_have_negative_polarity() {
        for codon in 0u8..64 {
            ...
            let neg_count = states.iter().filter(|s| s.polarity == Polarity::Negative).count();
            let pos_count = states.iter().filter(|s| s.polarity == Polarity::Positive).count();
            assert_eq!(neg_count, 4, "Codon {} should have 4 negative states", codon);
            assert_eq!(pos_count, 4, "Codon {} should have 4 positive states", codon);
        }
    }
    ...
    fn rotation_slots_are_unique_0_to_7() {
        for codon in 0u8..64 {
            ...
            assert_eq!(slots, vec![0, 1, 2, 3, 4, 5, 6, 7],
                "Codon {} should have rotation_slots 0-7", codon);
        }
    }
    ...
    fn rotation_degrees_match_slots() {
        for codon in 0u8..64 {
            ...
            for s in &states {
                assert_eq!(
                    s.rotation_degrees, (s.rotation_slot as u16) * 45,
                    "Codon {} slot {} should have degrees {}",
                    codon, s.rotation_slot, s.rotation_slot as u16 * 45
                );
            }
        }
    }
