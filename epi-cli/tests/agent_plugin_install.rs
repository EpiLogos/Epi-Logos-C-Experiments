...
/// Task 6: plugins/pleroma must be discoverable via epi agent plugins list.
#[test]
...
/// Task 6: plugins/pleroma must pass validation.
#[test]
fn pleroma_plugin_validates() {
    let env = TestEnv::repo_with_assets();
    seed_pleroma_plugin(&env);

    let pleroma_path = env.repo_root.join("plugins/pleroma");
    let out = run_epi(
        [
            "agent",
            "plugin",
...
fn seed_pleroma_plugin(env: &TestEnv) {
    let root = env.repo_root.join("plugins/pleroma");
    ...
    for dir in [&manifest_dir, &skills_dir, &agents_dir, &hooks_dir, &evals_dir] {
    ...
    // Also write the registry entry
    let registry = env.repo_root.join("plugins/registry.jsonl");
    let mut content = fs::read_to_string(&registry).unwrap_or_default();
    if !content.contains("pleroma") {
        content.push_str(
            &format!(
                r#"{{"name":"pleroma","version":"0.1.0","source":"local","root":"plugins/pleroma"}}"#
            )
        );
        content.push('\n');
