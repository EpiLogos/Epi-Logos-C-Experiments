...
fn pleroma_root() -> PathBuf {
    repo_root().join("plugins/pleroma")
}

/// Task 5: plugins/pleroma must have a valid plugin manifest.
#[test]
fn pleroma_has_plugin_manifest() {
    ...
    assert!(
        v.get("name").and_then(|n| n.as_str()).is_some_and(|n| !n.is_empty()),
        "manifest `name` must not be empty"
    );
    assert!(
        v.get("version").and_then(|v| v.as_str()).is_some_and(|v| !v.is_empty()),
        "manifest `version` must not be empty"
    );
}

/// Task 5: plugins/pleroma must have at least one skill directory with a SKILL.md.
#[test]
fn pleroma_has_non_empty_skills() {
    let skills_dir = pleroma_root().join("skills");
    assert!(
        skills_dir.exists(),
        "plugins/pleroma/skills/ does not exist"
    );
    ...
    assert!(
        ...
        "plugins/pleroma/skills/ has no skill subdirectories"
    );

    // At least one skill must have a SKILL.md
    let has_skill_md = skill_entries.iter().any(|e| e.path().join("SKILL.md").exists());
    assert!(has_skill_md, "no SKILL.md found in any plugins/pleroma/skills/ subdirectory");
}

/// Task 5: plugins/pleroma must have at least one agent ANIMA.md.
#[test]
fn pleroma_has_non_empty_agents() {
    let agents_dir = pleroma_root().join("agents");
    assert!(
        agents_dir.exists(),
        "plugins/pleroma/agents/ does not exist"
    );
    ...
    assert!(
        ...
        "plugins/pleroma/agents/ has no agent subdirectories"
    );

    let has_anima_md = agent_entries.iter().any(|e| e.path().join("ANIMA.md").exists());
    assert!(has_anima_md, "no ANIMA.md found in any plugins/pleroma/agents/ subdirectory");
}

/// Task 5: plugins/pleroma must have a hooks config.
#[test]
fn pleroma_has_hooks_config() {
    let hooks_json = pleroma_root().join("hooks/hooks.json");
    assert!(
        hooks_json.exists(),
        "plugins/pleroma/hooks/hooks.json does not exist"
    );
...
/// Task 5: plugins/pleroma must have an evals/suites/ directory.
#[test]
fn pleroma_has_eval_suites() {
    let suites = pleroma_root().join("evals/suites");
    assert!(
        suites.exists(),
        "plugins/pleroma/evals/suites/ does not exist"
    );
}
