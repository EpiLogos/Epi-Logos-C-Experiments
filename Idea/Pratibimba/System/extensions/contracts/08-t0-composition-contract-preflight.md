...
This contract package is the implementation-control artifact for `08.T0` under `Idea/Pratibimba/System/extensions/contracts` and the corresponding shared composition module at `Idea/Pratibimba/System/shared`. It defines what each Track 07 individual extension is expected to contribute when composed into the two integrated plugins, the fixture payload requirements drawn from Tracks 01–04, and the compatibility matrix that names ownership and absent-state readiness.
...
Track 08 **inherits** 07.T0's `KernelBridgeAPI` adapter, forbidden-import set, and readiness taxonomy. It does NOT introduce a parallel bridge, a second forbidden-import list, or a divergent readiness vocabulary. The composition contract shapes (`IntegratedSurfaceContribution`, `IntegratedViewPart`, `IntegratedMiniInspector`, `IntegratedEvidenceProducer`, `IntegratedLayoutClaim`, `IntegratedReadiness`) live at `Idea/Pratibimba/System/shared` and are imported by both integrated plugins and the `ide-shell-m0-m5` chrome extension.
...
- Composition orchestration is owned by `Idea/Pratibimba/System/shared` (favoured over per-plugin or ide-shell ownership) to avoid divergent layout arbitration.
...
The preflight validator at `Idea/Pratibimba/System/extensions/test/validate-composition-contract-preflight.test.mjs` asserts:
...
Run with `node --test Idea/Pratibimba/System/extensions/test/validate-composition-contract-preflight.test.mjs`.
