...
const manifestPath = join(
  repoRoot,
  "Idea/Pratibimba/System/extensions/contracts/07-t0-extension-contract-preflight.json"
);
const captureRequirementsPath = join(
  repoRoot,
  "Idea/Pratibimba/System/extensions/test/fixtures/07-t0-readiness-capture-requirements.json"
);
const validatorPath = join(
  repoRoot,
  "Idea/Pratibimba/System/extensions/scripts/validate-extension-contract-preflight.mjs"
);
...
test("Track 07 T0 contract package defines the full extension inventory", () => {
  ...
  for (const extension of manifest.extensions) {
    assert.equal(
      ...
      `Idea/Pratibimba/System/extensions/${extension.id}`
    );
