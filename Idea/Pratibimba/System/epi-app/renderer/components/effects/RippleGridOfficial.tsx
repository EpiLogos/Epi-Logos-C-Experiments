...
void main() {
    ...
    try {
      ...
      onErrorRef.current?.(error);
      if (canvasAppended && containerRef.current && gl.canvas.parentElement === containerRef.current) {
    ...
    const render = (t: number) => {
      ...
      if (!mesh) {
        onErrorRef.current?.(new Error('RippleGrid mesh unavailable'));
        return;
      }
    ...
    contextLostHandler = (event: Event) => {
      ...
      onErrorRef.current?.(new Error('RippleGrid WebGL context lost'));
    };
