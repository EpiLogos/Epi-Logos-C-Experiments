...
- There are now dedicated `Body/S/S3/redis-context` and `Body/S/S3/gateway` modules for Redis runtime residency and gateway protocol/session/runtime residency. There is not yet a dedicated `Body/S/S3/graphiti-runtime` module, and the live gateway dispatch/server body is still S0-hosted. `Body/S/S3` currently also holds `epi-app` and `epi-spacetime-module`; the next S3 extraction step is the live dispatch/server adapter and Graphiti runtime boundary.
...
| `session_surface` / `kairos_surface` / `global_temporal_surface` / `temporal_event` SpaceTimeDB tables | Live-state projection carrying canonical session, DAY/NOW, history, Redis, Graphiti orientation, safe Kairos transit facts, protected Pratibimba refs, shared portal/agent temporal orientation, and activity facts |
...
| `bind_global_temporal_surface` | Project the safe shared DAY/NOW/Kairos/Redis/Graphiti/Pratibimba-anchor surface consumed by TUI, desktop, gateway, and PI-agent clients |
