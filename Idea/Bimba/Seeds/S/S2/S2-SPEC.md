...
- `Body/S/S2/graph-schema` owns the canonical S2 graph schema constants: `:Bimba`, coordinate property, 3072 embedding dimensions, vector index names, compatibility labels/properties, and coordinate-driven node/relationship property specs. It now also owns the first kernel-resonance observation schema: `:Bimba:KernelResonanceObservation`, `HAS_KERNEL_RESONANCE`, `c_5_kernel_resonance_*`, `c_5_kernel_tick`, `s_3_session_key`, and `s_3_graphiti_arc_id`.
- `Body/S/S2/graph-services` owns the first Body-native S2 service authority: Neo4j primary graph role, live Neo4j config/client, schema creation, coordinate seed entrypoints, coordinate parser, GraphRAG query grammar, disclosure/query/retrieval result contracts, and Redis semantic-cache role/config/payload/client/health contracts. It resolves the RedisVL Python bridge through `Body/S/S3/redis-context`, because the bridge is Redis runtime residency while S2 owns the graph semantic-cache law using it. It now also owns `s2.graph.kernel_resonance.record`, which validates/plans parameterized Neo4j writes for kernel resonance observations while leaving computation in S0 and episodic deposition in S3/S5. `Body/S/S0/epi-cli/src/graph/client.rs`, `schema.rs`, `seed.rs`, `coordinate_array_parser.rs`, `retrieval/graphrag.rs`, `retrieval/hybrid.rs`, and `semantic_cache.rs` now mirror or delegate to this S2 authority where those contracts are involved.
...
- Remaining Neo4j property law work is now about extending the coordinate-driven registry, not inventing the pattern: node and relationship property specs already carry coordinate home, type/cardinality rule, disclosure/privacy class, source family, and index intent for canonical Bimba and kernel-resonance observation properties.
...
- `Body/S/S2/graph-services/src/lib.rs`, `schema.rs`, and `seed.rs` for the Body-native semantic cache, Neo4j client contracts, schema creation, seed entrypoints, retrieval query semantics, tokenization, disclosure law, retrieval result shape, and pure hybrid fusion/ranking law.
