...
| `Body/S/S3/epi-spacetime-module/` | SpacetimeDB presence module | [[S3]] presence implementation | Source home is now Body-native | Must align with gateway/presence tests |
| `Body/S/S3/epi-app/` | Electron app, S3 gateway client, M-domain UI, capability envelopes | [[S3.5]] / [[S4]] app bridge and inhabitation surface | Source home is now Body-native unless later split into S3 app bridge plus S4 UI shell | Must implement canonical envelope subset |
...
| [[S3]] / [[S3']] | `Body/S/S0/epi-cli/src/gate`, `.epi/gate`, `Body/S/S3/epi-spacetime-module`, `Body/S/S3/epi-app/main/s3-gateway-client.ts` | Gateway state is real but physically inside the S0 CLI package; Graphiti sidecar management and temporal library target conflict | Keep current wrapper as compatibility only; define `Body/S/S3/gateway` and `Body/S/S3/graphiti-runtime` targets |
...
| Redis | `Body/S/S0/epi-cli/src/graph/redis_cache.rs`; semantic cache tests; Graphiti wrapper Redis cache; gateway/session state | Redis is implemented in graph paths but target ownership is S3' temporal/context law | S3' owns Redis-backed live context; S2 consumes graph/cache handles through explicit APIs |
...
1. Produce a file-level staging disposition for `_staging/`, keyed by owning shard: [[S0']], [[S2']], [[S4']], [[S5']].
2. Audit `src/qv_data.c` dependency and decide whether it belongs under `epi-lib`, `epi-cli`, or archival Seeds.
3. Write the Neo4j schema decision note: label, coordinate property, embedding property, vector index name, dimension, and migration path.
4. Write the frontmatter parity note: Hen TS canonical law, Rust compatibility behavior, and tests needed to prove parity.
5. Write the plugin pipeline note: `Body/S/S4/pi-agent` source, `Body/S/S4/plugins/pleroma` package, `.epi` runtime, `_staging` promotion.
6. After those notes are accepted, execute the first low-risk moves with real tests before and after.
