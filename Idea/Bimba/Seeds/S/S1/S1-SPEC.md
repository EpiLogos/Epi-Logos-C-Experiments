...
The `s1'.base.ensure` public surface is contract-derived: `validate_frontmatter_contract` validates canonical [[CT]] template shape without applying instance-value rules to blanks or placeholders, requires canonical `c_1_ct_type` with `ctx_type` only as an absent-key legacy fallback, and returns `ValidatedFrontmatterContract`. `BaseEnsureParams` consumes those validated contracts and fails closed when the requested CT has no match; schema columns are a stable union with `coordinate` first, metadata in deterministic source order, and `p0` through `p5` families ordered positionally. The live [[S0]] adapter discovers the source forms under `Idea/Bimba/World`; [[Hen]] retains schema and reflection-residency authority.
