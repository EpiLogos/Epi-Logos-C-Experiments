...
Protected-local session-close return: [[Body/S/S0/epi-cli/src/gate/nara_close_bundle.rs]] is the material host adapter beneath the [[S3]] `nara.session_close` / `nara.session_close.read` contract consumed by [[M1']]. It derives the full active-[[PASU]] scope from the local identity authority rather than trusting caller metadata, derives `+7 mod 12` and octave-return aggregates from bounded traversal evidence, and atomically persists one opaque-ref aggregate bundle with private filesystem modes and symlink refusal. The additive contemplation projection may retain bounded `wisdom_delta_text` (non-empty, at most 4096 UTF-8 bytes) inside that protected local record; the active carrier reduces only its first eight bytes to a hex witness and never renders the prose. This textual field is not the C-kernel `uint64_t wisdom_delta`. This is S0 execution of S3/M1'/M5' law, not S0 ownership of session, contemplation, identity, or Third-Spanda semantics.

Typed [[M4']] Oracle and field adapters (Cycle-3 rerun, 2026-08-02): [[Body/S/S0/epi-cli/src/nara/oracle_composite.rs]] composes the existing Oracle engines into strict `nara.oracle.cast_iching`, `nara.oracle.cast_tarot`, `nara.oracle.history.read`, and `nara.oracle.update_position_state` gateway receipts. It preserves exact I-Ching line/codon law, kernel/[[portal-core]] Tarot mappings, append-only protected history, and the `generating -> muting -> mute`/reopen state machine; it attempts [[S3]] shared projection best-effort and reports `spacetimePublished` rather than treating transport failure as local data loss. `nara.field.handle` is the separate loopback-only opaque personal-field adapter; it returns no raw field, identity, journal, Oracle, or quaternion body. These are S0 host compositions over owning M4/S3 law, not new S0 domain ownership.
...
**Carrier correction.** The active consumer is [[Body/M/pratibimba-app]], whose one `GatewayClient`/bridge seam and Tauri host realise the carrier-independent contract below. References to a Theia frontend/backend extension pair in this Cycle-2 closure are frozen genealogy and do not establish a second runtime.
...
| `epi gate temporal context` | Operator mirror of the S3' DAY/NOW/session context report, including [[Kairos]], safe current kernel tick/pulse/energy, protected [[M4.4.4.4]] Pratibimba refs, Redis temporal keys, SpaceTimeDB projection facts, history archive path, and Graphiti arc orientation | [[S3']] / [[Chronos]], consumed by [[S4]] / [[S5]] agents |
...
Temporal clarification: the live portal state must also consume one shared [[S3']] temporal projection. [[DAY]], [[NOW]], vault root, [[Kairos]], safe current kernel pulse, [[Redis]] temporal keys, [[SpaceTimeDB]] projection tables, and protected [[Pratibimba]] anchor are not clock-plugin-private facts. The structural clock side reads them as M0'-M3' timing/orientation; the [[Nara]] / [[Epii]] side reads the same projection as M4'/M5' personal, review, inbox, and invocation context. `epi portal` now has a `PortalRuntimeState` carrying this shared temporal projection alongside the existing shared clock state. It hydrates from configured [[SpaceTimeDB]] projection reads by polling the canonical `session_surface` and `kairos_surface` SQL projection by default, or by opening the native `v1.json.spacetimedb` WebSocket subscription loop when `EPI_SPACETIME_SUBSCRIPTION_MODE=native-websocket` is set. In both paths it falls back to the local gateway session-store context used by `s3'.temporal.context`, and only then falls back to clock/session state. The future desktop/Tauri mirror must feed this same shape rather than inventing desktop- or plugin-specific state models. The kernel portion of this shared state is `portal-core::KernelTemporalProjection`: a safe public-current tick/pulse/energy projection computed from `KernelProjection`, not a UI-local duplicate and not the protected bioquaternion/resonance state.
