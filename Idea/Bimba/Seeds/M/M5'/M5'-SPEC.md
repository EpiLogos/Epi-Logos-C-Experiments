...
| Coordinate | IDE surface | Primary object | Operational role |
|---|---|---|---|
| [[M5-0']] | Library / Bimba Pedagogy | Bimba map, wisdom packets, Gnosis library graph, quintessential views | Reads and edits sedimented knowledge; teaches coordinate meaning; receives crystallisations from the Atelier |
| [[M5-1']] | Philosophy / Canon Studio | Epi-Logos philosophy files, worldview docs, foundational instructions | Authors, reviews, and refines the system's theoretical ground and pedagogical canon |
| [[M5-2']] | Backend Studio | S-family stack: S0 kernel/CLI, S1 vault, S2 graph, S3 gateway/SpaceTimeDB, S4 agent substrate, S5 review/autoresearch | Makes the actual S backend inspectable and agent-editable through governed tasks, tests, evidence, and live service contracts |
| [[M5-3']] | Frontend Studio | M'-Tauri/Theia app: `/body` 0/1 surface, `/pratibimba/system` IDE, M-extensions, integrated plugins | Shapes one Tauri instrument whose lightweight daily surface and deep Theia workbench share the same kernel bridge |
| [[M5-4']] | Agentic Control Room | Operational-capacity family, Pi/Sophia/Anima/Aletheia, S5/S5' Pi-agent, and S0/S0' CLI/library bridges | Routes surfacing, orchestration, review, bounded execution, and recursive improvement through governed agent capacities |
| [[M5-5']] | Logos Atelier | Etymological archaeology, word constellations, Logos-cycle FSM | Runs the scent-following method: root, cognate, drift, psychoid charge, pros-hen synthesis, and Mobius write-back |
...
The user does not arrive at M5' to be confronted with library-canon-code-graph-agent-review panels in their full technical glory. They arrive at M5' to **converse with the system about its own self-articulation, code, philosophy, and ongoing work**. The agent ([[Epii]] / [[Anima]] / [[Aletheia]] / [[Pi]] / [[Logos Atelier]] specialist) is the **primary surface**; the panes (library, canon, backend, frontend, agent control room, Logos Atelier) are *summonable contexts* that the agent opens conversationally as the conversation requires.

M5' is also the canon-recognition workbench: candidate artifacts are recognized as canon-worthy, evidence-worthy, or private/local before review and promotion. Editing is only one operation inside a larger cycle of recognition, validation, promotion, and return.

When the user says "show me the [[Bimba]] node for Word", the agent opens [[M5-0']] Library to that node. When the user says "let me edit the Sacrifice philosophy file", the agent opens [[M5-1']] Canon Studio. When the user says "run a review on this proposal", the agent opens [[M5-4']] Agentic Control Room with the proposal pre-loaded. When the user asks "what's the etymology of [[pratibimba]]?", the agent opens [[M5-5']] [[Logos Atelier]] with the word constellation.

The pedagogical depth of v3 (Gebser stadia, Name-Power conjugate-pair-vocabulary, L-lenses, Context-Frames, Spanda equations, 84-fold mode-tonic landscape, Major/Minor as conjugate-form-selection, Barry Harris 8+4 jazz-scales) is **content the agent teaches in conversation when the user engages with it**, not a wall of always-visible structural detail. [[M5-5']] [[Logos Atelier]] exposes the 6-fold-of-layers as navigable artefacts; [[M5-0']] Library exposes consciousness-register on each [[Bimba]] node; both are summoned, not always-on.

## §-Numbered M5' Canon Deltas Absorbed

§M5'.1 — **System-shape correction.** [[M5-2']] is the [[S-family]] stack itself, [[M5-3']] is the M'-[[Tauri]]/[[Theia]] app itself, and [[M5-4']] is the operational-capacity layer that binds [[Pi]], [[Sophia]], [[Anima]], [[Aletheia]], [[S5]]/[[S5']], and [[S0]]/[[S0']] bridges into governed work. [[m5-prime-system-shape-and-tauri-ide-canon]] and [[2026-05-31-theia-ide-shell-and-m-plugin-architecture]] are therefore canonical M5' dependencies, not optional implementation notes.

§M5'.2 — **One app, two surfaces.** M5' must be summoned from the lightweight `/body` 0/1 surface into the deep `/pratibimba/system` [[Theia]] IDE without becoming a separate product. The shared kernel-bridge is the first Theia extension and the live contract for harmonic profile, [[SpaceTimeDB]] presence, world clock, gateway RPC, kernel trace, audio, cymatic state, and autoresearch observability.
...
§M5'.6 — **Open contradictions.** [[alpha_quaternionic_integration_across_M_stack]] primarily assigns the `+1` of `137 = 64 + 72 + 1` to [[M1]] [[Paramaśiva]], while one residual corridor passage names [[M0]]'s witness-axis as the `+1`; M5' records this as an unresolved source contradiction rather than harmonising it silently. [[m5-prime-agentic-ide-research]] also preserves a nearer-term CodeMirror/Monaco/[[Tauri]]-workbench research path while [[m5-prime-system-shape-and-tauri-ide-canon]] canonises [[Theia]]; current M5' law follows Theia, while editor-widget migration remains an implementation question.

## User-Facing Surface

- **Primary surface: conversational agent.** [[Epii]] (or [[Anima]] / [[Aletheia]] / [[Logos Atelier]] specialist per task) is the entry point. The agent is the IDE; the panes are summonable working contexts.
- **Two-surface app rhythm.** `/body` remains the lightweight daily 0/1 surface; `/pratibimba/system` is the deep [[Theia]] IDE summoned through the omni panel, shared menubar/background process, and kernel-bridge state.
...
- **Review inbox** shows deposits from [[Nara]], [[Anima]]/[[Aletheia]], [[Graphiti]], tests, and implementation runs — surfaced when the user enters review mode or when a notification needs attention; not always-on dashboard.
...
- [[S1]] / vault/file contracts for editable philosophy, spec, library, and source artifacts.
- [[S2]] graph law and namespace-aware graph access for Bimba, Gnosis, etymology, and reviewed projections.
- [[S3]] gateway/session records, [[SpaceTimeDB]] `world_clock`, `pratibimba_presence`, shared archetype/coincidence signatures, [[Graphiti]]/[[Nara]] episodes, [[DAY/NOW]] deposition handles, and live run/event lineage.
- [[S4]] / [[S5]] bounded agent access, agent-run evidence, review inbox state, and autoresearch observation streams.
- [[S5']] live `s5'.review.*`, `s5'.improve.*`, and `s5'.epii.*` gateway routes for deposits, transitions, dry-run promotion, and Epii agent access.
- [[S0]] / kernel profile for harmonic clock evidence.
- The M5' kernel-bridge extension exposes these contracts to [[Theia]] as typed frontend capabilities rather than ad hoc UI calls.
...
M5' can inspect protected evidence only inside governed review capability. Public Epii surfaces may show evidence handles, summaries, and promotion decisions, but not private journal bodies or unreconciled Graphiti text. Promotion requires explicit review state and source lineage.
...
- Interaction: [[VAK]] execution collects CPF, CT, CP, CF, CFP, CS; routes through [[Anima]]/[[Aletheia]]; and deposits outputs to [[Epii]] inbox with completion evidence.
...
Shell `0` previews the integrated structural instrument from [[M1']] through [[M3']]. Shell `1` previews the integrated lived/world return from [[M4']] through [[M5']] with a renewed [[M0']] ground doorway. The 4+2 subsystem pages deliver the depth: [[M0']] through [[M3']] for structural/instrument work, M4' for [[Nara]], and M5' for the [[Epii]] IDE.

The one-app law is therefore explicit: the daily `/body` shell and the deep `/pratibimba/system` [[Theia]] IDE run inside one M'-[[Tauri]] instrument and share one kernel-bridge. The omni panel and menubar/background mode may summon M5' from daily use, but Theia remains the deep subsystem surface for canon, graph, code, agents, review, and Logos-cycle work.
