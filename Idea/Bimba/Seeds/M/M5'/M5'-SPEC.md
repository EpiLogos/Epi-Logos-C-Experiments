...
- [[M5-0']] Library / Gnostic Namespace substrate: **[[Body/S/S5/epi-gnostic]] IS the RAG-anything system** (production Python + Graphiti-integrated package; not a future composition target). Plus [[Body/S/S5/epi-kbase]] + [[Body/S/S5/epi-kbase-core]] for kbase corpus. Smart Connections vault semantic-input via `s1'.semantic.*` feeds candidate links + neighbours into the gnostic pipeline. Reached by agents through `s5'.gnostic.*` gateway methods consuming this substrate plus the `gnosis` graph namespace. The active [[M0']]↔[[M5-0']] Klein seam is the coordinate-scoped Library reading embedded in the existing Bimba graph host: map traversal invokes `s5'.gnostic.etymology`, separating direct `bimba_coordinate` anchors from `bimba_resonances` while projecting no entity bodies. It is not a standalone `bimba-graph-viewer` or generic view-mode ontology.
...
`ModalResonatorProfile`, serialized at `modalResonator`, is the standing-body contract under M5' drift review.
