...
| **M5-2** Backend Studio | **= The full S-family stack itself** | S0 (Terminal/kernel/CLI/lib), S1 (Obsidian/vault), S2 (Neo4j/bimba-map), S3 (Gateway Control Plane / SpaceTimeDB), S4 (Agent Runtime), S5 (Integral World Boundary / canon-promotion). M5-2 is NOT a parallel-M-side backend; M5-2 IS Epii's view-into-and-operational-relationship-with the S-stack |
...
- Desktop release-gate closure requires the Electron target to package with a fixed, builder-resolvable Electron version; `electron-builder --dir` is not accepted as release evidence until it can compute the installed Electron version and emit the packaged app directory.
