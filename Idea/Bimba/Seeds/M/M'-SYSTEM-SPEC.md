...
The shell grouping is:

```text
0 = structural side       -> M0' with routes into M1'/M2'/M3'
/ = command return        -> S0' over S0-S5, OmniPanel / command centre
1 = personal/world return -> M4'/M5'
```
...
Kernel tick
  -> Harmonic profile
  -> Pointer-web coordinate topology
  -> Temporal projection
  -> M' rendering
```
...
M' domains consume this chain differently:

| M' domain | Musical/harmonic expression |
|-----------|-----------------------------|
| [[M0']] | coordinates become nodes in the playable map; selected node exposes harmonic profile and pointer web |
| [[M1']] | graph traversal becomes melodic/topological movement across relations |
| [[M2']] | MEF/correspondence matrix exposes lens anchors, elements, modes, scales, semantic resonance |
| [[M3']] | clock renders epogdoon tick, helix phase, Kairos, solar/cosmic timing, sonic/cymatic pulse |
| [[M4']] | Nara journal and modalities inherit DAY/NOW harmonic context and can be played back as lived trajectory |
| [[M5']] | Epii reviews, teaches, researches, and improves the system from harmonic/profile evidence |

The instrument is therefore not a later feature. It is the proof that kernel, graph, temporal context, agent work, and user experience are one system.
...
- 8+4 sounded/nodal rendering role;
