...
- [ ] **I2 Aperture 17 / 18-stack reconciliation.** Per ratified DR-M3-LENS-18: kernel `lensCarrier` grows the `(-)` Operator no-frame aperture (aperture 16 `()` ≡ the existing fibonacciGround +1 — confirm and document the identity); Zod 17th-row rejection updated to the 18-shape; engine division cycling gains the no-frame view (Bimba graph without the 360° container). E1/E3 pins re-blessed. *(Register §3.2 — Architect confirm first)*
