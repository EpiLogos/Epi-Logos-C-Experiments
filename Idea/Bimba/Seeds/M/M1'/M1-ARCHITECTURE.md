...
- **DR-M1-1 (Wave A row 4) — LANDED 2026-07-07** (Tranche 02.T2.1): the residual witness-axis wording in `alpha_quaternionic_integration_across_M_stack.md §1.1` is downgraded; the source paper now carries the M1-5 (+1 parent) attribution per the standing invariant and `m1.h:526-549`. Cross-reference: [[M0-ARCHITECTURE]] for M0's own framing of "what M1 receives".
...
## T2.11 — Spanda dual-oscillator (landed 2026-07-08)

Track 02 T2.11 (cycle-3 full rerun, [[02-m1-paramasiva-reconciliation]]) landed the [[M1-3']] [[Spanda]] Core tick-floor of §2.4/§4.4/§5.4 as an actual mechanism, not a placeholder. It is authored in C at `Body/S/S0/epi-lib/src/m1.c` / `Body/S/S0/epi-lib/include/m1.h` (the `T2.11: SPANDA DUAL-OSCILLATOR — THE TICK FLOOR` block, `m1.h:404`) and mirrored in `Body/S/S0/portal-core/src/spanda.rs` — C ground → Rust surface; Rust reflects, never owns.

- **The primitive.** The (0/1)/(1/0) dual counter-phase oscillation is now a [[Haken–Kelso–Bunz]] relative-phase field: `φ̇ = Δω − a·sin φ − 2b·sin 2φ`, `V(φ) = −a·cos φ − b·cos 2φ` (`spanda_hkb_drift`/`_potential`/`_curvature`/`_settle`). Bistable at φ=0 (in-phase, `SPANDA_SEED`, the `=`) and φ=π (antiphase, the `≠`); the slash in `0/1` is the `b·cos 2φ` potential barrier. This is what SEED-at-`0x03` (both poles live) means dynamically.
...
- **Standing identity `0/1 + 1/0 = 1/1` as superposition** (`spanda_superposition`/`_standing_envelope`/`_pole_rms`): antinode 2× constructive (the 100%), node 0 (the `≠` heard as silence). The half-turn pole-swap exchanges node and antinode and is invisible on any solo-pole observable — the relational-only audibility of the `≠`.
- **Two distinct order-2 involutions**, composing to `5−n` (Klein four-group): reflection `#(n)=11−n` (`spanda_invert`, traversal-reversal; on `RING_QUATERNION_LUT` it IS the [[SU(2)]] antipode `q↦−q`) and half-turn `n↦(n+6) mod 12` (`spanda_half_turn` / C `spanda_half_turn_index` `m1.c:576`, antiphase pole-swap = π flip = 6 ticks × 30°). `spanda_invert` stays the reflection; `spanda_half_turn` is the new named op. Every `#` names which involution it means. The index-arithmetic half is pinned in `portal-core/tests/spanda_involutions.rs`.
- **The twelvefold is flowering-first.** It is generated by the fold progression 4→6→8→10→12 (`SPANDA_CF_FOLD_COUNT`, `m1.c:26`), prior to and generative of QL (`6 = 12/2` derives FROM it — `spanda_intrinsic_twelvefold`/`spanda_ql_positions_derived`). `tick12` is a readout of continuous cycle phase (`spanda_tick12_readout`); nothing re-grounds on it. The `RING_SIZE` comment at `m1.h:231-238` now reads flowering-first.
- **Codon advancement.** `spanda_codon_advance(Quaternion, cycle)` (`m1.c:609`) derives the rotational arc from the real [[SU(2)]] state (sign-carrying `atan2` over w/x — `q` and `−q` resolve differently), maps cycle → lens class, then applies the canonical epogdoon 9:8 compression 72→64 (`apply_epogdoon_compression`, imported from `m3.h`). It never reads the bare `tick12` integer. [[M3']] still owns the codon space; this owns only the advancement clock — the correct provenance for the `q_cosmic`/topology feed of §4.6.
- **Frequency anchor** (config-driven via `[ml.m1_paramasiva]` in `~/.epi-logos/config.toml`, loaded on the Rust surface): conserved-delta band ~1.5–4.0 Hz, centre ≈2.5 Hz, cited (`spanda_hkb_frequency_band`/`_citation`); derived defaults `a=1`, `b=9/16` (the 16/9 generative gap inverted — octave-minus-wholetone as second-harmonic coupling), `Δω=0`. Out-of-band `base_freq_hz` is refused by the loader; never the human-only fake-2.0.
- **Honesty fences.** The HKB 0/π bistability is NOT the [[SU(2)]] 720° double cover (that stays the [[M1-5']] quaternionic layer of §2.6); the ~2.5 Hz beat is not a physical frequency across [[M2']]/[[M3']] (the codon relation is a shared clock index, never a resonance); three rates stay distinct — ~2.5 Hz beat ≠ 1 Hz `profile.update` heartbeat ≠ 12 Hz UI display framerate.
- **Acceptance.** The six formerly expected-red kernel-truth tests are GREEN in `portal-core/tests/kernel_truth.rs` (`spanda_hkb_antiphase_bistability`, `spanda_standing_identity_superposition`, `tick12_flowers_from_oscillation`, `codon_advances_on_rotational_state_not_tick12`, `spanda_bistability_threshold_asymmetry`, `spanda_two_involutions_distinct`); their entries are removed from `plan.runs/kernel-truth-expected-failures.json`.

---

## T2.12–14 — Spanda transport arc (landed 2026-07-11)

Track 02 T2.12–14 (cycle-3 full rerun, [[02-m1-paramasiva-reconciliation]]; authored as SPANDA-T1..T3 in [[M1-3-SPANDA-TRANSPORT-ARCHITECTURE]] §8, decided by **DR-M1-5**) landed transport over the T2.11 tick floor: the phase became *holdable and walkable state*, not just mathematics. The arc keeps the Architect's constraint structural — the raw spanda oscillation (engine law) is never conflated with the gateway heartbeat (a portal pulse, derived from but never 1:1 with it). The kernel side is authored in `Body/S/S0/portal-core/src/spanda_anchor.rs` with the projection seam at `kernel.rs:756` (`from_phase_anchor`); the portal side in `Body/S/S0/epi-cli/src/gate/server/` (`mod.rs:84` `spanda_block_json`, `dispatch.rs:915` the walk arm).

- **The anchor (T2.12 / SPANDA-T1).** `SpandaPhaseAnchor` (`spanda_anchor.rs:48`) — epoch_ms, phase0, rate_hz, mode `Flowing|Held|Walking`, direction `Forward|Reflected` — is **kernel-owned, lazily evaluated** state: phase-at-instant is computed on read at any grain (audio at sample rate, UI at framerate, portal at 1 Hz), *no loop at any frequency*. The rate law reads beat = epogdoon-step from the cited ~1.5–4.0 Hz band — the cycle-rate reading would put the felt pulse 12× outside the band the config loader refuses. Both involutions ride the anchor as **named kernel acts**: `apply_reflection` (11−n, `spanda_anchor.rs:159`) and `apply_half_turn` (n+6 mod 12, `:166`), composing to 5−n — the Klein four-group as transport API. The heartbeat became a **portal sampler**: `tick12` derives from `spanda::tick12_readout` of the anchor, retiring the wall-clock tick12 derivation that contradicted T2.11's "nothing re-grounds on the bare tick"; wall-clock stays for timestamps and the world-owned kairos sky, which never holds.
- **The walk family (T2.13 / SPANDA-T2).** `m1.spanda.{hold,release,walk_to,step,half_turn}` registered through the full 16.T16.14 chain (contract constants → `METHOD_NAMES` → dispatch_plan rows → S3 route metadata `s0.product-spanda-transport` → S0 dispatch arm `dispatch.rs:951` → parity rows). **One anchor on `GatewayRuntimeState`** — the heartbeat installs and samples it, the walk arm mutates it, every subscriber sees the held or walked organism: one organism, no private timelines. Reflect and backward are distinct named acts (both-in-one refused); a missing anchor is answered honestly, never fabricated. `generation` stays a portal-owned monotonic emission counter, advancing while held — same phase content, new generations.
- **The wire (T2.14 / SPANDA-T3).** The **anchor rides `profile.update`** as public-safe plain numbers (epochMs/phase0/rateHz/mode/direction/tick12) via shared `spanda_block_json` — one shape, two carriers, no drift. Transport acts additionally push `portal.spanda_transport` **immediately**, between heartbeats (live capture: +27 ms after the last sample). The old `slerpFraction` ask **dissolved**: clients evaluate the shared anchor locally — carrier mirror `bridge/types.ts:810` (`SpandaAnchorBoundary`, `readSpandaAnchor` refuses malformed shapes) with `spandaPhaseAt`/`spandaTick12At`/`spandaFractionAt` as pure functions of the anchor. `slerpFraction` is never emitted (live capture confirms absent). Evaluating the shared anchor locally is the one clock read locally, not a private clock; what stays forbidden is a renderer inventing a rate or advancing an anchor itself.
- **The face (closed under 22.T22.1, fate A/B + C-unblocked per DR-FACE-7).** `SpandaNavigatorPane` (carrier `src/panes/`): twelve epogdoon stops (click = `walk_to`), hold/release, involutions named in the UI (`#` reflect = 11−n, half-turn = n+6), next-event preview (Klein flip 5→6, Möbius return 11→0), and the anchor-derived fraction bar — the dissolved `slerpFraction` reborn as local evaluation. The face owns nothing: no rAF/interval, renders per heartbeat and transport response; honest absence when no anchor rides the bus. `WalkPane` (graph-walk-as-melody) is a different, live face — untouched.
- **Naming law (the two transports, never conflated).** *Display-scrub* — a face replaying already-received records; face-local, always permitted (DR-TS-6), owned by the modulation engine's 720-ring. *Engine-walk* — kernel anchor mutation, always broadcast. A face scrubs its view; only the kernel walks the organism. Every `#` in the transport API names which involution it means.
- **Honesty fences.** Three rates stay distinct (T2.11 fence upheld): the ~2.5 Hz oscillator band ≠ the 1 Hz heartbeat ≠ the UI framerate. The anchor authority migrates INTO the realtime audio engine when it lands — implementor changes, contract does not; the portal never owns the phase in either era. Determinism is native (`from_phase_anchor` re-derives the same harmonic content at the same phase) — no event-log replay machinery exists or is needed.
- **Acceptance.** Kernel-truth anchor determinism green (held ⇒ constant readout across evaluations); `dispatch_contract` 27/27 with parity manifests; live-wire walk-smoke against a spawned gateway (flowing → hold → tick12 constant across advancing generations → release → flows again; transport event arriving between heartbeats); Playwright `spanda-navigator.spec.ts` drives the live face (hold, walk, reflect, release) — ledger closes 02.T2.12/13/14 + 22.T22.1, 2026-07-11.
