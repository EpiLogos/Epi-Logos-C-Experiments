...
| **22.T22.1** Spanda walk | Walk the matheme as a playable instrument: transport over the tick | **A/B + C** | Sorted in full by [[M1-3-SPANDA-TRANSPORT-ARCHITECTURE]] (DR-M1-5). Two transports, never conflated: **display-scrub** (face-local replay of received records — modulation engine already owns it; fate A/B: build the twelve-stop navigator + fraction bar + next-event preview over the anchor) and **engine-walk** (kernel anchor mutation via the `m1.spanda.*` walk family — fate C until SPANDA-T1..T3 land; buttons appear on capability presence, no disabled placeholders). The old `requestScrubToTick`/`slerpFraction` asks dissolve into the kernel-owned `SpandaPhaseAnchor` riding the wire. `WalkPane` is a *different, live* face (graph-walk-as-melody) — leave it alone. |
