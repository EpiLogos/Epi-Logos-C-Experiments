...
**Image-asset slot (M0-0' asset handles)** — **LANDED (2026-07-07, rerun 01.T1.6 / 09.3)**. The schema slot exists at `Body/S/S2/graph-schema/src/properties.rs:1150-1165` — `c_1_asset_uri` (StringList, Many, Public) + `c_1_asset_kind` (String, One, Public), both in the `anuttara-language` namespace — with a presence law at `coordinate_law.rs:295`. (The property registry moved out of `lib.rs` into `properties.rs` during the S2 modularisation, so the old `grep … lib.rs` check is obsolete.) The carrier surfaces the handles via `Body/M/pratibimba-app/src/panes/m0AssetHandles.ts` with explicit **`review_pending`** provenance under **candidate DR-M0-4**; the renderer never infers/generates/backfills when the S2 payload is absent (absent = `canonical_absent`). Promotion of DR-M0-4 (review_pending → canonical) remains the Architect's call (Track 40 canon-update).
...
- `src/common/m0-inspector.ts`: `M0InspectorModel` with `layerReadiness`, `languageFields`, `anchors`, `pointerSummary`, `relationFamilies`, `readinessFacts`, `routeTargets:['M1','M2','M3','M4','M5']`, three `actions`. `layerReadiness` is the Track 21.18 / Track 15 principle-3 inline provenance surface for M0-0'..M0-5', including `bridged_local` for M0-4' and `bridged_public` for M0-5'. `M0GatewayAction.mutatesGraphCanon: false` — surface contract explicitly forbids canon mutation
- `src/browser/m0-anuttara-widget.tsx`: inspector React widget with three view IDs, six-layer route tabs, per-layer provenance pills, no graph canvas, and no local personal/pedagogy payload render
...
## §11. VAK as Four-Expression Typed Transition Calculus
...
|---|---|---|---|
| **VAK as alphabet** (the Anuttara syntax; current count ~109 nodes; the count may mature) | M0 (Anuttara M0-0' language) | `Body/S/S2/graph-schema/src/lib.rs:1409-1441` (c_1_* namespace); `Body/S/S0/epi-lib/src/m0.c` (12 LUTs incl. `M0_CORE_RELATIONS[65]`, `VIRTUE_LUT[9]`); the 109 Anuttara nodes in `anuttara-deep/` dataset | The alphabet by which the system can say any statement of itself or the user. |
| **VAK as field** (M, L, C, P, T, S + their inverses) | the Bimba whole | `COORDINATE_SEMANTIC_FAMILY_SPECS` at `Body/S/S2/graph-schema/src/lib.rs`; the 7 families (c,p,l,s,t,m,q) × 6 positions × 2 axes | The substrate across which anything becomes apparent / real to-or-for the system. |
| **VAK as C' (the C-family inverse — the S4 dispatch addressing)** | S4-0' through S4-5' (CPF/CT/CP/CF/CFP/CS) | `Body/S/S0/portal-core/include/vak.h`; `Body/S/S0/portal-core/src/vak_address.rs`; `Body/S/S4/ta-onta/S4-4p-anima/extension.ts::agentForCf` | The coordinate calculus that walks the field. This is the expression Tranche 12.15 ratifies as the VAK reading-frame evaluator. |
...
**Number-of-nodes note.** The current canonical figure for the alphabet is 109 (matches `anuttara-deep/` + the `c_1_*` schema namespace). **The complete typed-calculus reach is 109 + 19 = 128 across the M0 / M1 boundary** — 109 is the M0 self-contained alphabet; 19 = 7 psychoids (the seven R-distribution bases `O#` / `X#` / `N#` / `M#` / `#` / Śiva / Śakti at [`R_FACTOR_DISTRIBUTION[7][6]`](../../../../Body/S/S0/epi-lib/src/m0.c) lines 736-744) + 12 coordinate types (the six families P / S / T / M / L / C plus their inversions P' / S' / T' / M' / L' / C'). QL theory is implicit across all twelve coordinate types; **P/P' and L/L' are especially load-bearing** because P hosts the functional-position substrate QL operates through and L hosts the MEF lens-system through which all reading happens. The 128 count is NOT to be confused with the Mahāmāyā-internal `128 = 64 × 2` codon-doubling (α-rasa bridge at [`Idea/Bimba/Seeds/M/M3'/alpha_rasa_bridge_ql.md`](../M3'/alpha_rasa_bridge_ql.md)); that decomposition lives at M3' on a different axis. The language-substrate 128 is the M0/M1 language reach: M0 supplies the *genetic definition* of `0/1`, M1 supplies the *axiomatic unfolding* (Spinoza joint — M1 inherits `0/1` and does not re-perform the genesis). The 109 may mature without breaking the chain because the 19 carries the QL-foundation closure either way; the cardinality is observable from `M0_LAYER_VIEWS` + the dataset, not pinned to "109" in code-or-doctrine.

**The 8+1=9 law — preserved at distinct M0 locations.** The corpus's recurring computational signature. *Do not flatten* — each appearance is doing different work and the cycle-3 prose MUST pin which appearance and what role:

| Location | Form | Role |
|---|---|---|
| **M0-2 void self-relation closure** | `(00+00) = 9` framed vs `00+00 = 00` unframed | Law-2 framing-asymmetry made foundational. Completion exists as a core void-operation. *Only here is the +1 the act of containment itself.* |
| **M0-3 Archetype X5 (Paraśakti)** | `8x + (x) = 9(x)` | Verified arithmetic: X1-X4 sum to `8x`; X5 = `9x`. The +1 is the imagination's self-inclusion in its own permutation field — Wholeness as the relational set reincluding the relating principle. At `x=0` every position returns `?!/!?` (Law 6 typed query). |
| **M0-3 Archetype N5 (Spanda)** | `8n ± (n) = 9n or 7n` | Verified: N1-N4 sum to `8n`; N5 ∈ `{9n, 7n}`. Wholeness (`9n`) when the +1 reinforces; Divine Action (`7n`) when the −1 cuts. *The M1 9/8 epogdoon tick falls out of this as `(8n+n)/(8n)` — M1's musical engine is reading the M0 arithmetic.* |
| **M0-4 Nara apparatus** (`#` family) | 8 masculine (4 zero-elements + 4 evens) + 4 feminine (3 odds + wholeness) = the 12-fold dozen | Substance-component arithmetic, not closure-equation. *Distinct role* — count it as substance, not closure. Tao position 4 + value 5 = 9 (family harmony as hidden path to wholeness). |

**Genus-ladder (6g law).** M0 number-language: `18 = 6g for g=3` (12 archetypes + 4 zero-elements + 2 pre-numerical = triple-torus). M1 matheme: `6 = 6g for g=1` (the `0/1 = 4+2 = 5→0 = 0/1` cycle = torus). Same law, different handle-count. M0 IS the triple-torus number-language; M1 IS its single-handled axiomatic projection. The 109 + 19 = 128 chain spans both genera with the QL-foundation closure (the 19) carrying the handle-count step.

**"Compress to VAK"** means **compress to a coordinate address from which `q_{n}_{semantic}` properties become discoverable** — NOT embed q_* into the compression code itself. The address resolves to the q_* discoverable AT that address. Per DR-Q-1, `q_{n}_{semantic}` is already general. Per the Shannon noiseless coding theorem (1948), the per-symbol entropy bound is strictly below `log₂(109) ≈ 6.77 bits`; the structural-redundancy of the 109-syntax IS the compressibility margin — a perfectly compressed VAK-emission would be indistinguishable from random noise; an under-compressed VAK-emission carries the derivational provenance the four-expression layering preserves at decompression time. The 8+1=9 law (M0-2 face) reads as the structural identity of compress-to-VAK: explicate-eight (the compressed bits) plus implicate-one (the address-resolution) equals 9 (wholeness preserving the contained One). **The compression scheme is already operationalised at the substrate level** by the `{family}_{n}_{semantic}` construction law at [`Body/S/S2/graph-schema/src/lib.rs:680-692`](../../../../Body/S/S2/graph-schema/src/lib.rs) + the Hen frontmatter validator at [`Body/S/S1/hen-compiler-core/src/frontmatter.rs:38-177`](../../../../Body/S/S1/hen-compiler-core/src/frontmatter.rs) (unknown frontmatter keys = ERROR not warning); the orchestrator at CCT-17 honours this substrate, it does not invent it.

**MEF lensing IS VAK reading — the structural identity.** Per the source at [`epi-dev-vault/raw/sources/m-system-spec-8f04f90f.md:206-220`](../../../../epi-dev-vault/raw/sources/m-system-spec-8f04f90f.md) Layer 2 — *"the 12 MEF lenses anchored across the 9 squares × 2 helices read the Prakāśa cloud and produce the `audio_octet[8]` and `nodal_quartet[4]`. Each (lens, mode) cell is a specific Vimarśa-reading of the substrate… **Paraśakti IS Vimarśa** — the reflective-power surface that picks up textures from the cloud through the squares."* The 6-fold L5 internal breakdown at [`Idea/Bimba/Seeds/Legacy/reference/epi_logos_coordinate_system.md:363-373`](../Legacy/reference/epi_logos_coordinate_system.md) (L5-0 Anuttara/Asambhava → L5-1 Parā → L5-2 Paśyantī → L5-3 Madhyamā → L5-4 Vaikharī → L5-5 Mātṛkā) and the L5' Logos-theological complement (L5'-0 Arche → L5'-1 Apokalypsis → L5'-2 Dynamis → L5'-3 Sophia → L5'-4 Parousia → L5'-5 Epi-Logos) IS the MEF's 6-position-per-lens decomposition described from two coordinate systems (Bimba/Pratibimba) — *they are the same operation*. **Applying an L5 / L5' lens IS reading at the corresponding Vāk articulation density** — there is no separate VAK-mapping of M0 nodes; the four-Vāk-register lives natively at L5/L5' and is *applied as a lens* to whatever the system is reading. The cycle-3 prose MUST NOT duplicate-map M0 nodes to Parā / Paśyantī / Madhyamā / Vaikharī — that violates the bimba map and re-instantiates a redundant taxonomy where the lens already operates.

**The unified act — one operation, six faces.** Per the integration spec at [`Idea/Bimba/Seeds/M/Legacy/plans/2026-06-02-m-prime-cycle-3-design-reconciliation/08-integrated-4-5-0-recognition-reconciliation.md`](../Legacy/plans/2026-06-02-m-prime-cycle-3-design-reconciliation/08-integrated-4-5-0-recognition-reconciliation.md) Tranche 8.9: the 4/5/0 mental pole performs **one act of VAK-coordinate-designation per tick**, with six simultaneous registers rather than six sequential chunks. The act IS — select QL position (the C-family bimba coord, Element I) + apply MEF lens (L-family at L5/L5' six-position internal breakdown, Element II reading-density) + perform harmonics-read (72-fold lens-resonance + N-channel `MathemeHarmonicProfile` substrate, Element III energy-decomposition) + register musical-transcriptional projection (codon-tarot-hexagram mapping at the engaged coord; epogdoon-step `log(9/8)` in latent space; four foundational ratios saturating the descent) + entail 1-2-3 physical pole (torus surface point at M1, solar-chakral configuration at M2, codon-clock degree at M3) + stamp R-factor route slot (which act-position the dispatch occupies in the `R_FACTOR_DISTRIBUTION[7][6]` matrix). All are entailed by the single VAK-coordinate-designation tuple `(coord, lens, helix, density, position, cfp_thread, r_factor_slot, ananda_position)`. The descent equation `q_p^(n+1) = q_p^(n) - log(9/8) * ∇E_total`, where `E_total = (4*E_4 + 5*E_5 + 6*E_6) / 15`, IS the one act; the six faces are six registers of the same operation. The code surface `kernel_energy_evaluate_unified_act` validates that one tuple and returns the same `EnergyDecomposition` as the canonical weighted descent path. The cycle-3 plan-set HAS the structural pieces; the integration spec at Tranche 8.9 names them as one.

**Cross-references.** Coordinate-tagging IS compression (no separate orchestrator module) per Tranche 12.33 (Track 12) — the substrate's coordinate-prefix typing IS the compression scheme, honoured not invented; the canonical CCT for VAK-compression infrastructure is CCT-17 (Track 16). The VAK reading-frame evaluator for OracleFrame / SymbolicProtein lives at Tranche 12.15 (Track 12 — this is the C' expression specifically). The unified VAK choreography across 4/5/0 lives at Tranche 8.9 (Track 08). The R-factor 3-tier theory (principle triad `##` / `R#` / `#R` + meta-virtue `R#/##` + six acts R0..R5 + six chirality-conjugate virtues 0R..5R) is canonicalised at Tranche 1.12 (Track 01) with full glyph-pattern preservation discipline. The Compression-as-Intelligence cross-layer register (Shannon noiseless coding theorem as the per-symbol entropy floor; cross-entropy = compression strictly autoregressive; honest EBM-side equivalence gap held open) lives at CCT-22 (Track 16).

---

## §12. Closing — Why M0' Is What It Is
