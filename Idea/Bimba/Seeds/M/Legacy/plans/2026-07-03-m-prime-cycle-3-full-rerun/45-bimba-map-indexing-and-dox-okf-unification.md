...
2. **T45.1 — Extend Hen `is_valid_coordinate` to the full multi-level coordinate grammar (+ tests)**
   ...
   Depends on Track 00 Tranche 3.
   Verify: `cargo test -p hen-compiler-core` covers deep coordinates — `is_valid_coordinate("M2-5-0")`, `is_valid_coordinate("#0-2-9")`, a context-frame `M0-4.(0/1)`, and a prime `M2-5-0'` all accepted; invalid over-range/over-depth still rejected; verifier ≠ closer.
