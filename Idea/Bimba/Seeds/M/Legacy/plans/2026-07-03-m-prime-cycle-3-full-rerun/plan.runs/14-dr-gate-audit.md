# 14.T14.2 — Decision-Register Gate Audit

Audited: 32 named rows against `13-decision-register.md` (machine audit, dr-gate-audit.mjs).
Gate law: every row VALIDATED or explicitly DOWNGRADED. Exceptions: 0.

| DR row | Status | Register line |
|---|---|---|
| DR-M0-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Downgrade UX §7 "full |
| DR-M0-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Coordinate-prefixed ` |
| DR-M0-3 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** M0' has six M0-X' dat |
| DR-M1-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Standing invariant. T |
| DR-M1-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** **Full 3D Bevy/wgpu r |
| DR-M2-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** 10 planets total incl |
| DR-M2-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Six 72-axes (MEF / ta |
| DR-M3-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Runtime law is author |
| DR-M3-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** The 72→64 fold is **t |
| DR-M3-3 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** **Never a contradicti |
| DR-M4-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** The canonical day pat |
| DR-M4-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user |
| DR-M5-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Cleanup 2026-06-03 (final per DR-S4 |
| DR-M5-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Sibling of DR-M1-1. S |
| DR-B-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Land it. `Body/S/S4/p |
| DR-B-3 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Cleanup 2026-06-03 (final per DR-S4 |
| DR-KB-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Already resolved at C |
| DR-KB-2 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Typed `MathemeHarmoni |
| DR-TS-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Spec says it cleanly: |
| DR-IG-1 | VALIDATED | **Status:** VALIDATED  ·  **Validated:** 2026-06-02  ·  **By:** user  ·  **Resolution:** Extend schema with `c |
| DR-S1-4 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** Pre-ratified via standing-i |
| DR-S1-5 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** `Idea/Bimba/World/Types/Coo |
| DR-IG-2 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-IG-3 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-IG-4 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-IG-5 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-IG-6 | VALIDATED | **Status:** VALIDATED with CORRECTED GEOMETRY · **Validated:** 2026-06-03 · **By:** user · **Resolution:** Ado |
| DR-M1-3 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Note:** `#` IS essentially (0/1) — the in |
| DR-M1-4 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-M2-3 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-M4-3 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
| DR-M5-3 | VALIDATED | **Status:** VALIDATED · **Validated:** 2026-06-03 · **By:** user · **Resolution:** see below. |
