...
3. **The `lib`/`bin` split must come FIRST (T53.2) or nothing else can pass.** `epi-logos` is simultaneously a library (`src/lib.rs`) and the `epi` binary (`src/main.rs`). The **library** is what carries every upward dependency — including `pub use epi_s5_*::*` re-export blocks in `lib.rs`. A composition root legitimately depends on everything it composes; a layer library must not. Until those are two crates, the S0 library cannot be clean no matter how much code moves.
...
5. **Do not add an allowlist entry to make a tranche pass.** The mechanism is deleted in T53.10. Any tranche that needs a new entry is a tranche that has mis-designed its move; fix the design.
...
1. **T53.1 — The method-handler port at S-root, and the inversion that makes residency possible**
...
2. **T53.2 — Split `epi-logos` into a composition-root binary and a clean S0 library**

   Brief: The keystone. `epi-logos` is both the `epi` binary and the S0 library, and the library carries every upward edge — `src/lib.rs` re-exports `epi_s5_epii_agent_core`, `epi_s5_epii_autoresearch_core`, `epi_s5_epii_review_core` wholesale. Until the composition root is its own crate, the S0 library cannot satisfy the boundary rule regardless of how much code moves. After this tranche the binary may depend on every layer (it composes them; it is not a layer), and the S0 library's forbidden-import row must be satisfiable.
   Build: separate the `[[bin]] epi` target into its own crate (`Body/S/S0/epi-cli` keeps the binary + command tree; the S0 library law moves to a sibling crate, or the binary crate takes the upward deps and the library keeps none — the split line is: anything that *decides* stays in a layer, anything that *wires* goes in the binary). Declare the binary crate in `rustSStackBoundary` explicitly as the composition root with an empty `forbiddenImports`, so its licence to depend downward on everything is stated in the contract rather than assumed. Delete the `pub use epi_s5_*::*` re-export blocks from the S0 library; consumers import from the owning crate.
   Depends on Track 53 Tranche 1.
   Verify: real behavioral/live-wire proof per Track 00 — `cargo build` + `cargo test` green from `Body/S/S0/epi-cli`; the boundary lint reports the S0 *library* row with zero forbidden edges (its allowlist entries removed, not re-pointed); `epi --help` and one live gateway call still work. verifier ≠ closer; evidence = fresh command output.

3. **T53.3 — S3 takes its own control plane: protocol, routing, session, subscription**
...
4. **T53.4 — S1 handlers return to S1**
...
5. **T53.5 — S2 takes the graph: 28 files of substrate plus its handlers**
...
6. **T53.6 — S3′ temporal, Redis and Graphiti handlers return to S3**
...
7. **T53.7 — S5 handlers return to S5**
...
8. **T53.8 — The remaining tail: agent, bimba, portal, nara, tui**
...
9. **T53.9 — Delete the allowlist and the escape hatch**
...
11. **T53.11 — The structural guard that replaces vigilance**
    ...
    Depends on Track 53 Tranche 10.
