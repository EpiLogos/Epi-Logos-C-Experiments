...
> ⚑ **SUPERSEDED-IN-PART — ratified ambient / epi-genetic architecture (2026-07-14).** This track's routing of the **live planetary / collective feed into the personal being-pattern (PASU) register** is **SUPERSEDED-IN-PART** by [[M'-AMBIENT-EPIGENETIC-TRANSFORM-SPEC]]. The live planetary / collective sky routes to the **`q_environment` transform factor** — an epi-genetic transform *onto* the M4 PASU base — **NOT the PASU personal identity register**. **Identity is the natal invariant** (birth place / time / natal chart), per **DR-ENV-1**; the transiting / collective sky **transforms, it never identifies**, and **planets must never collapse into `q_identity`**. Outer planets are the canonical ambient-condition band (**DR-ENV-2**), not a pending M2-5 personal-correspondence dataset; only natal-derived weights may source identity (**DR-ENV-4**). Tranche-level amendments below: **T37.2** (DR-ENV-1), **T37.4** (DR-ENV-4), **T37.9** (DR-ENV-2).
...
2. **T37.2 — Ratify the planetary-elemental projection**
   ...
   > **AMENDED — [[M'-AMBIENT-EPIGENETIC-TRANSFORM-SPEC]] (DR-ENV-1).** Split **natal** (identity, invariant — Sun-rooted, kept) from **live/transit** (environment): the transiting sky **transforms, it does not identify**. Identity is the natal invariant, never derived from the live planetary feed.
...
4. **T37.4 — Export the M2 elemental + cymatic contributions**
   ...
   > **AMENDED — [[M'-AMBIENT-EPIGENETIC-TRANSFORM-SPEC]] (DR-ENV-4).** Core-collapse tranche. The **LIVE/collective** band routes to the **`q_environment` transform factor** (the epi-genetic transform *onto* the M4 PASU base), **NOT the PASU personal-identity register**. Only **natal-chart-derived** elemental weights (a birth-time invariant) may source identity; the live sky may only transform.
