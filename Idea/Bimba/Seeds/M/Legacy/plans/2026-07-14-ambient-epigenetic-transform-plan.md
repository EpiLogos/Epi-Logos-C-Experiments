...
- **P6 remaining:** **Stage B (gateway serve)** — add `environment_quaternion: Option<[f32;4]>` to `MathemeHarmonicProfile` (profile.rs, additive serde-skip; **crosses the S3 wire contract — flag [[M5'-SPEC]]**) + `heartbeat_environment()` attach at `gate/server/mod.rs`. Decision to make: serve **static-calm** (the live server's env is identity today → strip flips 'pending'→'calm', proves the wire) vs **full-dynamic** (compute `derive_env_quaternion` from the live sky's outer planets aspected against the PASU natal in the heartbeat, call `update_environment_quaternion`, serve the result — the real P6.2 payoff; needs the natal in the heartbeat). **Stage C (UF)** — `visual-panes.spec.ts` env-strip assertion against the real gateway.
- **Also open:** P2 richer natal reference. Phase 0 (cycle-3 track amendments) landed earlier this session (`8a2e1dab`).
