...
- **Open (next):** P5 codon→RNA→amino-acid + Major-Arcana sequences (substrate partly landed — `codon_to_amino_acid`, tarot transcriptions in `m3_transcription_bridge.rs`; verify-first), P6 carrier + M4 sink live-wire, P2 richer natal reference. Phase 0 (cycle-3 track amendments) landed earlier this session (`8a2e1dab`).
