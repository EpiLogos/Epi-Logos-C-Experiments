...
- **P6.1 Stage A — DONE (app carrier)** (`00dc4cf4`). Mapped verify-first by a 4-agent workflow (kairos/quintessence vertical = the structural twin). Threaded an `environment` ModulationInputKey through all six stages (WireHp → `harmonicSnapshot.envQuaternion` length-4 guard → `EnvironmentFrame` + `ModulationFrame.environment` → `deriveFrame` → `availableInputs` → engine `KNOWN_INPUTS`). Pure `environmentReadout` with three HONEST states: `null` ⇒ env pending, `'calm'` ⇒ identity rotation (no ambient influence), `'active'` ⇒ real ambient wind + vector-part torque. `CosmicEngine` renders an `engine-environment` strip (badge / 'environment pending'). 6 modulator tests + 31/31 modulators, 170/170 engine, tsc clean.
- **P6.2 Stage B — DONE (gateway serve, full-dynamic — Architect-chosen)** (`a4bac986`). `MathemeHarmonicProfile.environment_quaternion: Option<[f32;4]>` (additive serde-skip → `environmentQuaternion`; **S3 wire contract — flag [[M5'-SPEC]]**). `heartbeat_environment(&sky)`: the live sky's transpersonal planets (Uranus/Neptune/Pluto, DR-ENV-2) each aspected against the PASU natal Sun (persisted `natal.json`, ecliptic 0..360) → `derive_env_quaternion`; None when no natal cached (honest pending). Attached in the heartbeat's live-sky block reusing the fetched degrees. Units verified (transit + natal both ecliptic 0..360). portal-core + epi-cli build clean; math proven by environment.rs (8 tests).
- **P6.2 Stage C — DONE (UF)** (`5cea7497`). `visual-panes.spec.ts` asserts the `engine-environment` strip is always in one honest state (badge `calm|active` present / `engine-environment-pending` absent), mirroring the kairos honesty assertion. Spec lists clean; full browser-vs-real-gateway run is the app-ui-flow gate.
- **Phase 6 COMPLETE.** The ambient transform is live end-to-end: outer planets aspected against the PASU natal → env quaternion → wire → carrier strip; `q_identity` never touched (DR-ENV-1).
- **Whole layer: Phases 0·1·3·4·5·6 COMPLETE.**
- **Open:** P2 richer natal reference (aspect against the full natal chart, not only the Sun); the standing **P3.2 core-law classification** ([[M3'-SPEC]] GREEN-name/RED-owner); and the **[[M5'-SPEC]]** flag for the `environmentQuaternion` wire field.
