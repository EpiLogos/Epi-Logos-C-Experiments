...
## Recent resolutions log (as of 2026-06-02)

The Track 05/06/07/08 parallel pass and the Track 13 modularity-cleanup tranche closed or hardened the following entries. Per-entry status lines below carry the citation; this log is the index.
...
- **IOD-17** (capability-matrix three-way parity) — Live as spec-time assertion in `extensions/agentic-control-room/tests/run-flow.test.mjs::parity over the real capability-matrix.json` plus `Body/S/S3/gateway/tests/dispatch_contract.rs` (gateway side) and `tests/canon-studio-save-routing.test.mjs` plus `tests/contract.test.mjs` in `@pratibimba/ide-shell-m0-m5` (UI side). Three-way parity (matrix ↔ UI ↔ gateway) verified across 42/42 tests. Live runtime parity check still recommended (Thread A finding #2 — expose `s4'.mediation.capabilities.list` on the gateway). **(2026-06-02 — Track 13 closure):** runtime parity follow-up surfaced by 13.T6 as 6 S0-mirrored S4 constants; FU-T6-A..E catalogued in `plan.runs/13-t10-modularity-report.md` §5.
- **IOD-18** (Smart Connections via Hen `smart_env.rs`) — Binding per [ADR-05-010](../../../Idea/Pratibimba/System/docs/decisions/adr-05-010-hen-vault-bridge.md) and ADR-05-011 §4. No client-side Obsidian plugin coupling; semantic-neighbours route through Hen-gateway `s1'.semantic.*` (gated on Track 03 T6.5). **(2026-06-02 — Track 13 closure):** Track 03 T6.5 confirmed landed by 13.T8 (`s1'.semantic.suggest_links` classified in `Body/S/S3/gateway/src/dispatch.rs:174-192`); deferred surfaces `s1'.semantic.{neighbors_of, search, by_block}` catalogued as FU-T8-A.
- **IOD-19** (Hen vault-write gatekeeper) — Binding per ADR-05-010 and ADR-05-011 §3. Canon Studio routes every save through `vault-bridge.s1prime.vault.write_file`; until T4.5 lands the vault-bridge extension, saves are rejected with the canonical reason `no vault-bridge registered` — verified by `tests/canon-studio-save-routing.test.mjs::vault-bridge write throws "no vault-bridge registered" before T4.5`. **(2026-06-02 — Track 13 closure):** Track 03 T6.5 `s1'.vault.{read_file, write_file, rename_file, move_file}` confirmed landed by 13.T8; wikilink-integrity fixture test `t8_governed_rename_leaves_no_orphan_wikilinks` proves rename reconciles `[[Source]]` → `[[Origin]]` end-to-end across alias and heading-anchor forms.
- **IOD-01** (S3 single WebSocket multiplexing) — **(2026-06-02 — Track 13 closure):** Resolved by 13.T2/T3/T4. S3 owns the canonical route table (`Body/S/S3/gateway-contract::METHOD_DISPATCH_PLAN` + `Body/S/S3/gateway/src/dispatch.rs::classify_method`); SpaceTimeDB subscription envelope unified at `gateway-contract::SpacetimeSubscriptionLifecycleEnvelope`; explicit `SpacetimeFallbackPolicy { NativeWebsocket, FallbackActive, Disabled }` + `silent-http-fallback-forbidden` sentinel. S0 retains zero native WebSocket decoding, zero lifecycle envelope construction, zero fallback policy decisions, zero readiness schema construction. Single client-facing manager invariant satisfied. See `plan.runs/13-t2-s3-dispatch-extraction-evidence.md`, `13-t3-s3-runtime-extraction-evidence.md`, `13-t4-spacetimedb-bridge-evidence.md`.
- **IOD-08** (Graphiti runtime boundary) — **(2026-06-02 — Track 13 closure):** Resolved by 13.T7. `Body/S/S0/epi-cli/src/gate/graphiti.rs` is a 5-line `pub use epi_s3_graphiti_runtime::{...}` re-export; S3 runtime owns deposit/search substrate; S5 owns governance via per-agent capability envelope (`EpiiAgentAccess::deposit`); S2 owns storage substrate (Gnosis config + Neo4j). S0 emits `governance_owner: "S5'"` + `storage_substrate: "S2"` markers on every `gnosis_context_retrieve` / `gnosis_status` response so any `epi techne` consumer sees the boundary on the wire.
- **IOD-09** (S5 state storage + `ReviewSource` metadata) — **(2026-06-02 — Track 13 closure):** Resolved by 13.T7. Canonical S5 gate-root layout pinned at `<state_root>/s5/{epii-review,epii-autoresearch}` via `STORE_SUBPATH: [&str; 2]` + `review_store_path` + `improvement_store_path` helpers. Store-location tests (`gate_review_store_subpath_is_stable_at_s0_s5_boundary`, `gate_improve_store_subpath_is_stable_at_s0_s5_boundary`) prevent layout drift. Backward-compatible with existing JSON-backed first production slice; embedded-storage migration seam preserved for when route/orchestration scale demands it.

The remaining entries (IOD-02..07, IOD-10, IOD-11, IOD-13, IOD-14, all UFV, DSD, and DCC entries) remain open as recorded below.

**Plan closure note (2026-06-02):** Track 13 (S/S' Modularity And S0 Membrane Cleanup) closed at 13.T10 — the final tranche of the 110-task implementation plan. The final S/S' modularity report at [`plan.runs/13-t10-modularity-report.md`](./plan.runs/13-t10-modularity-report.md) catalogues each method family's current S0 responsibility, Body-native authority, remaining compatibility shims, removal timetable, and 10 follow-up tranches.
...
- [[2026-05-31-theia-ide-shell-and-m-plugin-architecture]] - `Idea/Bimba/Seeds/S/S4/S4'/Legacy/plans/2026-05-31-theia-ide-shell-and-m-plugin-architecture.md`, especially sections 4, 8, 10, 11, and 12.
...
| IOD-01 | S3 single WebSocket surface — **Binding (Track 13 T2/T3/T4, 2026-06-02)** | Implementation-owner (Resolved) | 01, 03, 05, 06, 07, 08 |
...
| IOD-08 | Graphiti runtime boundary — **Binding (Track 13 T7, 2026-06-02)** | Implementation-owner (Resolved) | 02, 03, 04, 05, 06, 08 |
| IOD-09 | S5 state storage and `ReviewSource` metadata — **Binding (Track 13 T7, 2026-06-02)** | Implementation-owner (Resolved) | 04, 05, 06, 07, 08 |
...
### IOD-01 - S3 single WebSocket surface: physical multiplexing versus app-level manager — **RESOLVED (Binding)**

- **Status (as of 2026-06-02 — Track 13 closure):** Resolved by Track 13 T2 / T3 / T4. S3 owns the canonical route table (`Body/S/S3/gateway-contract::METHOD_DISPATCH_PLAN` cross-walked at `Body/S/S3/gateway/src/dispatch.rs::classify_method` + `dispatch_plan_entry`); S0 dispatches only through S3-owned route metadata (no parallel route table in `Body/S/S0/epi-cli/src/gate/server.rs`). SpaceTimeDB subscription envelope unified at `gateway-contract::SpacetimeSubscriptionLifecycleEnvelope` (one envelope type regardless of whether `s3'.temporal.subscribe` or `s3'.spacetime.subscribe` is dispatched). Explicit `SpacetimeFallbackPolicy { NativeWebsocket, FallbackActive, Disabled }` with named `silent-http-fallback-forbidden` sentinel (`SPACETIME_SILENT_HTTP_FALLBACK_FORBIDDEN` in `gateway-contract`); `gateway::spacetime::silent_fallback_refused()` returns it and `assert_no_silent_fallback_in_value()` audits any JSON value for forbidden emissions. S3 runtime emits `handlerOwner = "S3.gateway.sessions"` / `"S3.gateway.chat"` on every envelope as load-bearing evidence ownership lives in S3. T9 route-ownership cross-walk test (`Body/S/S3/gateway/tests/dispatch_contract.rs::t9_route_ownership_cross_walk`) reconciles METHOD_NAMES ↔ S3 dispatch ↔ S0 dispatch every CI run; drift in any direction fails the build. Evidence: `plan.runs/13-t2-s3-dispatch-extraction-evidence.md`, `13-t3-s3-runtime-extraction-evidence.md`, `13-t4-spacetimedb-bridge-evidence.md`, `13-t9-cross-layer-guardrails-evidence.md`.
...
### IOD-08 - Graphiti runtime boundary — **RESOLVED (Binding)**

- **Status (as of 2026-06-02 — Track 13 closure):** Resolved by Track 13 T7. `Body/S/S0/epi-cli/src/gate/graphiti.rs` is a 5-line `pub use epi_s3_graphiti_runtime::{...}` re-export — S3 runtime owns the deposit/search substrate. S5 owns governance via per-agent capability envelope at `EpiiAgentAccess::deposit` + `DisclosureLevel` gating (epii → `Chunk`, others → `SourceSummary`). S2 owns storage substrate (Gnosis config reads `EPILOGOS_NEO4J_URI`; documents/notebooks live under `~/.epi-logos/gnosis/`). S0 emits the canonical boundary triple (`storage_substrate: "S2"`, `governance_owner: "S5'"`, `runtime_authority: epi_s3_gateway_contract::GRAPHITI_RUNTIME_AUTHORITY`) on every `gnosis_context_retrieve` / `gnosis_status` / `world_return_status` response. Evidence: `plan.runs/13-t7-s5-adapter-evidence.md`.
...
### IOD-09 - S5 state storage and `ReviewSource` metadata — **RESOLVED (Binding)**

- **Status (as of 2026-06-02 — Track 13 closure):** Resolved by Track 13 T7. Canonical S5 gate-root layout pinned at `<state_root>/s5/{epii-review,epii-autoresearch}` via the `STORE_SUBPATH: [&str; 2]` constants + `pub fn review_store_path(state_root)` + `pub fn improvement_store_path(state_root)` helpers in `Body/S/S0/epi-cli/src/gate/{review,improve}.rs`. The internal `store(...)` helpers now delegate to these canonical helpers (improve's `review_store` delegates to `super::review::review_store_path` so both modules share a single source of truth). Two store-location tests pin the layout: `gate_review_store_subpath_is_stable_at_s0_s5_boundary` and `gate_improve_store_subpath_is_stable_at_s0_s5_boundary` (`Body/S/S0/epi-cli/tests/gate_epii_{review,improve}.rs`). The recommended-default JSON-backed first production slice is honoured; embedded-storage migration seam preserved for when route/orchestration scale demands it. Backward compatibility with existing `~/.epi/gate` / `EPI_GATE_STATE_ROOT` layouts preserved by 13.T3 sessions/chat extraction (state-root layout unchanged). Evidence: `plan.runs/13-t7-s5-adapter-evidence.md`.
...
### IOD-17 - `capability-matrix.json` as canonical agent-tool governance authority — **RESOLVED (Binding spec-time); runtime-parity follow-up next-tranche**

- **Status (as of 2026-06-02 — Track 13 closure):** Spec-time parity remains binding per the 2026-06-01 status below. Track 13 T6 surfaced the runtime-parity follow-up explicitly: 6 S0-mirrored S4 constants in `Body/S/S0/epi-cli/src/gate/anima.rs` (`MOIRAI_HOST_CF`, `DISPATCH_TOOLS`, `route_outcome`, `route_agent_for_outcome`, `authority().forbidden`, `constitutional_agents()`) each annotated `// S4_AUTHORITY:` and stamped with `S4_AUTHORITY_ORIGIN = "Body/S/S4 (capability-matrix.json + ta-onta/S4-4p-anima)"` on every returned payload so callers verify S0 is mirroring S4, not acting as a second authority. Runtime-parity collapse to a single `s4'.mediation.capabilities.list` fetch is catalogued as **FU-T6-A..E** in [`plan.runs/13-t10-modularity-report.md`](./plan.runs/13-t10-modularity-report.md) §5. Mediation route rejection test added: `s0_mediation_adapter_rejects_dispatch_tools_without_upstream_vak_evidence` (`Body/S/S0/epi-cli/tests/gate_anima_pleroma_access.rs`) — S0 enforces `dispatch_tools[*].upstream_required: ["vak-evaluate"]` at the gateway edge.
...
- **Status (as of 2026-06-02 — Track 13 closure):** Track 03 T6.5 confirmed landed by Track 13 T8. `s1'.semantic.suggest_links` is classified at `Body/S/S3/gateway/src/dispatch.rs:174-192` to `S1HenGateway` / `SemanticSurface` and implemented at `Body/S/S0/epi-cli/src/gate/s1_hen.rs`. Deferred surfaces `s1'.semantic.{neighbors_of, search, by_block}` are catalogued as **FU-T8-A** in [`plan.runs/13-t10-modularity-report.md`](./plan.runs/13-t10-modularity-report.md) §5 (named, not yet landed). T8 binding test `t8_governed_path_dispatch_method_names_match_dispatch_classifier` proves the audit's claimed canonical surface round-trips through `epi_s3_gateway::dispatch::classify_method` — if Track 03 T6.5 regresses or the audit goes stale, the test fails the build.
...
- **Status (as of 2026-06-02 — Track 13 closure):** Track 03 T6.5 confirmed landed by Track 13 T8. `s1'.vault.{read_file, write_file, rename_file, move_file}` all classified at `Body/S/S3/gateway/src/dispatch.rs:174-192` to `S1HenGateway` / `VaultGateway` and implemented at `Body/S/S0/epi-cli/src/gate/s1_hen.rs`. Wikilink-integrity fixture test (`Body/S/S0/epi-cli/tests/vault_hen_boundary_audit.rs::t8_governed_rename_leaves_no_orphan_wikilinks`) proves end-to-end that rename of `Source.md → Origin.md` reconciles 5 inbound references across plain, alias, heading-anchor, and double-ref forms — zero `[[Source]]`, `[[Source|...]]`, `[[Source#...]]` occurrences remain anywhere in the fixture vault; sibling-coordinate references untouched (byte-identical control note). T8 audit of S0 CLI vault paths (14 helpers): 6 classified `deprecated-local` with named gateway replacement (2 landed: `Create`→`write_file`, `Move`→`move_file`; 4 deferred); 7 `governed-local` (template scaffolding, no inbound wikilinks); 1 `out-of-scope` (env-config write). Deferred surfaces `s1'.vault.{update_frontmatter, append_block, delete_file, move_directory}` catalogued as **FU-T8-A** in [`plan.runs/13-t10-modularity-report.md`](./plan.runs/13-t10-modularity-report.md) §5. `archive_day` carries an explicit `TODO(IOD-19)` annotation in `Body/S/S0/epi-cli/src/vault/mod.rs` awaiting `s1'.vault.move_directory`.
