...
- Build the Electron desktop distribution as canonical and full-fidelity, with the standard Theia browser bundle retained as a gateway-mediated derived target for CI, optional Docker/headless, hosted access, phone/secondary-device access, and other stepped-down remote use.
...
- **Electron-First Target Discipline:** `electron-app` is the canonical full-fidelity development, smoke-build, and acceptance target. It must carry every Pratibimba surface package required for the full 0/1 + 4+2 product. `theia-app` browser mode is derived from the same extension/runtime source and is allowed to step down capability through gateway mediation, permission gates, and readiness labels; it must never be the only target that carries a surface package or the place where full-surface behavior is first validated.
...
2. **T1 - Theia Application Skeleton And Version Pin.**
   ...
   - Configure dual build targets in the same tree: an Electron-app target (canonical desktop distribution and primary dev/acceptance surface) built via `@theia/electron` + `electron-builder`, and a browser-app target (gateway-derived CI / Docker / optional headless / hosted remote access) producing the standard Theia browser bundle.
   ...
   - The Electron build target produces a runnable Electron application bundle via `electron-builder` (at least the unpacked directory; signed installers are out of scope for T1) and carries the full surface-package set.
   - The browser build target produces a real Theia browser bundle deployable via the Theia browser server, with any stepped-down capability state represented as gateway/readiness policy rather than missing product grammar.
   - The Theia app starts in development (Electron first, browser second) and activates the readiness contribution through Theia's real DI/contribution system.
...
10. **T9 - Full Theia-Shell Acceptance, Release Gate, And Replan.**
    ...
    - Optionally run the same acceptance against the browser-mode build to confirm gateway-derived CI / Docker / hosted remote parity, with explicit assertions for any stepped-down capabilities.
