...
4. **1.4 — Kernel-bridge profile-contract readiness ledger entries for M0** *(code-pending-closure; cross-link to Tranche 10)*
   ...
   Readiness note (2026-07-07, supersedes location above): the two `MathemeBinaryProjection` pending literals have relocated out of `kernel.rs` into the extracted projections submodule — `dataset_lut_state = "pending-dataset-lut"` at `Body/S/S0/portal-core/src/kernel/projections/binary.rs:64` and `m3_codec_provenance = "…tarot/amino LUTs pending"` at `binary.rs:72`. Both remain CODE-PENDING and stay assigned to Tranche 10; the Verify-line grep is scoped to `kernel.rs` and no longer catches them (closure attribution unchanged — only the host file moved). `kleinFlipState` is no longer absent: it landed as `KernelTemporalProjection.klein_flip_state: Valence` (`kernel.rs:716`, wired `:727-739`) via Tranche 03.T3.1, so only the dataset/codec literals remain for Tranche 10 to close. `cargo check -p portal-core` is green on the 2026-07-07 tree.
