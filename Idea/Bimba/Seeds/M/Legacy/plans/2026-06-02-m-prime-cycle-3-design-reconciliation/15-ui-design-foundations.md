...
3. **15.3 — Left-sidebar activity-bar system** *(spec-ahead-integration)*
   ...
   > **⚑ FATE-SORTED (2026-07-12, DR-FACE-7): spine-gap — the typed mode-registry spine built on the rerun carrier.** Sort rationale: not carried-by-integration (no left-sidebar mode inventory existed on the carrier — the face-1 left border is flat flexlayout tabs Vault/Journal/Calendar/Oracle, no typed activity-bar grammar); not face-gap (this close mounts NO pane body — the three named surfaces are either already live or 28.x-owned pending). What LANDS is the spine grammar: `Body/M/pratibimba-app/src/ui/leftSidebarModes.ts` — a typed module declaring the mode inventory `{id, label, surfaceId (CHROME-CONTRACT §2 key), availableInLayouts, pendingOwner}` with EXACTLY three modes for `daily-0-1` (Coordinate Tree → `coordinateTree` · Bimba Graph Viewer → `bimbaGraph` · Canon Studio → `editor`) and five for `ide-deep` (+ Backend Studio → `backendStudio` · Smart Connections → `smartConnections`); plus the mode-switch sub-record `useLeftSidebarModeStore` (module-scope zustand singleton — the `mode` field of 15.7's `BimbaPratibimbaUiState`) that survives the ⌘. 0/1 face toggle by construction (the face flip re-renders the shell, never dispatches to the store, exactly as the coordinate/session singletons behave) and preserves the active mode across the `daily-0-1` ↔ `ide-deep` switch where the mode exists in both, else falling back to the Coordinate Tree backbone; plus `registerLeftSidebarModeCommands` (the "switching rides commands/registry" path — ide-deep-only mode commands grey out in `daily-0-1`). **Proof:** `src/ui/leftSidebarModes.test.ts` (16 tests — exact-per-layout inventory, face-toggle survival at the render surface, cross-layout persistence + honest fallback, and the CHROME-CONTRACT §2 consistency duty: every mode surface id is a real §2 row and its live/pending status agrees with §2, same parser the chrome-contract validator uses) + `pnpm build`; the chrome-contract validator stays green (this module adds NO factory cases and NO §2 rows). **Stays 28.x-owned, honestly named:** the pane bodies — `coordinateTree` (28.6), `backendStudio` (28.13), `smartConnections` (28.12) still render honest pending markers; the activity-bar's actual mounting into `App.tsx` (28.2 first-build allowance) is unbuilt — the registry is the consumption contract those tranches take, not a mounted sidebar. `App.tsx`, `src/state/stores.ts`, and `CHROME-CONTRACT.md` were NOT edited; wiring `registerLeftSidebarModeCommands` beside the other `register*Commands` calls is a one-line controller diff.
...
5. **15.5 — 0/1 toggle gesture + lemniscate transition** *(no-orphan-fill; new affordance)*
   ...
   > **RERUN AUDIT (2026-07-16, 15.T15.5): PARTIAL — retain in review.** The active [[pratibimba-app]] carrier has a real `Meta+.` / `Ctrl+.` command-registry route (`App.tsx`) and the spawned-gateway Chromium path proves both face directions plus cross-face session identity; the focused carrier suite also proves the six-field composition spine round-trips through a real filesystem port. The tranche is not complete: no title-bar coin-flip affordance is mounted; `LEMNISCATE_MASK_LAW` and `TRANSITIONS.lemniscate01` are declaration/test-only and the rendered transition is an opacity + scale/rotate crossfade using CSS `ease`, not the ratified cubic-out lemniscate mask; and no one toggle-driven behavioral test primes and observes `(coordinate, lens, mode, profileGeneration, sessionKey, dayNow)` together. Current tests split the claim across coordinate-only `App.test.tsx`, a synthetic face-probe mode test, composition-provider persistence, and session-only Chromium continuity. Audit proof: `pnpm vitest run src/App.test.tsx src/ui/primitives.test.tsx src/ui/leftSidebarModes.test.ts src/composition/compositionState.test.tsx` (29/29) and `pnpm playwright test tests/e2e/session-continuity.spec.ts tests/e2e/visual-regression.spec.ts --output=/tmp/codex-15-toggle-playwright` against the real spawned gateway (6/6).
