...
1. **22.1 — Spanda walk navigator widget** *(code-pending-consumer / spec-ahead; consumes 15.6 / 15.9 / 02.5)*
   ...
   > **⚑ TRANSPORT RE-DERIVED (2026-07-11, DR-M1-5)** — `bridge.requestScrubToTick` conflated two transports and is superseded by [[M1-3-SPANDA-TRANSPORT-ARCHITECTURE]]: **display-scrub** (face-local replay of received records — already landed in the carrier's modulation engine) vs **engine-walk** (kernel-owned `SpandaPhaseAnchor` mutation via the `m1.spanda.*` walk family — SPANDA-T1..T3). `slerpFraction` dissolves into local evaluation of the shared anchor. The navigator face builds per that doc's §5; this section's pedagogy contract, privacy class, and tick-projection content remain law.
