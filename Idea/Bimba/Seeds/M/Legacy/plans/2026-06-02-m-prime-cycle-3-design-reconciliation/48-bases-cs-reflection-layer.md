...
The `.base` is the Obsidian-side surface; the same coordinate-keyed aggregation must also surface in
the **epi-theia** app. Two modes, both keyed on coordinate:

- **Static** — a base view exported to a pre-computed snapshot (coordinate + columns + rows) and
  rendered as an M-stack panel (table/cards/image-grid). No live query; cheap, deterministic,
  good for crystallised `/World` and `/Map` views. The export is a C5 reflection artifact.
- **Dynamic** — the panel issues a **live coordinate-keyed query** against the Bimba Map data layer
  (the S2 graph services / `bimba-mcp` graph surface reached over the gateway), re-rendering as the
  graph/vault changes. Good for the `Empty/Present` present-console and any view that must stay
  fresh.

**The Bimba Map extension is the natural data layer.** Because the coordinate is the universal join
key (§6), the same extension that renders the M0–M5 map can serve coordinate-keyed *aggregated*
rows to a base-view panel — the map projection and the base view are two reads of one coordinate
index. An M-stack "bases-view" capability must obtain its data **through the gateway / Bimba Map
data layer, not by importing S-stack crates directly**, per the S↔M forbidden-imports matrix
(`Body/M/epi-theia/extensions/contracts/07-t0-extension-contract-preflight.json`, Track 43.5).

> **Investigation note (in-flight):** concrete Theia view/render patterns (the M4-Nara widget
> pattern, `CosmicClockRenderService`, the existing gateway RPC seams, and exactly which service the
> Bimba Map extension exposes) are being mapped by a subagent; §7 + Tranche 48.6 are refined from
> that survey before L2 work begins.
...
| **Theia static+dynamic surfacing + coordinate-join data layer** (L2) | **M-stack epi-theia + Bimba Map ext** | `Body/M/epi-theia/extensions/**` over the gateway / S2 graph services |
...
48.6 are the L2 / code-reality follow-ons**, named and flagged to their owning specs.

### 48.1 — `.base` artifact class + the CS/Pratibimba framing law *(L1; canon/contract + bimba-vault-validate)*
...
### 48.2 — The CT-ladder base set (one CS-reflection per content type) *(L1; depends 48.1)*
...
### 48.3 — Per-zone bases *(L1; depends 48.1)*
...
### 48.4 — MOC↔Base collaboration (embed + feed living sections) *(L1; depends 48.1–48.3)*
...
### 48.5 — `s1'.base.ensure` + base-schema-from-CTx-contract *(L2; owning spec [[S1-SPEC]] / Hen)*
...
### 48.6 — Theia static + dynamic surfacing, backed by the Bimba Map data layer *(L2; M-stack; depends 48.2–48.3; refined from the §7 subagent survey)*

Surface coordinate-keyed aggregated base views in epi-theia: a **static** snapshot panel
(pre-computed rows) and a **dynamic** panel issuing a live coordinate-keyed query through the
gateway / Bimba Map data layer, re-rendering on change. Reuse the existing M-stack widget/render
pattern; obtain data **over the gateway, never by importing S-stack crates** (forbidden-imports,
Track 43.5). The Bimba Map extension serves as the coordinate-keyed data layer (the map projection
and the base view are two reads of one coordinate index).

**Implementation scope:** an M-stack "bases-view" render capability (static + dynamic) + a
coordinate-keyed aggregate read on the Bimba Map data layer over the gateway; contract-preflight
clean.

**Verification:** a coordinate-scoped aggregated view renders in Theia both statically (snapshot)
and dynamically (live, updating on a vault/graph change); the extension passes
`07-t0-extension-contract-preflight.json` (no forbidden S-stack import).
