...
3. **26.3 — Logos Atelier scent-following retrofit (audit + extend)** *(audit-extend + no-orphan-fill; sources WC-M5-3, SA-WC-M5-1; gates 6.2; CR 12.1)*
   ...
   > **CORRECTION (26.T26.3, ratified 2026-07-23) — agents use capabilities, they don't duplicate them.** The `aletheia_*` names above are Aletheia/Sophia **agent tools** routed through `s4'.mediation.route` under entitlement (see `12-agentic-layer-s4-s5.md` — "add all gnostic / Aletheia / GraphRAG tools to the canonical skill universe"), **not** bespoke gateway methods a pane invokes. A pane does not call `aletheia_*`; it uses the same underlying **capability** directly: root → `s5'.gnostic.etymology` (takes a `coord`), drift → `s5'.gnostic.query_with_layers`, cognate → `s1'.semantic.suggest_links`, psychoid → `s0'.anuttara.trace`, Möbius write-back → `s1'.entity.capture` (Hen candidate). `s5'.gnostic.*` is **LANDED** (6.1), so there is no "pending-gateway until 6.1" state — gnostic renders live; readiness follows the real nine-state bridge taxonomy. `pros-hen` is a **local** synthesis stage (Klein-V4 pull, no substrate call). The Aletheia subagents (below) surface only as **evidence lineage** — the agents that *use* these capabilities, shown as provenance, never a pane invocation. Realised in the carrier as `src/commands/atelier.ts` (`SCENT_FOLLOWING_STAGES` + six commands), not a widget.
