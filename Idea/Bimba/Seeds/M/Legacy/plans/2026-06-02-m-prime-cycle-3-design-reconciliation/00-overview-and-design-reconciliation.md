...
3. **Per-domain reconciliation (Tranches 01-06 + Track 36.3 / 36.5 / 36.6)** — M0..M5, each in parallel where possible, blocked only by decisions and profile fields they depend on. M3 owns the Maxwell/Mahamaya 15 inspector, M4 consumes protected Q handles, and M5 consumes the trace as a real EBM feature family.
...
6. **No-orphan audit + release gates (Tranche 14 + Track 36.7)** — fail if any canonical surface, carrier, agent, plugin, profile-field, or doc-claim lacks an owner or a decision, including `0/1->5`, paired Mahamaya 15s, `24*15=360`, and `AnuttaraPentadicRuntimeTrace`.
