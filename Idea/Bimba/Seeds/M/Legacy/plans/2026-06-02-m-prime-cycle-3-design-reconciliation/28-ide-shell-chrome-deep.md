...
**(SC-I-6) SharedBridgeAdapter is the only network surface.** Every gateway call routes through `KERNEL_BRIDGE_API.invokeCapability` with `method: 'invokeGatewayRpc'` + inner `gatewayMethod` (`s2.graph.node`, `s2'.coordinate.resolve`, `s5'.review.inbox`, `s5'.improve.history`, `aletheia_gnosis_query`, `aletheia_crystallise`, `vault-bridge.s1prime.vault.write_file`, etc.). [CORRECTION — see 28.7: the two `aletheia_*` entries are **agent tools** via `s4'.mediation.route`, not pane `gatewayMethod`s; the Logos Atelier pane binds the underlying capabilities `s5'.gnostic.*` / `s1'.semantic.suggest_links` / `s1'.entity.capture` directly.] NO direct S0/S2/S3/S5 imports anywhere in `Body/M/epi-theia/extensions/ide-shell-m0-m5/src/`. The forbidden-direct-import lint (11.5) enforces.
...
7. **28.7 — Logos Atelier: scent-following retrofit + Aletheia tools + Möbius write-back to Canon Studio** *(audit-extend; consumes 26.3 + 21-m0 SC-2 + 12.19 + 26.9 + 26.14)*
   ...
   > **CORRECTION (mirrors 26.T26.3, ratified 2026-07-23) — agents use capabilities, they don't duplicate them.** The `aletheia_*` names in this tranche (`aletheia_gnosis_query`, `aletheia_thought_route`, `aletheia_crystallise`) are Aletheia/Sophia **agent tools** routed through `s4'.mediation.route` under entitlement (see `12-agentic-layer-s4-s5.md` (a) at line 757 — "add all gnostic / Aletheia / GraphRAG tools to the canonical skill universe"), **not** bespoke `gatewayMethod`s a pane invokes. A pane does not call `aletheia_*`; it uses the same underlying **capability** directly: root → `s5'.gnostic.etymology` (takes a `coord`), drift → `s5'.gnostic.query_with_layers`, cognate → `s1'.semantic.suggest_links`, psychoid → `s0'.anuttara.trace`, Möbius write-back → `s1'.entity.capture` (Hen candidate). This voids: sub-item (b)'s three `gatewayMethod: 'aletheia_*'` bindings (read the capabilities above); the "Until `s5'.gnostic.*` lands at 6.1 … pending-gateway" gate at (b)/line 261 — `s5'.gnostic.*` is **LANDED** at 6.1, so gnostic renders live and readiness follows the real nine-state bridge taxonomy, never a `pending-gateway` placeholder; the SC-I-6 "only network surface" list entry (line 56) and the 28.19 (d) widget-binding-table entry (line 373) for `logos-atelier`; and the `aletheia_crystallise` mechanism named in SC-I-2 (line 48) + sub-item (d.1) (→ `s1'.entity.capture`). `pros-hen` is a **local** synthesis stage (Klein-V4 pull, no substrate call). The Aletheia subagents (sub-item (e)) surface only as **evidence lineage** — the agents that *use* these capabilities, shown as provenance, never a pane invocation. Realised in the carrier as `src/commands/atelier.ts` (`SCENT_FOLLOWING_STAGES` + six commands) per 26.T26.3, not `logos-atelier-widget.tsx`.
