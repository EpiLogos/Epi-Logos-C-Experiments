...
- **9 / `−1/9` / 72→64.** `m3.h:339` documents the ratio (`9:8 ratio. 72 × (8/9) = 64. Integer-only. O(1).`); `m3.h:350-353` implements `apply_epogdoon_compression(m2_idx) = (m2_idx * 8u) / 9u`; `m3.h:356-360` implements `is_evolutionary_gap` (`expanded != m2_vibration_index`). The mirror lives in M2: `m2.h:527` (`72 × (8/9) = 64: the epogdoon compression`) + `m2.h:544-547` `m2_epogdoon_compress(val_72) = (val_72 * 8u) / 9u`. The M2 indices that do **not** round-trip through compress→expand number **64** (the C `is_evolutionary_gap` authority, verified for all 72 in portal-core `kernel::epogdoon_bridge_tests` + epi-cli `kernel_bridge_epogdoon_projection`); only the **8** multiples of nine (0,9,18,27,36,45,54,63) round-trip cleanly, and those 8 collisions ARE the structural "missing states" (cf. the 8 `M3_RES_MATRIX` gap sentinels, 64→56). The archetypal **9** is the epogdoon denominator / Paramesvara ground, **not** a fold-point cardinality — corrected from the earlier "nine fold-points" gloss; see DR-37-6.
...
- **DR-37-6 (NEW, 2026-07-12) — epogdoon fold-point count: "exactly 9" is a spec error; the C authority is 64 round-trip failures / 8 structural collisions.** The §1 table's "9 evolutionary fold-points" gloss and the 37.1 verify-clause "exactly 9 indices report `isEvolutionaryGap`" contradict the authoritative C `is_evolutionary_gap` law (`m3.c:162-166`): over the 72 M2 addresses, `expanded != i` holds for **64** of them; only the **8** multiples of nine (0,9,18,27,36,45,54,63) round-trip cleanly through the 9:8 compress→expand. The structurally meaningful "missing-states" cardinality is **8** — the 72→64 collision count, matching the 8 `M3_RES_MATRIX` gap sentinels (64→56). The **9** is the epogdoon *denominator* / Paramesvara archetype, never a fold-point count. **Resolution:** the kernel-bridge projection surfaces the C law verbatim (architecture §7 — the renderer never recomputes biology); the spec's "9" is corrected to "64 round-trip failures / 8 structural collisions". Pinned behaviorally by `bridge_edge_gap_count_is_c_authoritative_64_not_9` (epi-cli) + `epogdoon_fold_structure_matches_canonical_c` (portal-core). **Follow-on (out of 37.T37.1 write-scope, flagged for the owning M3/Track-37 spec):** the `EPOGDOON_FOLD_POINT_COUNT = 9` doc-parity constant and the `is_evolutionary_gap` field doc-comment ("True at the nine fold-points") in `Body/S/S0/portal-core/src/kernel.rs` should be corrected to 64/8, and the Theia/pratibimba-app `EpogdoonBridgeEngine.EPOGDOON_FOLD_POINT_COUNT` re-derived from the live C count rather than a hardcoded 9.
...
1. **37.1 — Ratify the epogdoon-bridge projection** `kernelBridge.m2.epogdoonProjection(address72) → { compressedCodon, isEvolutionaryGap, expandedBack }` (consumed by Track 23.18). Kernel runs `apply_epogdoon_compression` + `is_evolutionary_gap` (`m3.h:280`, `m3.h:283`; impl `m3.c:158-166`). Add to the Track 10 / Track 18 bridge contract. **Verify:** projection round-trips against the C functions for all 72 indices; ~~exactly 9 indices report `isEvolutionaryGap`~~ → **64 indices report `isEvolutionaryGap`** (C authority; see the verify-clause correction below and DR-37-6).

   **LANDED (37.T37.1, cycle-3 rerun, 2026-07-12).** The projection is ratified in `portal-core` (`EpogdoonBridgeProjection::from_address72` + `epogdoon_bridge_lattice`, `Body/S/S0/portal-core/src/kernel.rs`) and surfaced at the bridge edge in `epi-cli` (`m2_epogdoon_projection` / `typed_json_m2_epogdoon_projection` / `typed_json_m2_epogdoon_lattice`, contract id `KERNEL_BRIDGE_M2_EPOGDOON_PROJECTION = "kernelBridge.m2.epogdoonProjection(address72)"`, `Body/S/S0/epi-cli/src/gate/kernel_bridge_runtime.rs`). It runs the C epogdoon law verbatim through FFI (`apply_epogdoon_compression` / `is_evolutionary_gap` / `m3_epogdoon_expand`) and never recomputes the fold in Rust (architecture §7). Round-trip verified against the C functions for **all 72** indices by `bridge_edge_round_trips_against_c_for_all_72_indices` (epi-cli) + `projection_round_trips_against_c_for_all_72_indices` (portal-core). **Verify-clause correction:** the "exactly 9" criterion was a spec error — the C `is_evolutionary_gap` law reports **64** round-trip failures over 0..71; only the **8** multiples of nine round-trip cleanly, and 8 (= the `M3_RES_MATRIX` gap sentinels) is the structural "missing-states" count. Pinned by `bridge_edge_gap_count_is_c_authoritative_64_not_9` (epi-cli) + `epogdoon_fold_structure_matches_canonical_c` (portal-core). See DR-37-6.
...
2. **37.2 — Ratify the planetary-elemental projection** `kernelBridge.m2.planetaryElementalWeights() → { weights{earth,fire,water,air}, perPlanet[], aspectGain[] }` (consumed by Track 23.19). Kernel sums `M2_PLANET_LUT[id].elem_sig` elements (Sun excluded) weighted by Keplerian velocity + `compute_aspects` (`aspect.rs:9`). Add to bridge contract. **Verify:** the four-element vector matches a kernel-side reference for a fixed kairos; aspect amplification matches `aspect.rs` output.
   ...
   > **LANDED — rerun-verified 2026-07-13 (37.T37.2).** The projection is ratified in `portal-core` and surfaced at the bridge edge in `epi-cli`, sibling to the epogdoon (37.1), cymatic (37.3) and bioquaternion (37.11) edges. **Substrate home** = [`portal-core/src/aspect.rs`](../../../../../Body/S/S0/portal-core/src/aspect.rs): `planetary_elemental_weights(state) -> PlanetaryElementalWeights` @105 (struct @92 = `{ weights, perPlanet, aspectGain }`). The nine orbiters each project one Mahabhuta element via `PLANET_ELEMENT_ID[10]` @33 (mirrors `ELEM_SIG_GET_ELEMENT(M2_PLANET_LUT[i].elem_sig)`), the **Sun (0) is the excluded identity root** (the 9:8 epogdoon asymmetry — nine orbiters : eight chakras), each contribution is weighted by `PLANET_KEPLERIAN_VEL[10]` @48 (`M2_PLANET_LUT[*].keplerian_vel`), and the aspect amplification reuses `compute_aspects` @237 verbatim — it never reimplements the aspect math (architecture §7). AKASHA/aether (Uranus) is reported per-planet but kept out of the four-element bar (Archetype-5 / quintessence, per the §-head dev_decision). **Bridge edge** = [`epi-cli/src/gate/kernel_bridge_runtime.rs`](../../../../../Body/S/S0/epi-cli/src/gate/kernel_bridge_runtime.rs): `typed_json_m2_planetary_elemental_weights(state)` @907, contract id `KERNEL_BRIDGE_M2_PLANETARY_ELEMENTAL_WEIGHTS = "kernelBridge.m2.planetaryElementalWeights()"` @34-35, registered in `capability_names()` @747 and gateway-method-resolved @1484 (Add to bridge contract). **Verify clauses proven:** (a) *four-element vector matches a kernel-side reference for a fixed kairos* — `elemental_weights_use_keplerian_reference_for_fixed_kairos` (portal-core, direct Keplerian reference) + the fresh bridge-edge `kernel_bridge_surfaces_m2_planetary_elemental_weights_for_fixed_kairos` (epi-cli) which re-derives `water = (47270+3600+½·mean)/total` through the typed-JSON surface; (b) *aspect amplification matches `aspect.rs` output* — `exact_shared_element_aspect_amplifies_weight_vector_from_compute_aspects` + `aspect_gain_signs_track_harmony` (portal-core), and the bridge-edge test asserts `aspectGain` equals `compute_aspects` over the same kairos handle-for-handle; (c) *Sun excluded / four-element normalisation* — `elemental_weights_normalise_over_four_elements` (9 orbiter contributions, `sum ≈ 1.0`, Uranus→aether out of the bar) + the bridge-edge `perPlanet` all-`planetId != 0` assertion. **Behavioral proof (fresh, this session):** `portal-core` `aspect::` 10/10 green; `epi-cli` `kernel_bridge_runtime_contract` 22/22 green (incl. the new bridge-edge test). Runners: `cargo test -p portal-core` (workspace member) + `cargo test --manifest-path Body/S/S0/epi-cli/Cargo.toml` (epi-cli is workspace-excluded, package `epi-logos`, so `-p epi-cli` never matches). **Field-order note (benign):** the design signature writes `weights{earth,fire,water,air}` for illustration; the serialized `ElementalWeightProjection` (camelCase) is `{ fire, water, air, earth }` — same four elements, no earth-first requirement. No new DR.
...
10. **37.10 — Element-ID converters + stale-spec flag (closes DR-37-3 / DR-37-5).** Extend the bridge normalization (37.6) with the two missing converters: `canonical_from_m3_decan_element` (scheme D `Clock_Degree_Entry.decan_element` Fire0/Earth1/Air2/Water3/Akasha4 → canonical-B) and `canonical_from_nucleotide` (scheme C name-binding A=Water/T=Fire/C=Earth/G=Air → canonical-B by name). Alias the existing `canonical_from_medicine_rs_legacy` ([medicine_frame.rs:770-779](../../../../../Body/S/S0/epi-cli/src/nara/medicine_frame.rs)) → `canonical_from_m2_tattva` for honesty (it converts the m2.h tattva enum, not a "legacy" scheme). Flag the **stale clock-spec §15.3 nucleotide-element table** ([cosmic-clock-spec:1665-1673](../specs/M/2026-03-12-cosmic-clock-full-architecture.md)) for in-place correction — it asserts A=Fire/T=Earth/C=Air/G=Water, contradicting the authoritative Golden-Dawn/Thoth code binding ([m3.h:70-73](../../../../../Body/S/S0/epi-lib/include/m3.h)). **Verify:** a single canonical element-ID enum is asserted in Track 18; the three converters round-trip to canonical-B; `grep` finds no frontend/projection mapping a raw element integer across the M2↔M3 boundary; the clock-spec §15.3 correction (or a `superseded-by-code` note) lands.
   ...
   > **LANDED — rerun-verified 2026-07-12 (37.T37.10).** All three converters live in [`medicine_frame.rs`](../../../../../Body/S/S0/epi-cli/src/nara/medicine_frame.rs) (current lines, citations above drifted from `:770-779`): `canonical_from_m2_tattva` @791 (scheme A→B), its honest-name situation resolved — `canonical_from_medicine_rs_legacy` @806 is now a thin alias delegating to it, keeping its three `medicine_cast.rs` callers green; `canonical_from_m3_decan_element` @827 (scheme D→B); `canonical_from_nucleotide` @844 (scheme C→B, A=Water/T=Fire/C=Earth/G=Air, mirrors `m4_nuc_to_elem` in [`m_canonical.h`](../../../../../Body/S/S0/epi-lib/include/m_canonical.h) from 5.16). The stale scheme-E table is flagged in place — `STALE_SPEC_FLAG (DR-37-3 / DR-37-5, Track 37.10)` at [cosmic-clock-spec ~1699-1731](../specs/M/2026-03-12-cosmic-clock-full-architecture.md) (HTML comment + blockquote `superseded-by-code` note). Behavioral proof: 14/14 `nara::medicine_frame::canonical_tests` green — all three converters round-trip to canonical-B, the alias is asserted byte-identical to the primary over the whole domain, and every converter's output is proven a member of the single canonical-B enum (0..=5 or the 0xFF sentinel), which is the Track-18 single-enum invariant asserted locally. `grep` finds no frontend/portal-core file mapping a raw element integer across the M2↔M3 boundary. **DR-37-3 / DR-37-5 are track-local to §9 of this doc (not `13-decision-register.md`) — CLOSED here.** Runner: `cargo test --manifest-path Body/S/S0/epi-cli/Cargo.toml` (the crate is workspace-excluded and its package name is `epi-logos`, so `-p epi-cli` never matches).
...
11. **37.11 — The bioquaternion transcription totality (one transcribed object, not scattered functions).** The bioquaternions across the stack are mutual transcriptions/translations — the conversions already exist in code but are not declared as one coherent, tested bridge (findings §III.6). This tranche makes the totality explicit. **(a) Declare the canonical codon→quaternion path:** the four-charge `m3_eval_to_quat` `{w=pp→Earth, x=nn→Fire, y=np→Water, z=pn→Air}` ([m3.h:490-497](../../../../../Body/S/S0/epi-lib/include/m3.h) + element map [m1.h:440-450](../../../../../Body/S/S0/epi-lib/include/m1.h)) is canonical for the **elemental** quaternion; `m3_quat_from_codon` ([m3.h:217-232](../../../../../Body/S/S0/epi-lib/include/m3.h)) is the ring-position shortcut — name which is which so renderers never pick the wrong one (closes the two-path note in §III.6). **(b) Assert the unifying identity:** the four codon charges `pp/nn/np/pn` = the four X# permutations `X2/X1/X4/X3` (§III.2) = the four elements Earth/Fire/Water/Air — one fact, three registers. **(c) Round-trip the transcription:** `m3_eval_to_quat ∘ m3_quat_to_eval == id` ([m3.h:490-505](../../../../../Body/S/S0/epi-lib/include/m3.h)); `m3_tarot_translate` codon↔hexagram invertible ([m3.h:834-850](../../../../../Body/S/S0/epi-lib/include/m3.h)); base-pair `XOR 0x01` = the `R#`↔`##` (`0↔1`) flip ([m3.h:85-88](../../../../../Body/S/S0/epi-lib/include/m3.h)). **(d) Surface one projection** `kernelBridge.m3.bioquaternionTranscription(codon) → { charges{pp,nn,np,pn}, quaternion[4], elementsCanonical[4], aminoAcid, hexagramId, tarot, complement }` so the frontend consumes the whole transcribed object, never recomputing a conversion. **Verify:** the `charge = X# = element` identity holds for all 64 codons; `eval↔quat` and `tarot codon↔hexagram` round-trip; the canonical-vs-shortcut quaternion path is declared and grepped; no renderer recomputes any conversion.
   ...
   > **LANDED — rerun-verified 2026-07-12 (37.T37.11).** The totality is ONE transcribed object, not scattered functions. Substrate home = [`portal-core/src/m3_transcription_bridge.rs`](../../../../../Body/S/S0/portal-core/src/m3_transcription_bridge.rs): `bioquaternion_transcription(codon) -> BioquaternionTranscription` @356 assembles the whole object; the `BioquaternionTranscription` struct @249-261 carries `codon / charges{pp,nn,np,pn} / quaternion[4] / elementsCanonical[4] / chargeIdentity[4] / aminoAcid / hexagramId / tarot / complement`. **(a)** the canonical-vs-shortcut split is *declared* as first-class consts — `M3_ELEMENTAL_QUATERNION_PATH = "m3_compute_charges/evaluate_codon -> m3_eval_to_quat"` @22-23 (canonical *elemental* path) and `M3_RING_POSITION_QUATERNION_SHORTCUT = "m3_quat_from_codon"` @24 (ring-position shortcut) — both surfaced in the packet (`canonicalQuaternionPath` / `ringPositionQuaternionShortcut`) and asserted, so renderers never pick the wrong path. **(b)** the unifying identity is a materialized table — `charge_identity()` @454-481 binds `pp↔X2↔Earth↔w`, `nn↔X1↔Fire↔x`, `np↔X4↔Water↔y`, `pn↔X3↔Air↔z` (one fact, three registers + the quaternion axis). **(c)** round-trips: `evaluate_codon → m3_eval_to_quat → m3_quat_to_eval == id` and `m3_tarot_translate` codon↔hexagram invertibility both proven for all 64 codons; base-pair complement is per-base `XOR 0x01` (mask `0x15` = `0b01_01_01` across the 3 nucleotides), the `R#↔##` flip, asserted involutive. **(d)** the single projection is surfaced at the bridge edge in [`epi-cli/src/gate/kernel_bridge_runtime.rs`](../../../../../Body/S/S0/epi-cli/src/gate/kernel_bridge_runtime.rs): `typed_json_m3_bioquaternion_transcription` @952, contract id `KERNEL_BRIDGE_M3_BIOQUATERNION_TRANSCRIPTION = "kernelBridge.m3.bioquaternionTranscription(codon)"` @38-39, routed in the capability dispatcher @549-550 with a `codon space 0..63` guard. The whole object is consumed downstream (never recomputed): [`hen-compiler-core/src/birth_codon.rs:146`](../../../../../Body/S/S1/hen-compiler-core/src/birth_codon.rs) reads `bioquaternion_transcription(codon).charges` for `c_5_birth_chromosome` (CCT-14b). **Behavioral proof (fresh, this session):** portal-core inline `bioquaternion_transcription_declares_one_totality_for_all_codons` + `c_eval_quaternion_and_tarot_transcriptions_round_trip` + `m3_transcription_projection_round_trip` (4/4 green); portal-core `kernel_truth::canonical_element_identity_crosses_m2_m3_as_canonical_b` (DR-37-3 canonical-B) + `m3_compute_charges_ffi_reachable_with_4x_invariant_for_all_64_codons` (the X-logic `pp+nn+np+pn == 4·X` identity, 1+1 green); epi-cli `kernel_bridge_runtime_contract::kernel_bridge_surfaces_m3_bioquaternion_transcription_as_one_object` drives the whole object through the real `invoke_capability` runtime (contract id + codon guard, 1/1 green). Runners: `cargo test -p portal-core` (workspace member) + `cargo test --manifest-path Body/S/S0/epi-cli/Cargo.toml` (epi-cli is workspace-excluded, package `epi-logos`, so `-p epi-cli` never matches). The 37.11 substrate materialized under commit `16.T16.14` (CCT-14b birth-codon seam); this tranche verifies it against the 37.11 (a)-(d) acceptance and pins the totality. **Verify-clause note:** `tarot.roundTrip` holds for all 64 codons — genuinely through `m3_tarot_translate` for the 56 minor-arcana-primary codons, and trivially (identity fallback) for the codons that carry no minor-arcana card, which is the honest behaviour since only 56 of 64 codons are tarot-primary.
