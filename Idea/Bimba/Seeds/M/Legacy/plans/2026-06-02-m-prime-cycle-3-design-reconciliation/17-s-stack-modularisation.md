...
15. **17.15 — Split `Body/S/S5/epii-autoresearch-core/src/lib.rs` (2,049 LOC) + `epii-agent-core/src/lib.rs` (1,170 LOC)** *(code-cleanup-refactor)*
    ...
    **LANDED (rerun T17.15 — F2 only):** the four canonical F2 submodules now carry the full split along the finding axis — `types.rs` (226, pure value types) · `kernel_evidence.rs` (217, the `KernelEvidence*` types **plus** their six validators `validate_kernel_evidence`/`validate_kernel_trajectory`/`validate_public_kernel_projection`/`required_str`/`required_u64`/`parse_f64`, now self-contained — the prior `use crate::{…}` back-import into the crate root is removed) · `orchestration.rs` (199, lifecycle types **plus** the `validate_orchestration_transition` state machine) · `promotion.rs` (394, promotion types **plus** the destination/governance gate logic `validate_destination_for_run`/`validate_approved_review`/`rollback_plan_for`/`system_hen_timestamp` + the `destination_*`/`governance_*` classifiers). A prior pass had extracted only the pure type-defs (leaving kernel/orchestration/promotion logic in the façade); this reorg completes the F2 axis, so `lib.rs` shrank 1,526 → 1,126 LOC, retaining only the `ImprovementStore` façade/impl and its surfacing/proposal helpers. **Pure move refactor — no logic edits:** every public item stays re-exported from `lib.rs` (`pub use {types,kernel_evidence,orchestration,promotion}::*`); `ReviewCategory` is kept as a crate-internal `pub(crate) use` re-export so the `capacity_workflows::*` modules still resolve `crate::ReviewCategory`; downstream `epii-agent-core` builds unchanged. Full crate suite: **66 passed / 0 failed / 0 ignored — identical before/after** (`cargo test --manifest-path Body/S/S5/epii-autoresearch-core/Cargo.toml`; crate is workspace-excluded, so `-p` does not resolve). **F3 (`epii-agent-core/src/lib.rs`) is a disjoint write scope — not landed here.**
...
16. **17.16 — Collapse `classify_method` (270 LOC) + `METHOD_DISPATCH_PLAN` (940 LOC)** *(code-cleanup-refactor; DR-S3-3 VALIDATED 2026-06-03)*
    ...
    **LANDED (rerun T17.16 — verified, substrate already collapsed):** the collapse is fully present at the canonical shape in `Body/S/S3/gateway/src/dispatch.rs`. `classify_method` is a pure lookup — `method_dispatch_plan_entry(method).and_then(dispatch_route_for_plan_entry).or_else(|| extension_route(method))` — over the now-canonical `METHOD_DISPATCH_PLAN` (defined once in `epi_s3_gateway_contract::dispatch_plan`, 196 entries, one per `METHOD_NAMES` name). **No duplicate method list remains** in the gateway crate: route metadata (`owner` / `class` / `coordinate_owner` / `agent_access_owner` / `route_id`) is derived from `entry.kind` + `entry.authority_path` in `route_metadata_for_plan_entry` and its `s3_native_/s5_governance_/s1_hen_/s0_product_route_metadata` helpers, not a per-method match. The only non-plan routes are the deliberate extension surfaces (`nara.*` prefix + the three `s5'.epii.*` methods) that the contract list intentionally does not enumerate, handled by `extension_route`/`canonical_method_name`. The DR-S3-3 co-PR rule is **machine-enforced**: `tests/dispatch_contract.rs::classify_method_is_derived_from_dispatch_plan_for_every_method` asserts `dispatch_plan().len() == METHOD_NAMES.len()` and, for every plan entry, `classify_method(entry.method) == dispatch_route_for_plan_entry(entry)`; `dispatch_plan_and_route_table_agree_on_method_set` asserts zero drift in both directions. **Pure verify — no code change:** `classify_method` is LOAD-BEARING (the `nara_bounded_access` gate `t5_10`/05.T5.10, the `canon_update` SUBSTRATE classification 40.1, `oracle_spread` 5.17, and the route-ownership tests all depend on its exact per-method output), so it was left byte-identical to committed. Behaviour-preservation proof: full-classification snapshot over `METHOD_NAMES` + the 6 extension methods = **202 methods, 0 unclassified**; full gateway suite **85 passed / 0 failed / 0 ignored** (`cargo test --manifest-path Body/S/S3/gateway/Cargo.toml`), including `dispatch_contract` (27 — collapse contract + `t5_10` bounded-access + route-ownership) and `oracle_spread_contract` (3). `METHOD_DISPATCH_PLAN` itself resides in the `gateway-contract` crate (disjoint write scope) and was not touched here.
...
17. **17.17 — `coordinate_home: &'static str` → typed `CoordinateHome` enum** *(code-cleanup-refactor)*
    ...
    **LANDED (rerun T17.17 — verified + proof-gap closed):** the migration is fully present. `Body/S/S2/graph-schema/src/coordinate_home.rs` defines the `CoordinateHome` enum (74 variants — one per distinct home string, e.g. `S2_3Prime`/`C4SlashLPrime`/`S1SlashS2`/`MCompat`) with the const `as_str(self) -> &'static str` lookup as the single source of truth, and every `coordinate_home` field across all six spec tables now carries a typed variant — **177 usage sites** (`LABEL_SPECS` 21 · `RELATIONSHIP_TYPE_SPECS` 35 · `NODE_PROPERTY_SPECS` 88 · `RELATIONSHIP_PROPERTY_SPECS` 19 · `COORDINATE_PREFIX_FAMILY_SPECS` 7 · `COORDINATE_SEMANTIC_FAMILY_SPECS` 7), zero `coordinate_home: &'static str` field declarations remaining. **Behaviour-preserving — the legacy `&'static str` surface is preserved exactly** by five impls on the enum: `Serialize` (via `serialize_str(self.as_str())`), `Display`, `Deref<Target = str>`, and `PartialEq<&str>` in both directions — so every prior string comparison, format, and JSON-string emission yields the identical byte for byte value; no `coordinate_home` value changed (the migration commits `e5e32069` + `229ba117` are pure type-swaps, and the working tree matches them). **Proof:** the pre-existing spot-check `coordinate_home_is_typed_but_preserves_legacy_string_surface` (`coordinate_law.rs`) is green, and this rerun **closed the proof gap** by adding `every_spec_table_coordinate_home_round_trips_the_legacy_string_surface` — an exhaustive walk over all 177 usage sites asserting `as_str` ≡ `to_string()` ≡ `Deref<str>` ≡ `PartialEq<&str>` (both directions) and non-empty, floored at ≥150 sites. Full crate suite: **26 passed / 0 failed / 0 ignored** (was 25/0/0 before the added test — the 25 pre-existing tests pass identically) via `cargo test -p epi-s2-graph-schema`. Disjoint scope note: `tests/world_namespace.rs` and the `World`/`Gnostic` label constants belong to Tranche 9.14 (blocked on an Architect graph-write decision) and were not touched here.
