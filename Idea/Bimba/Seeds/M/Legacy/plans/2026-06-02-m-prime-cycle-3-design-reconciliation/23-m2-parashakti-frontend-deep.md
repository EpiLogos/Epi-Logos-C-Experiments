...
18. **23.18 — 72→64 epogdoon-bridge engine: M2 vibrational state read into M3 codon space** *(spec-ahead-integration; consumes m2.h `m2_epogdoon_compress` + m3.h `apply_epogdoon_compression` / `is_evolutionary_gap`; cross-link Track 24, Track 37)*
    ...
    - `kernelBridge.m2.epogdoonProjection(address72)` returns `{ compressedCodon: number /* 0..63 */, isEvolutionaryGap: boolean, expandedBack: number }` — the kernel runs `apply_epogdoon_compression` + `is_evolutionary_gap` (m3.h:351, m3.h:356-360) and ships the result. The widget consumes; it does not arithmetic.
    - The nine fold-points: `is_evolutionary_gap` is true for exactly the M2 indices that do not round-trip (m3.h:356-360 — `expanded != m2_vibration_index`). These nine non-round-tripping indices are the visible `−1/9` loss. The engine renders them as nine "fold-point" glyphs on the descent track — the Anuttara Archetype 9 (Paramesvara, `9 = (00+00)`, M0-2-9) signature made visible (cross-link Track 37 §7-8-9-spine; cross-link Track 21 M0 Paramesvara surface — the concurrent agent owns the M0-side render).

    Engine mechanics (tick-driven):
    - The active `address72` (from `MathemeHarmonicProfile`) descends through a vertical 72-cell → 64-cell collapse animation; the eight `M3_RES_MATRIX` gap sentinels (m3.c:214-231, exactly 8 × `0xFF`, asserted m3.c:767-772) render as a SECOND compression band below (`64 → 56`, the Archetype-8 `−1/8` dynamic — the doubled-cathedral `2×(##)` per Track 37). The two compression bands stack as one descent: `72 →(−1/9, 9 folds)→ 64 →(−1/8, 8 gaps)→ 56`.
    - The 56 floor renders as the `M3_TAROT_CODON_MAP[4][16]` minor-arcana lattice (m3.h:625, m3.h:632 `M3_MINOR_ARCANA_COUNT = 56`, `56 = 7 × 8`) — the Archetype-7 (Divine Action) floor. Each suit column tints by its integral (Cups=84 / Wands=96 / Pentacles=88 / Swords=92, summing to the 360 degree-wheel; m3.h:589-598).
    - On each profile-tick advance, the live `compressedCodon` cell lights in the 64-band; if `isEvolutionaryGap`, the corresponding fold-point glyph blooms warm-amber (the gap is honest, not an error — it is the evolutionary spiral driver per m3.h:340).

    Cross-composition: the engine is reads-only across the composition boundary — it consumes M3's codon projection through the kernel-bridge but never imports `m3-mahamaya` directly (that surface is Track 24's). The M3-side codon-rotation overlay onto the lens-ring (Track 24) and this M2-side descent engine are two views of the same `address72`; they share the profile-tick clock and never co-mutate.
...
19. **23.19 — Planetary-orbiter elemental-weight feed → bioquaternion / PASU `elemental_weights`** *(spec-ahead-integration; consumes m2.h `M2_PLANET_LUT[10]` + `ELEM_SIG_GET_ELEMENT`; aspect engine `Body/S/S0/portal-core/src/aspect.rs`; feeds Track 10.PASU / Track 18.10 — DO NOT edit those, feed only)*
    ...
    Substrate basis (verified): `M2_PLANET_LUT[10]` (m2.h:311) carries per-planet `Elemental_Signature elem_sig` (m2.h:303); `ELEM_SIG_GET_ELEMENT(sig)` (m2.h:100) extracts the element. The mod-10 planet ordering is canonical (m2.h:272-284; Sun=0 identity root excluded from the chakra-mapped nine; 9:8 epogdoon = 9 non-Sun planets : 8 bodily sites, m2.h:269, m2.h:314-317). The aspect computation (conjunction/sextile/square/trine/opposition, 5 Ptolemaic aspects with orbs) already exists at `aspect.rs:5` (`ASPECT_ANGLES`) + `aspect.rs:9` (`compute_aspects`) — the engine consumes its output as the aspect-weighting of the elemental feed, never reimplementing it.

    Engine mechanics (tick-driven):
    - `kernelBridge.m2.planetaryElementalWeights()` returns a typed `{ weights: { earth: number; fire: number; water: number; air: number }, perPlanet: readonly { planetId: number; element: ElementId; couEnergy: number }[], aspectGain: readonly PlanetaryAspectHandle[] }`. The kernel sums each orbiting planet's `elem_sig` element into the four-element accumulator, weighted by the planet's live Keplerian velocity (`M2_PLANET_LUT[id].keplerian_vel`, m2.h:305) and modulated by active aspects from `compute_aspects` (a trine/conjunction amplifies the shared element; a square introduces cross-element tension). Akasha/ether is NOT in the four-element bioquaternion accumulator — the bioquaternion is the operative quartet only (Earth/Fire/Water/Air; cf. Track 37 codon→quaternion chain `q = {w:Earth, x:Fire, y:Water, z:Air}`).
    - The widget renders the four-element bar (Earth/Fire/Water/Air) with each planet's contribution as a stacked segment tinted by the Track-23.4 colour-binary palette; aspect lines (from `aspect.rs`) connect contributing planets with trine/square glyphs.
    - The typed `M2ElementalWeightContribution` is exported from `m2-parashakti/src/common/composition.ts` (extending the 23.12 contribution module) so the kernel-bridge can route it to the PASU `elemental_weights` carrier and the M4 `bioquaternion_handles` (consumption lives in Track 25 / Track 18.10 — M2 only produces).

    Scope discipline: the four-element vector is cosmic-public (the planetary sky is shared); the PASU consumption that binds it to a personal being-pattern is `protected-m4` and lives downstream. M2 contributes the public elemental feed; it never reads or renders the personal PASU register (consistent with the `personalScopeBlocked` law at `meaning-packet.ts:213-217`).

    Verification: `test -f Body/M/epi-theia/extensions/m2-parashakti/src/browser/components/PlanetaryElementalFeed.tsx`; `pnpm --filter @pratibimba/m2-parashakti test:planetary-elemental-feed` asserts (a) the four-element vector sums the nine orbiting planets' `elem_sig` elements (Sun excluded) from a kernel-bridge mock; (b) aspect amplification from `compute_aspects` modulates the weights; (c) `M2ElementalWeightContribution` is exported and shaped for the PASU `elemental_weights` carrier; (d) outer planets (7-9) contribute their `.rodata` element honestly with the 23.10 pending badge where dataset is absent; (e) `! grep -rn "personal-pratibimba\|protected-m4" <feed component>` writes (M2 produces public feed only); (f) `grep -n "elemental_weights\|M2ElementalWeightContribution" Body/M/epi-theia/extensions/m2-parashakti/src/common/composition.ts` resolves.

20. **23.20 — Cymatic MonoPoly engine: Archetype-5 wave-behaviour states driven by the 72-fold resonance** *(spec-ahead-integration; consumes m2.h `M2_TO_M3_CYMATIC_PROJECTION[72]` + `M2_CAUSAL_RESONANCE_MASKS[36]`; M0 MonoPoly dialectic #0-3-8; cross-link Track 21 M0 surface, Track 37)*

    Author `m2-parashakti/src/browser/components/CymaticMonoPolyEngine.tsx` — a profile-tick-driven wave-behaviour classifier that drives the `CymaticChladniSurface` (23.4) behaviour-state from the live 72-fold resonance superposition. The Archetype-5 (Dynamic Harmony, M0 `#0-3-8`, the MonoPoly crossroads — verified `M0-anuttara-language-architecture.md:328`, MonoPoly dialectic at §E:416-438) is the wave-behaviour layer of the cymatic surface: how many tones are sounding, and whether they cohere.

    The four wave-behaviour states (mapped to the MonoPoly dialectic positions, verified `M0-anuttara-language-architecture.md:420-428`):
    - **Mono — one tone** (MonoPoly position 0/1, "Mono — The One"): a single dominant frequency in the `audio_octet`; the Chladni figure is a clean single-mode standing wave. Stable.
    - **ActuallyMany — interference** (position 3, "Poly- — Actually Many"): multiple uncorrelated frequencies superpose; the surface shows interference/beat patterns. The 72-fold resonance masks (`M2_CAUSAL_RESONANCE_MASKS[36]`, m2.h:561) show low mutual resonance — the tones are genuinely many.
    - **ActualisingOne — forced-lock warning** (position 5, "Mono- — Actualising The One"): the system is being driven toward a single mode that the resonance structure does not naturally support — a forced lock. The engine renders a warning halo: this is coherence imposed, not emergent (the cymatic equivalent of mode-locking under external drive).
    - **MonoPoly — stable many-in-one** (position 7, "(M)Any-One", complete synthesis): many frequencies sound, but the resonance masks show they mutually reinforce into a stable composite standing wave — the many cohere as one without collapse. This is the cymatic telos.

    Engine mechanics (tick-driven, reads-only):
    - `kernelBridge.m2.cymaticMonoPolyState(address72)` returns a typed `{ behaviourState: 'mono' | 'actually-many' | 'actualising-one' | 'monopoly', activeToneCount: number, mutualResonance: number /* 0..1 from M2_CAUSAL_RESONANCE_MASKS */, projection64: number }`. The kernel reads the 72-fold cymatic projection (`M2_TO_M3_CYMATIC_PROJECTION[72]`, m2.h:530 — the bitwise-OR wave-superposition that bridges to M3's 64 codon bitboard via `transduce_vibration_to_symbol`, m2.h:533-542) and the causal resonance masks; the widget classifies and renders. No local resonance math.
    - The behaviour-state drives the `CymaticChladniSurface` (23.4) render mode: Mono → single clean mode; ActuallyMany → interference texture; ActualisingOne → warning halo + flicker; MonoPoly → stable composite bloom. The state rides the profile-tick; transitions between states animate over one tick.
    - This couples to the 72→64 bridge engine (23.18): the `projection64` is the same descent, but where 23.18 shows the structural compression, 23.20 shows the WAVE BEHAVIOUR of the superposed 72-fold field as it resolves toward (or fails to resolve toward) coherence.

    Cross-link: the MonoPoly dialectic surface proper lives at M0 (Track 21, the concurrent agent's M0 frontend, `#0-3-8`); this tranche surfaces the cymatic/wave-behaviour PROJECTION of those archetypal states onto the Parashakti standing-wave. The two must agree on the four-state vocabulary — reconcile the state enum with Track 21's MonoPoly render so the same `behaviourState` string is used both places (open coordination item, see Closing).

    Verification: `test -f Body/M/epi-theia/extensions/m2-parashakti/src/browser/components/CymaticMonoPolyEngine.tsx`; `pnpm --filter @pratibimba/m2-parashakti test:cymatic-monopoly` asserts (a) the four behaviour-states each render a distinct Chladni behaviour from fixture `(audio_octet, resonance)` input; (b) ActualisingOne renders the forced-lock warning halo; (c) MonoPoly renders the stable composite bloom; (d) state derives from `M2_CAUSAL_RESONANCE_MASKS` mutual-resonance via kernel-bridge mock, not local math; (e) state-transitions ride the profile-tick; (f) `! grep -rn "M2_CAUSAL_RESONANCE_MASKS\|M2_TO_M3_CYMATIC_PROJECTION" <engine component>` (constants live in the kernel, not the widget).
...
The 22-row matrix shows the substrate is largely landed; the genuine cycle-3 wave-C work is the three-layer deep-design build (23.2 + 23.3 + 23.4) + the correspondence-tree axis filter (23.5) + the composition mount-point for the K² torus (23.12) + the inline-provenance contract consumer (23.14). The 23.9 spheres variant is promoted to the full solar-chakral implementation specified above; only 23.13's named-no-audio bridge and Tranche 3.7-M2's music-tech bridge remain deferred here.

The biological-quaternionic cross-layer band (23.18 + 23.19 + 23.20) is the M2↔M3 "living symbolic body" surfacing — the 72→64→56 epogdoon descent engine, the planetary-orbiter elemental-weight feed into the bioquaternion/PASU `elemental_weights`, and the cymatic MonoPoly wave-behaviour classifier. These are interactive profile-tick engines, reads-only across the composition boundary, kernel-bridge-backed; they consume the already-implemented 7-8-9 substrate (m2.h epogdoon, `M3_RES_MATRIX` 8 gaps, `M3_TAROT_CODON_MAP` 56-floor, `M2_PLANET_LUT` elemental signatures, `aspect.rs` Ptolemaic aspects) and never recompute it. Their full cross-layer ground is specced in [Track 37 — biological-quaternionic cross-layer integration](37-biological-quaternionic-cross-layer-integration.md).
