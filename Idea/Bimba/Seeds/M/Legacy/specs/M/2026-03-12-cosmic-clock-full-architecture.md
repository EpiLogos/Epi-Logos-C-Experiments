...
> The canonical precedence for field shapes, hash semantics, EarthBody, `tick12`, and oracle
> payload routing now lives in `Idea/Bimba/Seeds/M/M4'/Legacy/plans/CLOCK-AND-NARA-SPECS/00-canonical-invariants.md`,
> `Idea/Bimba/Seeds/M/M4'/Legacy/plans/CLOCK-AND-NARA-SPECS/00-spec-harmonization-plan.md`, and the dataset-bridge docs.
...
**Updated:** 2026-04-11 — Fibonacci Ground (§-1) added. See also:
  `docs/datasets/mahamaya-deep/fibonacci-60-pisano-integration.md` and
  `Idea/Bimba/Seeds/M/M4'/Legacy/plans/CLOCK-AND-NARA-SPECS/02-16-lenses-backbone-temporal.md §0`.

---

## -1. The Fibonacci Ground: Why the Clock Is 360° and Has 385 Nodes

**This section is prior to §0.** The structural facts in §0 (385 nodes, 384=360+24=64×6) are results.
This section is their generative cause.

The clock's 360° circumference is not chosen — it is generated by the **Fibonacci Ground**, whose
coordinate root is `#2-0` ("Archetypal Numerical Ground", Parashakti):

```
x² - x - 1 = 0   (x = φ, the golden ratio)  ← defining formulation of #2-0
```

The Fibonacci sequence's last digits (mod 10) repeat with period exactly **60** (the Pisano period
π(10)=60). Laid over the circle at 6° per step:

```
60 positions × 6°/position = 360°
```

The clock face is 360° because the Fibonacci ground period (60) inscribes itself at 6° per step.
The 6° is not a choice — it is the natural quantum of the clock, the intrinsic step of its ground.

The 60-fold period is the **Least Common Multiple of the system's three fundamental clocks**:

```
LCM(6, 5, 12) = 60
  6  = QL root (#0–#5)
  5  = pentadic principle (#2-0's φ, and 360/5 = 72 = vibrational template)
  12 = zodiacal / double-QL (2×6)
```

**New structural law** (companion to the 384=64×6 law in §0):
```c
static_assert(60 * 6 == 360,
    "Fibonacci ground: LCM(QL=6, pentad=5, zodiacal=12) × 6° = full clock face");
```

**The four-level clock hierarchy** (full treatment in `02-16-lenses-backbone-temporal.md §0`):
```
Level 0 — Fibonacci Ground  (#2-0, 60-fold): generates the clock space
Level 1 — Tick              (M1, tick12 = 60/pentad = 12): the heartbeat
Level 2 — Lens              (16-fold): static partitions of the 360° space
Level 3 — Walk              (9-fold): sequential traversal patterns
```

**The central node is the Fibonacci period completion point:**
After 60 steps × 6° = 360°, the Fibonacci sequence returns to its seed state (digit 0). This return
IS the central Axis Mundi. The Fibonacci ground "arrives" at the central node after one period —
making the center not just the geometric middle but the **attractor state of the Fibonacci growth
process**. The clock face (0°–359°) is one Fibonacci orbit; the central node is where that orbit
completes and the ground state is restored.

**Ontological circuit** (complete diagram in `fibonacci-60-pisano-integration.md §4`):
```
#2-0 (Parashakti ground, x²-x-1=0) → #3-5 (clock face, 60 digits × 6°) → Central Node (degree 360°) → #2-0
```
...
**Fibonacci ground companion law** (see §-1 above):
```c
static_assert(60 * 6 == 360,
    "Fibonacci ground: LCM(QL=6, pentad=5, zodiacal=12) × 6° = full clock face");
```
The two structural laws are complementary: the first describes the clock as I-Ching transformation
space (hexagram/codon, M3). The second describes the clock as Fibonacci growth space (golden ratio,
M2/Parashakti). Both are simultaneously true — two faces of the same 360° topology.
...
typedef struct {
    ...
    // M1 layer
    uint8_t  tick12;                // 0-11: (degree % 360) / 30
    uint8_t  m1_ananda_value;       // get_ananda_harmonic(ANANDA_BIMBA, stage, decan%12)
...
The quintessence identity hash is the user's **BLAKE3 archetypal address** in symbolic-genetic
space. It does not replace the quintessence quaternion; rather, the hash is the canonical
identity surface and the quaternion is its derived elemental / rotational form. Together they let
the system hold stable identity and live orientation without collapsing them into one field.

- Deterministic from the same identity inputs
- A stable archetypal address (`quintessence_hash[32]`)
- A derived quaternion orientation in the 720° spinor space
- A specific hexagram/codon/tarot position with elemental/planetary/chakra meaning

### 8.2 The Computation Chain

```
M4 identity layers (natal required, enrichments optional)
    ↓ canonical BLAKE3 over layer presence + layer payloads
uint8_t quintessence_hash[32]      — the 256-bit archetypal identity
    ↓ independently derive weighted elemental balance from identity layers
quintessence_quaternion = normalize([w=EARTH, x=FIRE, y=WATER, z=AIR])
    ↓ derive canonical M1 discrete state
tick12 = quantize_to_spanda_substage(y, x)
    ↓ derive or read degree anchor from natal / cast / kairos context
exact_degree_720 = degree_node_360 + phase * 360
    ↓ read_clock_full(exact_degree_720, live_kairos, active_lens)
...
typedef struct {
    // Canonical identity
    uint8_t  hash[32];                   // full BLAKE3 archetypal address

    // The quaternion (unit, derived from identity layer elemental balance)
    float w, x, y, z;                    // canonical [w=EARTH, x=FIRE, y=WATER, z=AIR]

    // The torus address
    uint16_t exact_degree_720;           // position in 720° SU(2) space
    uint8_t  tick12;                     // 0-11: canonical M1 discrete state
...
// Compute from canonical identity layers plus derived elemental balance
M4_Quintessence_Identity m4_compute_quintessence_identity(
    const M4_Identity_Matrix* identity
);

// The identity hash IS the M4 identity at the C level.
// The quaternion and clock position are derived carriers of that identity.
```
...
2. **Cross-reference sources**:
   ...
   - `ANANDA_BIMBA` matrix: tick12 × decan position → Ananda harmonic
...
pub struct TorusRenderer {
    ...
    identity_pos: Option<(u16, u8)>,   // (degree, tick12) of M4 identity hash
}
...
- [ ] Derive `quintessence_quaternion` from canonical identity layers
- [ ] Derive `tick12` from `quantize_to_spanda_substage(y, x)`
- [ ] Use natal / kairos / oracle anchors for `exact_degree_720`; do NOT infer degree directly from quaternion
- [ ] Compute canonical `quintessence_hash[32]` from identity payloads
