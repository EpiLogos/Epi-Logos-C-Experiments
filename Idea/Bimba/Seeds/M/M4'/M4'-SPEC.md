...
- **Journal surface**: an anchored day IS the daily-note editor (CodeMirror, debounced write-back through the scoped vault service); the journal timeline reads real Present day containers from the substrate (no gateway `nara.journal.timeline` method exists yet — when S3 grows one, the pane swaps sources without shape change).
...
- Open (unchanged by this foothold): psychoid-cymatic field renderer, Dream Journal modality, highlight sendoff envelopes, Graphiti episode deposition, session-NOW integration.
