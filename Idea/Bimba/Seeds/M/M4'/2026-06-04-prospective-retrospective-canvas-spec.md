...
- **App surface targeting (carrier correction 2026-07-16).** The old Electron renderer at `Body/S/S3/epi-app/` is retired genealogy and `Body/M/epi-theia/` is a frozen parts warehouse. The canonical app surface is `Body/M/pratibimba-app/`; Tiptap/highlight internals may be ported from the frozen `m4-nara` extension, but all new build lands in the active carrier. Rerun `11.T11.10` lands the minimum viable canvas in `src/panes/NaraCanvasEditor.tsx` with real vault write-back.
...
**Second**, the UX. The active target is the `NowPane` personal-day surface in `Body/M/pratibimba-app/`. Its day anchor and scoped vault IPC already own real `Idea/Empty/Present/{day_id}/daily-note.md` bytes. The frozen Theia `m4-nara` extension remains the port source for its Tiptap mark/service/menu internals and for the canonical `'agent-chat'` artifact kind; it is not an executable target. The portal-core Rust side stays canonical too: `NaraActivityKind::Highlight` distinguishes highlights from journal/dream/oracle inputs.

Rerun `11.T11.10` now provides the canvas-as-input editor, Markdown-aware `HighlightMark`, component-local `HighlightService`, floating selection menu, and protected-local agent-selection intents in the active carrier. It deliberately corrects the frozen port by persisting body edits through `vault_write` while byte-preserving frontmatter. Rerun `11.T11.11` adds the complete six-category agent visual register and non-destructive `inscribeAgentMark` bridge. Rerun `11.T11.12` mounts the ambient/tuning chrome: optional Medicine/spread producers remain honestly pending when absent, while all three tuning keys structured-write to the latest real session NOW. Still to land are runtime agent dispatch/deposition and the broader temporal control-plane operations.
