...
| **M2-5** Planetary-Chakral Harmonic Integration / DET | **M2-5' Solar-Chakral Runtime + Symbolic Projection Gate**       | Canonical 10-planet LUT with Cousto frequencies/Keplerian velocities/digital-roots ([`m2.h:299-311`](Body/S/S0/epi-lib/include/m2.h), [`m2.c:264-295`](Body/S/S0/epi-lib/src/m2.c)); Earth is the 10th planet as observer-centre; DET `M2_TO_M3_CYMATIC_PROJECTION[72]` 72×u64 masks ([`m2.h:530`](Body/S/S0/epi-lib/include/m2.h), [`m2.c:821`](Body/S/S0/epi-lib/src/m2.c)); 9:8 epogdoon compress/expand ([`m2.h:545-551`](Body/S/S0/epi-lib/include/m2.h)) | Holds live solar-chakral state; emits cymatic frame packets (CPU reference); projects M3-ready 64-bit symbolic evidence via DET WITHOUT classifying codons; Earth-as-observer clock/map centre for the 9:8 bridge                                |
...
`M2_PLANET_LUT[10]` is canon, with Earth as the 10th planet in the observer-centre sense: the geocentric clock/map centre from which the planetary-chakral field is read, not a separate handle layered on top of a 10-planet list. **Venus carries the 9:8 epogdoon** — visible in the Cousto frequency ratio `Venus/Sun = 221/126 ≈ 1.754` and structurally as the beauty-operator of the M2→M3 compression. The 9:8 explicit bake-in at `m2_epogdoon_compress(val_72) = val_72*8/9` ([`m2.h:545-547`](Body/S/S0/epi-lib/include/m2.h)).

The 10-LUT enters the 72-space via `decan_link → planet_ruler` (`M2_DECAN_DESC[i].ruling_planet`) and `shem_link → planet_link` (`M2_SHEM_DESC[i].planet_link`); the planet itself is not 72-cardinality. **DCC-03 / DR-M2-1 is ratified:** keep `M2_PLANET_LUT[10]`; count the 9 non-Earth planets × 8 chakras as the literal `9:8` epogdoon; treat Earth as observer-centre rather than a separate bridge field.
...
3. **Planet-count semantics are ratified** — `M2_PLANET_LUT[10]` is canon, Earth is the 10th planet as observer-centre, and the 9:8 bridge counts nine non-Earth planets to eight chakral/nodal reception points. No new bridge field is required; existing observer handles are provenance for the centre, not an extra planet.
...
- **Tranche 3.5-M2** — DCC-03 planet-count + Earth-observer semantics ratified
...
12. **DCC-03 ratified** (Tranche 3.5-M2): `M2_PLANET_LUT[10]` is canon; Earth is the 10th planet as observer-centre; the 9:8 epogdoon counts nine non-Earth planets to eight chakral/nodal reception points; the old planet-count decision marker is absent from [`meaning-packet.ts`](Body/M/epi-theia/extensions/m2-parashakti/src/common/meaning-packet.ts).
