...
> **The M1' -> M2' Boundary:** M1' is the sole audio/frequency genesis engine. M2' is the Parashakti harmonic-correspondential instrument: it consumes M1''s pitch/tick stream, reads it through M2's 72-fold symbolic-vibrational field, and includes a cymatic renderer that consumes the 8+4 stream as visible Chladni/standing-wave geometry.
...
> **Companion documents:**
> - [`m1-prime-paramasiva-instrument.md`](../M1'/m1-prime-paramasiva-instrument.md) - the M1' instrument spec and the authoritative producer of the 8+4 bus
...
M2' is **Parashakti read from the playable correspondence side**. It is not a second audio source and not merely a visualizer. It is the functional surface where Parashakti's 72-fold vibrational architecture receives M1''s produced pitch/tick stream, reads it through MEF, tattva, decan, sacred name, mantra, maqam, planetary, and chakral systems, and then makes that field directly engageable as matrix, mode, body, clock, and cymatic form.

The cymatic renderer remains essential, but it is now named correctly: it is a **module within M2'**, not the identity of M2' as such. M2' is the harmonic-correspondential instrument; cymatics is the visible standing-wave expression of that instrument when the M1' 8+4 bus is routed into a material/medium solver.

The M1' spec made the boundary explicit: the 8 explicate-sung positions of the matheme become `m1_prime_audio_octet`, while the 4 implicate-nodal positions become `m1_prime_nodal_quartet`. M2' consumes both. The eight are not re-generated by M2'. The four are not decorative metadata. The visible Chladni pattern is the intersection of those two streams: antinodal motion from the eight, nodal constraint from the four.
...
M2' begins where M1' hands off, but its field is broader than the cymatic renderer:

```text
M1' audio octet                 ->  eight explicate antinodal drivers
       |
       v
M1' nodal quartet               ->  four implicate boundary constraints
       |
       v
