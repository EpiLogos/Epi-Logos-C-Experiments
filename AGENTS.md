...
- **`node .codex/scripts/verify-tranche.mjs <TASK_ID> --owner <VERIFIER_ID>`** — the independent verifier (verifier ≠ closer): re-executes a tranche's Verify commands fresh, records verbatim output + `verifier-owner` under `plan.runs/verifications/`, refuses a pass while any gate stage is red.
