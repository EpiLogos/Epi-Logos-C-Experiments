...
## Verification Law (Track 00 harness — gates every rerun tranche)

Cycle 3 died of tests that asserted contract strings instead of running systems. The Track-00 harness ([[2026-07-03-m-prime-cycle-3-full-rerun/CHARTER]] Verification law) is the repo-wide gate:
...
- **`node Body/M/pratibimba-app/scripts/live-wire.mjs`** — spawns a REAL gateway, strict-parses every `profile.update`/`m123.chime` frame against the Zod contracts, asserts the declared projection manifest, dumps captures to the plan's `plan.runs/wire-captures/`. A tranche landing a profile field or gateway method MUST register its assertion in `PROJECTION_MANIFEST`.
- **`node .codex/scripts/lint-test-honesty.mjs`** — rejects test blocks whose only assertions are source-greps / file-existence / manifest-string / test-count, and ledger evidence in the banned classes ("design only", "deferred", "direct close", file-existence). Polyglot reach (T16): Rust `assert!(true)` / self-equal constant asserts / blanket `#[ignore]` without a reason across `Body/S`; Python bare `assert True` / reasonless skip (epi-gnostic); C literal-true asserts (epi-lib). Every `#[ignore]` must carry its reason (live-infra fence or expected-red). Allowlist (justification required): `.codex/scripts/test-honesty-allowlist.json`.
- **`node .codex/scripts/kernel-truth.mjs`** — runs the expected-red kernel-truth tests against `plan.runs/kernel-truth-expected-failures.json`; a standing red is its owning track's work order; an unexpectedly-green or unmanifested red fails the stage until investigated.
- **`node .codex/scripts/verify-tranche.mjs <TASK_ID> [<TASK_ID>…] --owner <VERIFIER_ID>`** — the independent verifier (verifier ≠ closer): re-executes each tranche's Verify commands fresh, records verbatim output + `verifier-owner` under `plan.runs/verifications/`, refuses a pass while any gate stage is red. Batching ids runs the shared gate ONCE for N records. The gate is **class-scoped** (K/W/UF/D per `verification-classes.json`) — proper verification, not a straitjacket; `--full` runs the whole 28-suite sweep and is reserved for nightly/checkpoint passes. A machine-wide **gate-lane lock** (`.codex/verify.lock`) queues concurrent verify runs so parallel sessions never collide on the fixed gateway/tmux resources.
...
- **External review continuation:** `m-dev-plan-assess.mjs --continue-through-review` is opt-in only when a human owns review. It makes `review` dependencies provisional for route/claim scheduling, never automatically dispatches verification, and refuses `done` closure until every direct dependency is `done`.
- **Real-UI gate (macOS-shaped):** this is one product surface with two verification layers. `tauri-driver` does not support macOS, so Playwright drives the renderer bundle in real Chromium against a REAL spawned gateway (`pnpm test:e2e` in `Body/M/pratibimba-app`, suite `app-ui-flow`) with Tauri IPC shimmed to a real-filesystem temp-vault sidecar. Chromium is interaction coverage, not native-boot authority: `pnpm smoke` (suite `app-smoke`) launches the REAL Tauri/WKWebView carrier and requires a non-zero mounted FlexLayout receipt before its gateway heartbeat checks. Both run inside verify-all.
