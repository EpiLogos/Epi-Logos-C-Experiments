...
`Map` is the M0–M5 ontological map made navigable at repo level — the **downward / reflection** branch of the wikilink↔Neo4j system, the complement to the **upward / crystallisation** branches (`Seeds`, `World`, which author *into* the graph). It does not author or re-promote data; the graph already holds it. `Map` is where the coordinate *relations* become legible in-repo, so work stays modular instead of guessing how `S/S'` and `M/M'` coordinates reflect each other.

`Map/datasets/` is the raw per-branch dataset JSON (the legacy `#`-tagged import provenance + build scripts) and is not a navigation surface. `Map/M0/` through `Map/M5/` are a **generated projection** (`c_4_artifact_role: "map-index"`): one pithy, wikilink-open `{coord}.md` per coordinate, in a directory tree that mirrors the coordinate hierarchy (a coordinate with children is a folder with a `{coord}.md` folder-note; leaves are files). Each node carries its M-form coordinate identity, a verbatim pithy essence, its structural `Contains`, and a full `Relations` index — every dataset edge as `[[source]] - [[relation_type]] - [[target]]` with key relation properties, relation types rendered as wikilinks so their backlinks index every edge of a type. Full node detail stays in Neo4j; this is its navigable surface.

The projection is regenerated by `Map/datasets/scripts/project-map-index.mjs`, never hand-edited. See [[45-bimba-map-indexing-and-dox-okf-unification]].
